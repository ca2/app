...
vec2 pmod(vec2 p,::f32 n){
  ::f32 np = PI*2./n;
  ::f32 r = atan(p.x,p.y)-0.5*np;
  r =  mod(r,np)-0.5*np;
  return length(p)*vec2(cos(r),sin(r));
}

::f32 cube(vec3 p,vec3 s){
  vec3 q =abs(p);
  vec3 m = max(s-q,0.);
...
::f32 tube(vec3 p){
  ::f32 st = max(abs(p.z)-0.2,0.);
  return length(vec2(length(p.xy)-0.02,st));
}

vec4 cllo(vec3 p,::f32 s){
  vec3 col = vec3(0.5,1.,2.);
  ::f32 ex = 999.;
  ::f32 s1 = cube(p,vec3(ex,s,s));
  ::f32 s2 = cube(p,vec3(s,ex,s));
  ::f32 s3 = cube(p,vec3(s,s,ex));
  if(s1<s2&&s1<s3){
    col = vec3(2.,0.4,0.7);
  }
