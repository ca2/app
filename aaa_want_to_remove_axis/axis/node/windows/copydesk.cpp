...
namespace windows
{
   ...
   HGLOBAL copydesk::hglobal_get_image(::image * pimage)
   {
   ...
   bool copydesk::_get_filea(::file::patha & patha, e_op & eop)
   {
      ...
      for (UINT ui = 0; ui < c; ui++)
      {

         UINT uiLen = ::DragQueryFileW(hdrop, ui, nullptr, 0);

         wstring wstr(get_buffer, uiLen);

         ::DragQueryFileW(hdrop, ui, wstr, (UINT) wstr.length());
...
#undef new
   ...
   bool copydesk::_image_to_desk(::image * pimage)
   {
