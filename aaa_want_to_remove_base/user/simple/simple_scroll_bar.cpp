...
if(m_eorientation == orientation_horizontal)
{
puiParent->SendMessage(WM_HSCROLL, MAKEWPARAM(SB_LINERIGHT, m_scrollinfo.nPos), (LPARAM) this->get_handle());
}
else
{
puiParent->SendMessage(WM_VSCROLL, MAKEWPARAM(SB_LINEDOWN, m_scrollinfo.nPos), (LPARAM) this->get_handle());
}*/
...
//void simple_scroll_bar::OnDisplayChange(i32 iBitsPerPixel, const ::size sizeScreen)
//{
...
void simple_scroll_bar::pre_translate_message(::message::message * pmessage)
{
   ...
   if(pbase->m_puserinteraction == this)
   {
      switch(pbase->m_id)
      {
      case :
         //OnDisplayChange((i32) pbase->m_wparam, size(LOWORD(pbase->m_lparam), HIWORD(pbase->m_lparam)));
         pbase->m_bRet = true;
         return;
      default:
         break;
