...
namespace user
{
   ...
   void document::update_title()
   {
      ...
      __pointer(::user::frame_window) pframe = get_view(0)->GetParentFrame();
   ...
   void document::report_save_load_exception(const var & varFile, file_result presult, bool bSave, const char * pszDefault)
   {
      ...
      try
      {
         ...
         System.message_box(strPrompt + message_box_icon_exclamation);
   ...
   void document::add_view(::user::impact * pview)
   {
      ...
      ASSERT(pview->::user::impact::get_document() == nullptr); // must not be already attached
