...
namespace user
{
   ...
   void plain_edit::_001OnDraw(::draw2d::graphics_pointer & pgraphics)
   {
      ...
      COLORREF crEditBackground = _001GetColor(::user::color_edit_background);
...
//      string_array & straLines = m_plines->lines;
            ...
            if (m_errora.get_size() > 0)
            {
               ...
               if (m_errora[0].m_tick.elapsed() > tickTimeout)
               {
                  ...
                  if (m_errora[0].m_tick.elapsed() < (tickTimeout + tickPeriod))
                  {
                     ...
                     iErrorA = (m_errora[0].m_tick.elapsed() - tickTimeout) * 255 / tickPeriod;
   ...
   void plain_edit::_001OnCreate(::message::message * pmessage)
   {
      ...
      if (m_ptree == nullptr)
      {
         ...
         _001OnSetText(::source_system);

         _001OnUpdate(::source_system);

      }

      m_ppropertyText = fetch_property(m_id);
   ...
   void plain_edit::_001OnRButtonDown(::message::message * pmessage)
   {
...

      {
         ...
         strsize iHit = char_hit_test(point.x, point.y);
   ...
   void plain_edit::_001GetText(string & str, index iBegParam, index iEndParam) const
   {
      ...
      if (iEndParam < 0)
      {
         ...
         if (iBegParam < 0)
         {
            ...
            iEnd = (filesize)m_ptree->m_editfile.get_length();
      ...
      m_ptree->m_editfile.seek((filesize)iBeg, ::file::seek_begin);
   ...
   void plain_edit::_001SetSelText(const char * psz, const ::action_context & context)
   {

      {
         ...
         m_ptree->m_editfile.Delete((memsize)(m_ptree->m_iSelEnd - m_ptree->m_iSelBeg));
      ...
      if (plain_edit_is_enabled())
      {
         ...
         if (m_bLMouseDown)
         {
            ...
            if (m_pointLastCursor != point)
            {
               ...
               _001SetSelEnd(char_hit_test(point.x, point.y));
   ...
   void plain_edit::_001OnLButtonDown(::message::message * pmessage)
   {
      ...
      if (plain_edit_is_enabled())
      {
...

         {
            ...
            m_ptree->m_iSelBeg = char_hit_test(point.x, point.y);
   ...
   void plain_edit::_001OnCalcOffset(index iLineUpdate)
   {
      ...
      ::draw2d::graphics_pointer pgraphics = m_pmemorygraphics;
   ...
   void plain_edit::_001OnCalcLayout(index iLineUpdate)
   {
      ...
      if (!m_bMultiLine)
      {
         ...
         if (strTextPrevious != strText)
         {
            ...
            m_ptree->m_editfile.seek(0, ::file::seek_begin);
            m_ptree->m_editfile.Delete((memsize)m_ptree->m_editfile.get_length());
            m_ptree->m_editfile.seek(0, ::file::seek_begin);
...
//      string_array & straLines = m_straLines;
            ...
            while (*pszNext != '\0')
            {
               ...
               if (size.cx > rectClient.width() + 200)
               {
                  ...
                  while (*psz != '\0')
                  {
                     ...
                     m_daExtent[m_iLineStart + i][(psz - pszStart)] = -1;
                     psz++;
   ...
   index plain_edit::SelToLineX(strsize iSel, i32 & x)
   {
      ...
      for (index iLine = 0; iLine < m_iaLineLen.get_size(); iLine++)
      {
         ...
         if (iSel < i2)
         {
            ...
            x = (i32) (get_line_extent(iLine, iRel));
...
//      string_array & straLines = m_plines->lines;
   ...
   strsize plain_edit::LineXToSel(index iLine, i32 x)
   {
   ...
   index plain_edit::SelToColumnX(strsize iSel, i32 & x)
   {
   ...
   strsize plain_edit::char_hit_test(i32 px, i32 py)
   {
   ...
   strsize plain_edit::line_char_hit_test(i32 px, index iLine)
   {
...
finished_update:
   ...
   void plain_edit::_001EditDelete()
   {
      ...
      if (!m_bReadOnly)
      {
         ...
         if (i1 != i2)
         {
            ...
            m_ptree->m_editfile.Delete((memsize)(i2 - i1));
         ...
         else if (m_ptree->m_iSelEnd< m_ptree->m_editfile.get_length())
         {
            ...
            m_ptree->m_editfile.Delete((memsize)(iMultiByteUtf8DeleteCount));
   ...
   bool plain_edit::_001ReplaceSel(const char * pszText, bool & bFullUpdate, index & iLineUpdate)
   {
      ...
      m_ptree->m_editfile.Insert(pszText, ansi_length(pszText));
...

         {
            ...
            else if (pkey->m_ekey == ::user::key_right)
            {
               ...
               else if (m_ptree->m_iSelEnd < m_ptree->m_editfile.get_length())
               {
                  char buf[32];
                  m_ptree->m_editfile.seek(m_ptree->m_iSelEnd, ::file::seek_begin);
                  memsize uiRead = m_ptree->m_editfile.read(buf, 32);
                  if (uiRead == 2 &&
                        buf[0] == '\r' &&
                        buf[1] == '\n')
                  {
            ...
            else if (pkey->m_ekey == ::user::key_left)
            {
               ...
               else if (m_ptree->m_iSelEnd > 0)
               {
                  if (m_ptree->m_iSelEnd > 2)
                  {
                     ...
                     memsize uiRead = m_ptree->m_editfile.read(buf, 64);
                     if (uiRead == 2 &&
                           psz[0] == '\r' &&
                           psz[1] == '\n')
                     {
            ...
            else if (pkey->m_ekey == ::user::key_end)
            {
               ...
               if (bControl)
               {
                  ...
                  m_ptree->m_iSelEnd = LineXToSel(iLine, i32 (m_iaLineLen[iLine]));
...
// HWND hwndIme = ImmGetDefaultIMEWnd(get_handle());
   ...
   void plain_edit::on_text_composition(string strText)
   {
         ...
         && !strText.contains('\n'))
      {
         ...
         m_ptree->m_editfile.change_insert_item_data(m_pitemComposing, strText);

         index i1 = m_pitemComposing->m_position + m_pitemComposing->get_extent();

         int iLineUpdate = (int) SelToLine(i1);
         ...
         if (iLineUpdate < 0)
         {
            ...
            iLineUpdate = (int) SelToLine(m_ptree->m_iSelEnd);
   ...
   void plain_edit::_001OnUpdate(const ::action_context & context)
   {
   ...
   void plain_edit::OnLineUpdate(index iLine, const ::action_context & context)
   {
   ...
   bool plain_edit::_edit_undo()
   {

      {
...

         {
            ...
            __pointer(plain_text_command) pcommand = m_pitem->m_pitem;
   ...
   bool plain_edit::edit_undo()
   {
      ...
      _001OnUpdate(::source_user);

      //_001OnAfterChangeText(::source_user);
   ...
   bool plain_edit::edit_redo()
   {

      {
         ...
         __pointer(plain_text_command) pcommand = ptreeitem->m_pitem;
   ...
   ::count plain_edit::GetRedoBranchCount()
   {
      ...
      return m_pitem->get_expandable_children_count()
             + (m_pitem->get_next() != nullptr ? 1 : 0)
             + (m_pitem->first_child() != nullptr ? 1 : 0);
   }
   ...
   void plain_edit::_001SetText(const string & strParam, const ::action_context & context)
   {
...

      {
         ...
         if(m_ptree->m_editfile.get_length() > 0)
         {
   ...
   void plain_edit::_001OnAfterChangeText(const ::action_context & context)
   {
      ...
      if(::is_set(m_ppropertyText))
      {
         ...
         Application.call_update(m_ppropertyText->m_id, ::source_sync);
   ...
   void plain_edit::_001OnSetText(const ::action_context & context)
   {
...
//         strLine = m_plines->lines[iLine - m_iLineStart];
   ...
   void plain_edit::insert_text(string strText, bool bForceNewStep)
   {
            ...
            && m_ptree->m_editfile.m_ptreeitem != m_ptree->m_editfile.m_ptreeitemFlush)
      {
         ...
         m_ptree->m_editfile.m_size += strText.get_length();
      ...
      else
      {
         ...
         m_pinsert = m_ptree->m_editfile.Insert(strText, iLength);
   ...
   void plain_edit::internal_edit_update(bool bFullUpdate, index iLineUpdate)
   {
      ...
      else if (iLineUpdate >= 0)
      {
         ...
         OnLineUpdate(iLineUpdate, ::source_user);
