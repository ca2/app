...
namespace dynamic_source
{
   ...
   script_compiler::script_compiler(::object * pobject):
      ::object(pobject)
   {
...
//      compile_library();
   ...
   void script_compiler::compile(ds_script * pscript)
   {
      ...
      strClog.Format(m_strTime / "dynamic_source/%s-compile-log.txt",strTransformName.c_str());
...
#if defined(LINUX) || defined(MACOS)
      strBuildCmd.Format(Context.dir().install() / "platform-" __PLATFORM "\\stage\\dynamic_source" / m_strDynamicSourceConfiguration + "_cl" + m_strPlat1 + ".bash");
#else
      strBuildCmd.Format(Context.dir().install() / ("platform-" __PLATFORM "\\stage\\dynamic_source_" + m_strVsTools) / m_strDynamicSourceConfiguration + ::file::path("_c") + m_strPlat1 + ".bat");
#endif
      ...
      if(!bTimeout && strLog.has_char())
      {
         ...
         strBuildCmd.Format(Context.dir().install() / "platform-" __PLATFORM"\\stage\\dynamic_source" / m_strDynamicSourceConfiguration + "_cl" + m_strPlat1 + ".bash");
#else
         strBuildCmd.Format(Context.dir().install() / ("platform-" __PLATFORM "\\stage\\dynamic_source_" + m_strVsTools) / m_strDynamicSourceConfiguration + ::file::path("_l") + m_strPlat1 + ".bat");
#endif
         ...
         if(!bTimeout && strLog.has_char())
         {
            ...
            pscript->m_strError = pscript->m_fileError.to_string();
   ...
   void script_compiler::cppize1(ds_script * pscript)
   {
      ...
      strDest += "{\r\n";
      strDest += "   return new " + m_pmanager->m_strNamespace + "_dynamic_source_script(pscript);\r\n";
      strDest += "}\r\n";
      strDest += "\r\n";
...
//
      ...
      if(!::str::ends(strFolder, "/") && !::str::ends(strFolder, "\\"))
         strFolder += "/";
      string strTemplate;
      string strSource = "platform-" __PLATFORM "/stage/dynamic_source/";
      strSource += lpcszSource;
...
//   string strN = m_pmanager->m_strNetnodePath;
      ...
      strTemplate = strFolder/ strSource;
      //#else
      // strTemplate = strFolder, "app/stage/aura/account/app/main/matter/dynamic_source_cl.bat", false);
      //#endif
      string str;
      str = Context.file().as_string(strTemplate);
      /*string strVars = getenv("VS100COMNTOOLS");
      ...
      strCmd = strFolder/ strDest;
      //#else
      // strCmd = strFolder, "app\\stage\\aura\\account\\app\\main\\front\\dynamic_source_cl.bat", false);
      //#endif
      Context.dir().mk(strCmd.folder());
      //Context.file().put_contents_utf8(strCmd, str);
      Context.file().put_contents(strCmd, str);
...
#endif
      //#else
      // plib->m_strLibraryPath.Format(strFolder, "app/stage/aura/account/app/main/front/Release/%s.dll", false), strName);
      //#endif
...
//      string strN = m_pmanager->m_strNetnodePath;
      ...
      string vars1batDst;
      string vars2batDst;
      vars1batSrc = Context.dir().install() / "platform-" __PLATFORM "/stage/dynamic_source/vc_vars.bat";
      vars2batSrc = Context.dir().install() / "platform-" __PLATFORM "/stage/dynamic_source/vc_vars_query_registry.bat";
...
//#endif
#ifdef LINUX
         strCmd.Format(Context.dir().install() / "platform-" __PLATFORM "\\stage\\dynamic_source" / m_strDynamicSourceConfiguration + "_libc" + m_strPlat1 + ".bash");
#else
         strCmd.Format(Context.dir().install() / "platform-" __PLATFORM "\\stage\\dynamic_source" / m_strDynamicSourceConfiguration + ::file::path("_libc") + m_strPlat1 + ".bat");
#endif

         //#else
         //    strCmd.Format(strFolder, "app\\stage\\aura\\account\\app\\main\\front\\dynamic_source_cl.bat", false));
         //#endif
...
//#endif
#ifdef LINUX
      strCmd.Format(Context.dir().install() / "platform-" __PLATFORM "\\stage\\dynamic_source" / m_strDynamicSourceConfiguration + "_libl" + m_strPlat1 + ".bash");
#else
      strCmd.Format(Context.dir().install() / "platform-" __PLATFORM "\\stage\\dynamic_source" / m_strDynamicSourceConfiguration + ::file::path("_libl") + m_strPlat1 + ".bat");
#endif
      //#else
      // strCmd.Format(strFolder, "app\\stage\\aura\\account\\app\\main\\front\\dynamic_source_libl.bat", false));
      //#endif
      string str = Context.file().as_string(strCmd);
...
ch_else:
   ...
   void script_compiler::run_persistent()
   {
      ...
      string strPath = m_pmanager->m_strNetseedDsCa2Path/ "aura\\persistent";
      ...
      {
         string str = stra[i];
         if(::str::begins_ci(str, m_pmanager->m_strNetseedDsCa2Path/ "aura\\persistent")
               && ::str::ends_ci(str, ".ds"))
         {
            strBody += Context.file().as_string(str);
         }
      }
      //Context.file().put_contents_utf8(strCat, strBody);
      ...
      if(pinstance != nullptr)
      {
         ...
         __pointer(::dynamic_source::ds_script) pdsscript = pinstance->m_pscript;
         if(pdsscript != nullptr)
         {
            ...
            try
            {
               ...
               m_pmanager->m_strPersistentError += pdsscript->m_fileError.to_string();
