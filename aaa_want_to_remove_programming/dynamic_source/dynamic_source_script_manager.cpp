...
namespace dynamic_source
{
   ...
   ::estatus script_manager::initialize(::object * pobjectContext)
   {
      ...
      estatus = __compose_new(m_spqueue);
   ...
   ::estatus script_manager::init_thread()
   {
      ...
      if(m_bCompiler)
      {
         ...
         m_pcache                   = __new(script_cache(get_context_object()));
         m_pcache->m_pmanager       = this;
         m_pcompiler                = __new(script_compiler(get_context_object()));
...

      {
         ...
         auto pwatcher = __new(clear_include_matches_file_watcher(get_context_object()));

         pwatcher->m_pmanager = this;

         Context.dir().watcher().add_watch(m_strNetseedDsCa2Path, pwatcher, true);
      ...
      forallref(listing)
      {
         ...
         if(::str::begins_ci(item.title(),"net-"))
         {
            ...
            Context.dir().watcher().add_watch(item, pwatcher, true);
   ...
   void script_manager::handle(::dynamic_source::httpd_socket * pdssocket)
   {
      ...
      if (pinstance != nullptr)
      {
         ...
         pinstance->initialize_dynamic_source_script_interface(pinstance, nullptr, pdssocket, this);
         pinstance->dinit();
      }
      ...
      if(pinstance != nullptr)
      {
         ...
         if (pinstance->m_iDebug > 0)
         {
            ...
            pinstance->print(pinstance->m_pscript->m_strError);

         }

         if (!pinstance->m_pscript->HasCompileOrLinkError())
         {
   ...
   var script_manager::get_output_internal(::dynamic_source::script_interface * pinstanceParent, const string & strNameParam)
   {
      ...
      if(strName.is_empty())
      {
         ...
         if(pinstanceParent != nullptr)
         {
            ...
            if(pinstanceParent->main_instance()->m_iDebug > 0)
            {
               ...
               if(pinstanceParent->m_pscript->m_streamError.m_p->get_size() > 0)
               {

                  pinstanceParent->main_instance()->netnodesocket()->response().print("script_manager::get_output_internal is_empty script parent" + pinstanceParent->m_pscript->m_strName);
      ...
      __pointer(::dynamic_source::script_interface) pinstance = get(strName, pscript);

      if (pinstanceParent != nullptr && pscript != nullptr)
      {
      ...
      pinstance->initialize_dynamic_source_script_interface(pinstanceParent->main_instance(), pinstanceParent, pinstanceParent->main_instance()->netnodesocket(), this);

      pinstance->dinit();

      return pinstance->call_run();
   ...
   void script_manager::LoadEnv()
   {
      ...
      ::file::path str;
      str = Context.dir().module();
      str.go_up(2);
      str = str/ "stage\\basis";
      str = ";" + str;
      ::file::path str2;
      str2 = Context.dir().module();
      str2.go_up(2);
...
//         memfile.transfer_from(plib->m_memfileError);
   ...
   bool script_manager::include_matches_file_exists(const string & strPath)
   {
      ...
      {
         bool bFileExists = Context.file().exists(strPath);
   ...
   bool script_manager::include_matches_is_dir(const string & strPath)
   {
      ...
      {
         bool bIsDir = Context.dir().is(strPath);
   ...
   bool script_manager::include_has_script(const string & strPath)
   {
      ...
      else
      {
         ...
         bool bHasScript = Context.file().as_string(strPath).find("<?") >= 0;
...
#define new AURA_NEW
   ...
   ::file::path script_manager::get_script_path(const ::file::path & strName, const string & strModifier)
   {
      ...
      return Context.dir().install()/m_pcompiler->m_strDynamicSourceStage /m_pcompiler->m_strStagePlatform /m_pcompiler->m_strDynamicSourceConfiguration/"dynamic_source" /strTransformName.folder()/strScript + strModifier + ".dll";

#else

      return Context.dir().install() / m_pcompiler->m_strDynamicSourceStage / m_pcompiler->m_strStagePlatform / m_pcompiler->m_strDynamicSourceConfiguration / "dynamic_source" / strTransformName.folder() / strScript + strModifier + ".so";
