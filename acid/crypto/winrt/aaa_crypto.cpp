...
namespace crypto
{
   ...
   **/
   ...
   string crypto::strkey()
   {
      memory storage;
      key(storage);
      return ::apexacesystem()->base64().encode(storage);
   }
   ...
   i32 crypto::encrypt(string& strEncrypt, const ::scoped_string & scopedstrDecrypt, const ::scoped_string & scopedstrKey)
   {
      ...
      ::apexacesystem()->base64().decode(storageKey, pszKey);
      i32 cipherlen = encrypt(storageEncrypt, storageDecrypt, storageKey);
      strEncrypt = ::apexacesystem()->base64().encode(storageEncrypt);
      return cipherlen;
   }
   ...
   i32 crypto::decrypt(string& strDecrypt, const ::scoped_string & scopedstrEncrypt, const ::scoped_string & scopedstrKey)
   {
      ...
      ::apexacesystem()->base64().decode(storageEncrypt, pszEncrypt);
...
pacedirectory->system() / "user" / "databin.bin";
   ...
   ::pointer<::crypto::rsa>crypto::generate_rsa_key()
   {
      ...
      return __allocate< ::winrt::rsa >(provider->CreateKeyPair(1024));
...
//   {
//
//      auto popensslrsa = __allocate< ::winrt::rsa >();
//
...
//      }
//
//#if (defined(UNIVERSAL_WINDOWS) && (!defined(RASPBERRYPIOS))) && (OPENSSL_API_COMPAT < 0x10100000L)
//
...
//
   ::pointer<rsa>crypto::read_priv_pem(const string& strFile)
   {
      ...
      auto pwinrtrsa = __allocate< ::winrt::rsa >();
