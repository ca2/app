...
#include "acid/operating_system/universal_windows/file_winrt.h"
#endif
...
using namespace ::winrt::Windowsacesystem();
...
//      wstring wstrModuleFilePath(get_buffer, MAX_PATH * 8);
//
//      GetModuleFileNameW(::GetModuleHandleA("acid.dll"), wstrModuleFilePath, MAX_PATH + 1);
//
...
//         }
//
//         str = ::dir::pathfind(::file::path(str).folder(), "libace.dylib", "rfs"); // readable - normal file - non zero size_f64
//
//         if(str.has_char())
//         {
//
...
//   #endif
//
//      s_pstrCalc = aaa_primitive_new string(str);
//
//      return *s_pstrCalc;
//
//
...
pacedirectory->create CreateDirectoryW last error(%d)=%s", dwError, pszError);
   ...
   ::file::path pathfind(const string & pszEnv,const string & pszTopic,const string & pszMode)
   {
      ...
      for(i32 i = 0; i < stra.get_count(); i++)
      {
         ...
         if(acefile()->exists(strCandidate))
         {
            return strCandidate;
         }
...
pacedirectory->create(psz) != false;
