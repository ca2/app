...
#include "acid/compress/not_dir.h"
#include "acid/constant/id.h"
#include "acid/exception/interface_only.h"
#include "acid/filesystem/file/folder.h"
...
#include "acid/filesystem/filesystem/dir_system.h"
...
bool dir_context::enumerate(::file::listing& listing)
{
   ...
   return acedirectory()->enumerate(listing);
...
bool dir_context::_enumerate(::file::listing& listing)
{
   ...
   if(acedirectory()->defer_enumerate_protocol(listing))
   {
   ...
   if(acedirectory()->defer_enumerate_media_library(listing))
   {
...
::file::e_type dir_context::cached_file_type(const ::file::path& path)
{
   ...
   if (string_ends_ci(path, ".lnk"))
   {
      ...
      auto plink = acepath()->resolve_link(pathTarget);
...
::file::e_type dir_context::_file_type(const ::file::path& path)
{
   ...
   return acedirectory()->file_type(path);
   ...
   if (psystem->m_pdirsystem->m_bMatterFromHttpCache)
   {
      ...
      if (psystem->m_pdirsystem->m_pathLocalAppMatterCacheFolder.has_char())
      {
         ...
         if (!(patha[0].flags() & ::file::e_flag_bypass_cache) && path.has_char())
         {
            ...
            auto etype = acepath()->get_type(strFinal);
   ...
   if (psystem->m_pdirsystem->m_bMatterFromHttpCache)
   {
      ...
      for (auto& strLs : straLocaleSchema)
      {
         ...
         for (auto& pathItem : patha)
         {
            ...
            path = acedirectory()->ca2roaming() / "_matter" / strMatter;

            //                  if (::acepath()->is_file_or_dir(path, nullptr))
            //                  {
            //
            //                     goto ret;
            //
      ...
      if (strMatter.has_char() && ::found(iFind))
      {
         ...
         if (bDir)
         {
            ...
            acedirectory()->create(psystem->m_pdirsystem->m_pathLocalAppMatterFolder / strMatter);
   ...
   else
   {
      ...
      for (auto& strLs : straLocaleSchema)
      {
         ...
         for (auto& pathItem : patha)
         {
            ...
            auto etype = acepath()->get_type(path);
...
ret:
   ...
   if (pathCache.has_char() && psystem->m_pdirsystem->m_bMatterFromHttpCache)
   {
      ...
      //   || (!(path & ::file::e_flag_bypass_cache) && acepath()->is_file_or_dir(pathCache, nullptr)))
      //{
...
::file::path dir_context::appdata(const ::string& strAppIdParameter)
{
   ...
   return acedirectory()->roaming() / strAppId;
...
::file::path dir_context::cache()
{
   ...
   return acedirectory()->ca2roaming() / "cache";
...
::file::path dir_context::standalone()
{
   ...
   return acedirectory()->roaming() / application()->m_strStandalone;
...
::file::path dir_context::pathfind(const string& pszEnv, const string& pszTopic, const string& pszMode)
{
   ...
   return acedirectory()->pathfind(pszEnv, pszTopic, pszMode);
