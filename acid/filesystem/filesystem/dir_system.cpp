...
#include "acid/filesystem/watcher/watcher.h"
...
void dir_system::initialize(::particle * pparticle)
{
   ...
   //m_pathModule = psystem->m_pacedirectory->module();
...
//   }
//
   m_pathInstall = acedirectory()->install();
...
//   m_pathCa2Module = pathCa2ModuleFolder;
   ...
   auto pacedirectory = acedirectory();

   #if defined(__APPLE__) || (defined(DEBUG)) || defined(ANDROID) || defined(UNIVERSAL_WINDOWS)

   if ( pacedirectory->is(system()->side_get_matter_path("app/_matter/main")))
   {
   ...
   pacedirectory->create(acedirectory()->bookmark());
   ...
   ::string strAppId = acedirectory()->appid();
...
::file::path dir_system::compute_default_app_data_path()
{
   ...
   return acedirectory()->home() / "application";
