...
#include "acid/primitive/primitive/url_domain.h"
...
#include "acid/primitive/primitive/read_only_memory.h"
...
#include "acid/crypto/crypto.h"
#include "acid/crypto/hasher.h"
#include "acid/filesystem/filesystem/dir_system.h"
//#include "apex/networking/http/context.h"
#include "acid/platform/application.h"
#include "acid/platform/context.h"
//#include "acid/platform/machine_event.h"
//#include "acid/platform/machine_event_central.h"
//#include "acid/platform/os_context.h"
...
::string file_context::get_configuration_text(const ::scoped_string &scopedConfigurationFile)
{
   ...
   try
   {
      ...
      str = as_string(acedirectory()->config() / scopedConfigurationFile);
...
void file_context::copy(::payload varTarget, ::payload varSource, bool bFailIfExists, enum_extract eextract)
{
   ...
   if(preader.nok())
   {
      ...
      if (exists(pathSource))
      {
         ...
         pacefile->copy(pathTarget, pathSource, !bFailIfExists);
...
::file::path file_context::sys_temp(const ::scoped_string & scopedstrName, const ::scoped_string & scopedstrExtension)
{
   ...
   string strTempDir = acedirectory()->sys_temp();
...
::file::path file_context::sys_temp_unique(const ::file::path &lpszName)
{
   ...
   return acedirectory()->sys_temp() / lpszName;
...
::file_pointer file_context::get_temporary_upload_file(const ::file::path &pathCurrent)
{
   ...
   while (true)
   {
      ...
      acefile()->append_wait("C:\\ca2\\toomuchuploads.txt", strMessage);
...
file_pointer file_context::http_get_file(const ::payload &payloadFile, ::file::e_open eopen)
{
...

   {
      ...
      if (path.flags().is_clear(::file::e_flag_bypass_cache) && acefile()->exists(pathCache))
      {
...
::file::path file_context::dropbox_info_network_payload()
{
   ...
   pathNetworkPayload = acedirectory()->home() / ".dropbox/info" NETWORK_PAYLOAD_DEFAULT_EXTENSION;
...
::file::path file_context::onedrive_global_ini()
{
   ...
   pathGlobalIni = acedirectory()->ca2roaming() / "OneDrive/settings/Personal/global.ini";
...
::file::path file_context::onedrive_cid_ini()
{
   ...
   string strIni = acefile()->as_string(pathGlobalIni);
   ...
   ::file::path pathIni = acedirectory()->ca2roaming() / "OneDrive/Settings/Personal/" + strCid + ".ini";
...
::file::enum_type file_context::safe_get_type(const ::file::path& path)
{
   ...
   return acepath()->safe_get_type(path);
