#include "framework.h"
#include "request.h"
#include "acid/user/user/element.h"
...
::acid::application * request::create_get_app(::acid::application * pappFallback)
{
...
void request::_001ParseCommandLine(const ::string& strCommandLine)
{
   ...
   if (!has_property("build") || payload("build").is_empty())
   {
      ...
      if (acefile()->exists(acedirectory()->system() / "config\\plugin\\build.txt"))
      {

         string str = acefile()->as_string(acedirectory()->system() / "config\\plugin\\build.txt");
