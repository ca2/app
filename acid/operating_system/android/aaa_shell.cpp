...
#include "framework.h"
#include "_android.h"
#include "acid/compress/zip/context.h"
#include "shell.h"
...
//                     string strParams;
//
//                     //if(pcontext->m_papexcontext->file()->resolve_link(strTarget, strFilePath, ::aceacesystem()->ui_from_handle))
...
pacedirectory->config() / "android/app_theme" / m_strShellThemePrefix + strExtension + ".ico";
//
...
//            }
      ...
      if (string_ends_ci(imagekey.m_strPath, ".acid"))
      {
         ...
         if (str.case_insensitive_begins_eat("ca2prompt\r\n"))
         {
            ...
            if (crBk == 0)
            {
               ::aceacesystem()->imaging().Createcolor_blend_ImageList(
                  m_pil48,
                  m_pil48Hover,
                  rgb(255, 255, 240),
                  64);
      ...
      if (android_get_file_image(pimage48, strPath))
      {
         ...
         if (crBk == 0)
         {
            ...
            ::aceacesystem()->imaging().color_blend(
            m_pimagelist[48],
...
//         SHGetMalloc(&m_pmalloc);
   ...
   //      {
   //            ::aceacesystem()->imaging().color_blend(pimage, rgb(255, 255, 240), 64); 
   //         }
