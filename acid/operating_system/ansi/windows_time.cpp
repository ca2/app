...
PLARGE_INTEGER Time)
{
   ...
   if( tfTimeFields->Millisecond < 0 || tfTimeFields->Millisecond > 999 ||
   ...
   Time->QuadPart = (((((::i64) day * HOURSPERDAY +
                     ...
                     tfTimeFields->Millisecond ) * TICKSPERMSEC;
 ...
 *
 * PARAMS
 *   SystemTime [I] ::aceacesystem() time to convert.
...
static time_t find_dst_change(time_t minimum, time_t maximum, i32 *is_dst)
{
   ...
   tm = localtime(&start);
   *is_dst = !tm->tm_isdst;
// xxx    informationf("starting date isdst %d, %s", !*is_dst, ctime(&start));
...
//xxx    informationf("tz data will be valid through year %d\n", tm->tm_year + 1900);
...
//xxx    informationf("year_start: %s", ctime(&year_start));
...
//xxx    informationf("Standard: %s", ctimeiStandardstd));
//xxx    informationf("dlt: %s", ctime(&dlt));

   if (dlt == iStandard || !dlt || !iStandard)
   {
//xxx   informationf("there is no daylight saving rules in this time zone");
   }
   else
   {
      tmp = dlt - tzi->Bias * 60;
      tm = gmtime(&tmp);
//xxx   informationf("dlt gmtime: %s", asctime(tm));
...
//xxx        informationf("daylight (d/m/y): %u/%02u/%04u day of week %u %u:%02u:%02u.%03u bias %d\n",
...
//xxx            tzi->DaylightBias);
      ...
      tm = gmtime(&tmp);
//xxx        informationf("Standard gmtime: %s", asctime(tm));
...
//xxx        informationf("standard (d/m/y): %u/%02u/%04u day of week %u %u:%02u:%02u.%03u bias %d\n",
 ...
 *   Failure: An NTSTATUS error code indicating the problem.
 */
NTSTATUS RtlQueryTimeZoneInformation(RTL_TIME_ZONE_INFORMATION *tzinfo)
{
   init_tz_info( tzinfo );
...
NTSTATUS NtSetSystemTime(const LARGE_INTEGER *NewTime, LARGE_INTEGER *OldTime)
{
   ...
   if (!settimeofday(&tv, nullptr)) /* 0 is OK, -1 is error */
      return STATUS_SUCCESS;
   //tm_t = sec;
   // xxx errorf("Cannot set time to %s, time adjustment %ld: %s\n",
 ...
 */
CLASS_DECL_ACID int_bool LocalFileTimeToFileTime( const FILETIME *localft, LPFILETIME utcft )
{
   NTSTATUS status;
 ...
 */
CLASS_DECL_ACID int_bool FileTimeToLocalFileTime( const FILETIME *utcft, LPFILETIME localft )
{
   NTSTATUS status;
   LARGE_INTEGER local, utc;
 ...
 *   Nothing.
 */
CLASS_DECL_ACID void GetSystemTimeAsFileTime(
 ...
 *  Nothing.
 */
CLASS_DECL_ACID void GetSystemTime(LPSYSTEMTIME systime)
{
   FILETIME ft;
   LARGE_INTEGER t;
...
CLASS_DECL_ACID void GetSystemTime(system_time * psystemtime)
{
