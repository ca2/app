...
#include "acid/operating_system/linux/_linux.h"
//!!!#include <X11/Xatom.h>
////#include "sn/sn.h"
#include <gdk/gdkx.h>
...
//CLASS_DECL_ACID thread_int_ptr < DWORD_PTR > t_time1;
...
CLASS_DECL_ACID void hook_window_create(::user::interaction * pWnd);
CLASS_DECL_ACID bool unhook_window_create();
void CLASS_DECL_ACID __pre_init_dialog(
::user::interaction * pWnd, RECT32 * pRectOld, ::u32* pdwStyleOld);

void CLASS_DECL_ACID __post_init_dialog(
            ...
            if (window == 0)
            {
               ...
               try
               {
                  ...
                  if(dwLastError == 0x0000057e)
                  {
                     ...
                     ::aceacesystem()->output_error_message("cannot create a top-level child interaction_impl.");

                  }
                  else
                  {

                     ::aceacesystem()->message_box(strMessage);
   ...
   void interaction_impl::aaa_message_handler(::user::message * pusermessage)
   {
      ...
      else
      {
         ...
         if(iDebugmessage_handlerTime > 20)
         {
            ...
            ::acid::get()->platform()->informationf("interaction_impl::message handler flooded?\n");
...
//         pusermessage->m_atom == e_message_mouse_wheel)
      {
         ...
         if(m_bTranslateMouseMessageCursor && !pmouse->m_bTranslated)
         {
            ...
            if(::aceacesystem()->get_monitor_count() > 0)
            {
               ::rectangle_f64 rcMonitor;
               ::aceacesystem()->get_monitor_rectangle(0, &rcMonitor);
...
//         else
//         hWnd = ::aceacesystem()->GetMainWnd()->get_handle();*/
//      }
//
