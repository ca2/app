...
void distro__release_and_desktop_environment::initialize(::particle *pparticle)
{
   ...
   if (acefile()->exists("/etc/os-release"))
   {

      auto set = acefile()->parse_standard_configuration("/etc/os-release");
      ...
      if (m_strDistroFamily.case_insensitive_equals("arch"))
      {
         ...
         auto setArch = acefile()->parse_standard_configuration("/etc/os-release");
...
//# linuxbase.org
   ...
   else if (acefile()->exists("/etc/lsb-release"))
   {

      //# For some versions of Debian/Ubuntu without lsb_release command
      auto set = acefile()->parse_standard_configuration("/etc/lsb-release");
      //./ etc / lsb - release
   ...
   else if (acefile()->exists("/etc/debian_version"))
   {
...
//# Older Debian/Ubuntu/etc.
      ...
      strOs = "Debian";
      strVer = acefile()->as_string("/etc/debian_version");
      strOs.make_lower();
      strVer.make_lower();


   }
   else if (acefile()->exists("/etc/SuSe-release"))
   {
...
//              ...
   ...
   else if (acefile()->exists("/etc/redhat-release"))
   {
