...
#include "acid/filesystem/file/exception.h"
#include "acid/filesystem/file/status.h"
   ...
   if (!m_pfile)
   {
      ...
      if (!estatus)
      {
         ...
         if (estatus == error_file_access_denied)
         {
            ...
            if (acefile()->exists(path))
            {

               if (!bTriedSetFileNormal)
               {

                  acefile()->set_file_normal(path);

                  acefile()->clear_read_only(path);
...
::pointer <stdio_file> stdio_open(::particle * pparticle, const ::file::path & pathParam, const ::scoped_string & scopedstrAttrs, int iShare)
{
   ...
   auto path = pparticle->acepath()->defer_process_relative_path(pathParam);
...
memory ace_file::as_memory(const ::file::path & pathParam, strsize iReadAtMostByteCount, bool bNoExceptionIfNotFound)
{
...
memory ace_file::safe_get_memory(const ::file::path & pathParam, strsize iReadAtMostByteCount, bool bNoExceptionIfNotFound)
{
...
CLASS_DECL_ACID FILE * trace_level_FILE(enum_trace_level etracelevel, enum_trace_level etracelevelInformation)
{
...
CLASS_DECL_ACID trace_function std_inline_log(enum_trace_level etracelevelInformation)
{
...
CLASS_DECL_ACID trace_function std_get_output(::string * pstrOutput)
{
...
memsize ace_file::as_memory(const ::file::path & pathParam, void * p, memsize s)
{
...
void ace_file::as_memory(memory_base & memory, const ::file::path & pathParam, memsize iReadAtMostByteCount, bool bNoExceptionOnOpen)
{
...
void ace_file::append_wait(const ::file::path & pathFile, const block & block, const class time & time)
{

   auto pacedirectory = m_pacedirectory;

   pacedirectory->create(::file_path_folder(pathFile));


   if (!pacedirectory->is(::file_path_folder(pathFile)))
   {
...
//filesize ace_file::get_size(FILE * pfile)
//{
//
...
string ace_file::line(const ::file::path & pathParam, ::collection::index iLine)
{
...
void ace_file::append_wait(const ::string & strFile, const block & block, const class time & time)
{

   m_pacedirectory->create(::file_path_folder(strFile));

   if (!m_pacedirectory->is(::file_path_folder(strFile)))
   {
