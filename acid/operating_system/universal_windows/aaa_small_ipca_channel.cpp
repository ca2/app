...
//{
   ...
   namespace inteprocess_channel
   {
      ...
      bool tx::send(const ::scoped_string & scopedstrMessage, duration durationTimeout)
      {
         ...
         string anotherappURI = m_strBaseChannel + ":///send?message=" + ::aceacesystem()->url_encode(pszMessage);
      ...
      bool tx::send(int message, void * pdata, int len, ::duration durationTimeout)
      {
         ...
         string anotherappURI = m_strBaseChannel + "://send?messagebin=" + as_string(message)+ "," + ::aceacesystem()->url_encode(::aceacesystem()->base64().encode((::u8 *)pdata, len));
