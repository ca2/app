...
extern CLASS_DECL_ACID image_array * g_pimagea;
...
namespace universal_windows
{
   ...
   void directx_base::SetDpi(float dpi)
   {
      ...
      m_window->Dispatcher->RunAsync(CoreDispatcherPriority::Normal, ref aaa_primitive_new ::winrt::Windows::UI::Core::DispatchedHandler([this,dpi]()
      {
         OnChangeDpi(dpi);

         //         ::aceacesystem()->system()->m_possystemwindow->m_bWindowSizeChange = true;
   ...
   void directx_base::UpdateForWindowSizeChange()
   {
      ...
      m_window->Dispatcher->RunAsync(CoreDispatcherPriority::Normal,ref aaa_primitive_new ::winrt::Windows::UI::Core::DispatchedHandler([this]()
      {
   ...
   void directx_base::defer_resize_top_level_windows()
   {
      ...
      if (::aceacesystem()->m_bExperienceMainFrame)
      {
   ...
   void directx_base::CreateWindowSizeDependentResources()
   {
      ...
      if(m_swapChain != nullptr)
      {
         ...
         {
            // If the device was erased for any reason, a aaa_primitive_new device and swapchain will need to be created.
            HandleDeviceLost();
      ...
      {
         // Otherwise, create a aaa_primitive_new one using the same adapter as the existing Direct3D device.
   ...
   void directx_base::ValidateDevice()
   {
      ...
      // a aaa_primitive_new D3D device must be created.
            ...
            FAILED(m_d3dDevice->GetDeviceRemovedReason()))
      {
         ...
         // Create a aaa_primitive_new device and __swap chain.
         HandleDeviceLost();
      }
   }
