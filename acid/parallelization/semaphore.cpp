...
//#include "acid/primitive/time/time.h"
////#include "acid/exception/exception.h"
#include "acid/primitive/time/timespec.h"
...
#include <sys/ipc.h>
#include <sys/sem.h>
#include "acid/operating_system/ansi/_ansi.h"
#include <errno.h>
...
semaphore::semaphore(::i32 lInitialCount, ::i32 lMaxCount, const char * pstrName, security_attributes * psecurityattributes)
{
   ...
   if(pstrName != nullptr && *pstrName != '\0')
   {
      ...
      m_strName = "/acid/time/ftok/event/" + string(pstrName);
   ...
   if(pstrName != nullptr && *pstrName != '\0')
   {
      ...
      if(strName.case_insensitive_begins("Local\\") || strName.case_insensitive_begins("Local\\"))
      {
         ...
         strPath = pacedirectory->home() / (".ca2/ftok/semaphore/" + strName);
      ...
      pacedirectory->create(::file::path(strPath).folder());
...
CLASS_DECL_ACID bool _hsynchronization_wait(::hsynchronization h, const class ::time& timeWait);
