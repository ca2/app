...
task_group::task_group(::matter * pmatter, ::enum_priority epriority)
{
   ...
   for(::collection::index iThread = iPreviousSize; iThread < cOrder ; iThread++)
   {
      ...
      if (epriority == ::e_priority_none)
      {
         ...
         ptooltask->branch(e_parallelization_asynchronous, ::e_priority_highest);

      }
      else
      {

         ptooltask->branch(e_parallelization_asynchronous, epriority);
...
void tool_task::initialize_tool_task(::task_group* pgroup)
{
   ...
   m_iIndex = 0;
   m_pevStart = __allocate< manual_reset_event >();
...
//CLASS_DECL_ACID ::task_group * get_task_group(::enum_priority epriority)
//{
//
//   return  ::apexacesystem()->tools(epriority);
//
//}
//
//
//CLASS_DECL_ACID ::task_groupet * get_task_groupet(::enum_task_tool etool)
//{
//
//   return ::apexacesystem()->toolset(etool);
//
//}
