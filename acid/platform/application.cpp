...
//
// app to application and back to acid namespace by camilo on 2022-09-17 18:54 <3ThomasBorregaardSorensen!!
#include "framework.h"
...
#include "acid/filesystem/filesystem/file_system_options.h"
#include "acid/handler/request.h"
#include "acid/parallelization/synchronous_lock.h"
#include "acid/platform/node.h"
#include "acid/platform/platform.h"
#include "acid/platform/sequencer.h"
#include "acid/platform/system.h"
#include "acid/platform/session.h"
#include "acid/primitive/string/_str.h"
#include "acid/primitive/text/context.h"
...
CLASS_DECL_ACID::file::path get_module_path();
...
application::application()
{
   ...
   //if (!::platform::platform::s_pplatform->m_paceapplication)
   //{
   
   //   ::platform::platform::s_pplatform->m_paceapplication = this;
   
   //}
   
   //m_ace.m_paceapplication = this;
   //m_ace.m_paceapplication = this;
   
   m_paceapplication = this;
...
application::~application()
{
   ...
   if (m_pplatform->m_paceapplication == this)
   {
      
      m_pplatform->m_paceapplication = nullptr;
...
void application::on_set_platform()
{
   ...
   ::acid::context::on_set_platform();
   
   factory()->add_factory_item < ::acid::system >();
   factory()->add_factory_item < ::acid::session >();
...
void application::on_initialize_particle()
{
   ...
   ::acid::context::on_initialize_particle();
...
void application::initialize_application(::platform::platform * pplatform)
{
   ...
   if (m_pplatform->m_paceapplication == nullptr)
   {
      
      m_pplatform->m_paceapplication = this;
   ...
   if (!m_pacesystem)
   {
      ...
      m_pacesystem = m_pplatform->m_psystem;
...
::i32 application::application_main(::platform::platform * pplatform)
{
   ...
   //output_debug_string("acid::application implement_application\n");
   ...
   //auto pfactoryitem = this->platform()->m_pfactory->get_factory_item<::acid::system>();
   
   //::pointer<::acid::system> psystem = pfactoryitem->create_particle();
   
   ::set_task(m_pacesystem);
   
   m_pacesystem->initialize_system();
   
   //information() << "acid implement_application system_construct";
   
   m_pacesystem->system_construct(this);
   
   //information() << "acid implement_application create_os_node";
   
   m_pacesystem->create_os_node();
   
   auto pnode = m_pacesystem->node();
...
void application::term_task()
{
   ...
   information() << "acid::application::term_task";
...
::file::path application::get_app_localconfig_folder()
{
   ...
   ::file::path pathFolder = acedirectory()->roaming() / m_strAppId;
...
::pointer<::handle::ini>application::get_ini()
{
   ...
   auto pini = acefile()->get_ini(pathIni);
...
string application::get_visual_studio_build()
{
   ...
   ::file::path path = acedirectory()->config() / "programming/vs_build.txt";
   
   string strBuild = acefile()->as_string(path);
...
void application::pos_run()
{
   ...
   information() << "acid::application::pos_run";
   ...
   catch (...)
   {
      ...
      information() << "acid::application::pos_run exception.4";
...
void* application_system(void* pApplication)
{
   ...
   auto papp = (::acid::application*)pApplication;
...
CLASS_DECL_ACID void application_send_status(::enum_status estatus, ::particle * pparticle, long long ll, void *p)
{
