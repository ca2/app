...
#include "acid/filesystem/filesystem/dir_context.h"
#include "acid/filesystem/filesystem/dir_system.h"
#include "acid/parallelization/counter.h"
#include "acid/parallelization/fork.h"
#include "acid/parallelization/retry.h"
...
#include "acid/primitive/text/data.h"
#include "acid/primitive/text/text.h"
#include "acid/primitive/text/translator.h"
...
namespace acid
{
   ...
   context::context()
   {
      ...
      m_pacesession = nullptr;
      m_pacesystem = nullptr;
      m_pacenode = nullptr;
...
//      m_ptexttranslator->m_pcontext = this;
   ...
   void context::initialize(::particle * pparticle)
   {
      ...
      if(!m_pacesystem)
      {
         ...
         m_pacesystem = pparticle->m_pcontext->m_pacesystem;
      ...
      if(!m_pacesession)
      {
         ...
         m_pacesession = pparticle->m_pcontext->m_pacesession;
      ...
      if(!m_paceapplication)
      {
         ...
         m_paceapplication = pparticle->m_pcontext->m_paceapplication;
      ...
      if (!m_pacenode)
      {
         ...
         m_pacenode = pparticle->m_pcontext->m_pacenode;
   ...
   ::dir_system * context::dirsystem()
   {
      ...
      return m_pacesystem->m_pdirsystem;
   ...
   ::file_system * context::filesystem()
   {
      ...
      return m_pacesystem->m_pfilesystem;
   ...
   ::ace_file* context::acefile() { return m_pacesystem->m_pacefile; }
   ::ace_path* context::acepath() { return m_pacesystem->m_pacepath; }
   ::ace_directory* context::acedirectory() { return m_pacesystem->m_pacedirectory; }
   ::acid::node* context::node() { return m_pacesystem->node(); }
...
//   }
      ...
      if (path.case_insensitive_begins_eat("appmatter:/"))
      {
         ...
         if (path.flags().has(::file::e_flag_get_local_path)
            || (path.flags().is_clear(::file::e_flag_bypass_cache)
               && ::is_existing_file_or_folder(acepath()->get_type(pathCache))))
         {
         ...
         if (path.flags().is_clear(::file::e_flag_bypass_cache))
         {
            ...
            string strFirstLine = acefile()->line(pathMeta, 0);
               ...
               if (!retry([this, pathMeta]()
                  {
                     ...
                     return acefile()->line(pathMeta, 0) != "processing";
         ...
         auto etype = acepath()->get_type(pathSide);
               ...
               if (!retry([&]()
                  {
                     ...
                     return acefile()->set_line(pathMeta, 0, strFsType);
               ...
               retry([&]()
                  {
                     ...
                     return acefile()->set_line(pathMeta, 0, "itdoesntexist");
   ...
   string context::matter_locator(::acid::application* papp)
   {
   ...
   void context::add_matter_locator(::acid::application* papp)
   {
...
//   }
   ...
   void context::http_download(const ::payload & payloadFile, const ::scoped_string & scopedstrUrl, const class ::time & timeTimeout)
   {
      ...
      auto pfile = acefile()->get_writer(payloadFile);
