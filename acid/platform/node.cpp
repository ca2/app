...
#include "acid/filesystem/filesystem/file_dialog.h"
#include "acid/filesystem/filesystem/folder_dialog.h"
#include "acid/handler/request.h"
#include "acid/memory/counter.h"
#include "acid/platform/exclusive.h"
#include "acid/operating_system/application.h"
#include "acid/parallelization/install_mutex.h"
...
#include "acid/nano/user/message_box.h"
#include "acid/nano/user/user.h"
#include "acid/user/user/os_theme_colors.h"
...
namespace acid
{
   ...
   ::process_identifier_array node::module_list_file_processes_identifiers(const ::scoped_string & scopedstrAppId)
   {
      ...
      pathModule = acedirectory()->roaming() / scopedstrAppId;

      pathModule /= "module_list.txt";

      string strModuleList = acefile()->as_string(pathModule);
         ...
         if (str.has_char())
         {
            ...
            if (a.get_size() >= 2)
            {
               ...
               if (strPath.has_char())
               {
                  ...
                  if (acepath()->real_path_is_same(strPath, strProcessName))
                  {
   ...
   //::pointer < ::particle > node::create_quit_particle(::pointer<::acid::node> &  pnode, ::pointer<::acid::system> & psystem)
   //{
   //   
   //   return nullptr;
   //   
   ...
   void node::node_implement_main()
   {
      ...
      if (psystem->m_paceapplication->m_bSession)
      {

         psystem->m_pacesession->init_task();

      }

      psystem->m_paceapplication->init_task();
      ...
      else
      {
         ...
         psystem->m_paceapplication->get_property_set().merge(prequest->get_property_set());

         psystem->m_paceapplication->main();
   ...
   ::file::path node::get_default_base_integration_folder()
   {
      ...
      return acedirectory()->home() / "integration/_____";
   ...
   void node::node_main()
   {
      ...
      if (psystem->m_pfnImplement || psystem->m_paceapplication->is_console())
      {
   ...
   //void node::start_application(::pointer<::acid::node>& pnode, ::pointer<::acid::system>& psystem)
   void node::start_application(::pointer<::acid::node> & pnode)
   {
   ...
   //   //ace_application_main(pApplication, argc, argv);

   //   ace_application_main(psystem);
   ...
   //   //::acid::apple::node::implement(pnode, psystem);
   ...
   void node::ace_application_main(::acid::system * psystem)
   {
...
//   void node::implement(::pointer<::acid::node>& pnode, ::pointer<::acid::system> psystem)
//   {
//
//      //      auto psystem = system();
//      //
...
//   }
   ...
   ::pointer < ::acid::exclusive > node::_get_exclusive(::particle * pparticleContext, const ::string & strName, security_attributes * psecurityattributes)
   {
   ...
   ::pointer < ::acid::exclusive > node::get_exclusive(::particle * pparticleContext, const ::string & strName, ::security_attributes * psecurityattributes)
   {
   ...
   bool node::is_application_installed(const ::file::path & pathExe, string strAppId, string & strBuild, const ::string & pszPlatform, const ::string & pszConfiguration, const ::string & pszLocale, const ::string & pszSchema)
   {
      ...
      strBuild = acefile()->as_string(path);
   ...
   void node::set_application_installed(const ::file::path & pathExe, string strAppId, const ::string & pszBuild, const ::string & pszPlatform, const ::string & pszConfiguration, const ::string & pszLocale, const ::string & pszSchema)
   {
      ...
      acefile()->put_contents(path, pszBuild);
   ...
   ::file::path node::application_installer_folder(const ::file::path & pathExe, string strAppId, const ::string & pszPlatform, const ::string & pszConfiguration, const ::string & pszLocale, const ::string & pszSchema)
   {
      ...
      return acedirectory()->ca2roaming() / "appdata" / strFolder / strAppId / pszPlatform / pszConfiguration / pszLocale / pszSchema;
   ...
   ::file::path node::get_application_path(string strAppId, const ::string & pszPlatform, const ::string & pszConfiguration)
   {
      ...
      if (pathLastRun.has_char() && acefile()->exists(pathLastRun))
      {
      ...
      pathFolder = acedirectory()->stage(strAppId, pszPlatform, pszConfiguration);
   ...
   ::file::path node::get_last_run_application_path_file(const ::string & strAppIdParam)
   {
      ...
      ::file::path pathFile = acedirectory()->roaming() / strAppId / "last_run_path.txt";
   ...
   ::file::path node::get_last_run_application_path(const ::string & strAppId)
   {
      ...
      ::file::path path = acefile()->as_string(pathFile);
   ...
   void node::set_last_run_application_path(const ::string & strAppId)
   {
      ...
      ::file::path path = acefile()->module();

      ::file::path pathFile = get_last_run_application_path_file(strAppId);

      information() << "node::set_last_run_application_path_file path:" << path;

      return acefile()->put_contents(pathFile, path);
...
//void node::get_system_time_as_file_time(file_time_t * pfile_time)
   ...
   string node::file_memory_map_path_from_name(const string& strName)
   {
      ...
      auto pathFolder = acedirectory()->get_memory_map_base_folder_path();
...
//#endif
   ...
   ::file::path node::get_executable_path_by_app_id(const ::scoped_string & scopedstrRepos, const ::scoped_string & scopedstrApp, bool bSingleExecutableVersion)
   {
      ...
      auto pathFolder = acedirectory()->roaming() / scopedstrRepos / scopedstrApp / "x64" ;
      ...
      auto pathFolder = acedirectory()->home();
...
//#endif
   ...
   //   return ::ace_windows::predicate_process([&](auto pid)
   //      {
...
return false;
   ...
   void node::create_app_shortcut(::acid::application * papp)
   {
...
::process_identifier node::current_process_identifier()
   ...
   ::file::path node::core_app_path(string strAppId)
   {
      ...
      path = "C:\\acid\\time\\x64\\basis\\" + strAppId + ".exe";
...
bool node::are_framework_shared_libraries_busy(const ::scoped_string & scopedstrRepos, const ::scoped_string & scopedstrApp)
{
   ...
   stra.add(this->library_file_name("acid"));
   ...
   ::file::path pathBin = acedirectory()->roaming() / scopedstrRepos / scopedstrApp / "x64";
   ...
   for (auto & pathSystem : pathaSystem)
   {
      ...
      for (auto & path : patha)
      {
         ...
         if (acepath()->real_path_is_same(pathSystem, path))
         {
...
//   }
   ...
   void node::register_user_auto_start(::acid::application * papplication, const string & strArguments, bool bRegister)
   {
