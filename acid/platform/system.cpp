...
#include "hyperlink.h"
#include "acid/compress/compress.h"
#include "acid/compress/uncompress.h"
#include "acid/constant/id.h"
//#include "acid/constant/idpool.h"
#include "acid/exception/library_not_loaded.h"
...
//static ::acid::system * g_psystem = nullptr;
...
//      }
      ...
      //factory()->add_factory_item<::acid::idpool>();

      //m_pace = nullptr;
      //m_pacedirectory = nullptr;
      //m_pacepath = nullptr;
   ...
   void system::initialize_system()
   {
      ...
      //set_os_data(LAYERED_ace, this);

      //information() << "acid initialize_system end";
   ...
   void system::create_os_node()
   {
      ...
      //information() <<"::acid::system create_os_node";
      ...
      if (m_pacesession)
      {
         ...
         m_pacesession->m_pacenode = m_pnode;

      }

      if (m_paceapplication)
      {

         m_paceapplication->m_pacenode = m_pnode;
   ...
   ::pointer<::factory::factory> & system::node_factory()
   {
      ...
      auto & pfactory = factory("acid", OPERATING_SYSTEM_NAME);
...
//      /// Create/Replace logger
      ...
      // auto estatus = ([a-z0-9_]+)_factory("acid", OPERATING_SYSTEM_NAME);
      ...
      //estatus = __construct(m_pacenode);
      ...
      //auto estatus = __raw_construct(m_pacedirectory);

      __raw_construct(m_pacedirectory);
      ...
      /*if (!estatus)
       ...
       //      m_pacedirectory = pacedirectory;

       //    m_pacedirectory->increment_reference_count();

       //estatus = __raw_construct(m_pacefile);

      __raw_construct(m_pacefile);
      ...
      //estatus = __raw_construct(m_pacepath);

      __raw_construct(m_pacepath);
      ...
      m_pacedirectory->m_pacefile = m_pacefile;
      m_pacedirectory->m_pacepath = m_pacepath;
      m_pacefile->m_pacedirectory = m_pacedirectory;
      m_pacefile->m_pacepath = m_pacepath;
      m_pacepath->m_pacedirectory = m_pacedirectory;
      m_pacepath->m_pacefile = m_pacefile;

      //estatus = m_pacefile->initialize(this);

      m_pacefile->initialize(this);
      ...
      //estatus = m_pacepath->initialize(this);

      m_pacepath->initialize(this);
      ...
      //estatus = m_pacedirectory->initialize(this);

      m_pacedirectory->initialize(this);
   ...
   void system::TermSystem()
   {
      ...
      //   ::acid::acid::g_pace->factory_close();
   ...
   //CLASS_DECL_ACID class system * system()
   //{
   //
   //   return ::g_psystem;
   //
   ...
   void system::init_system()
   {
      ...
      //::acid::application * pappStartup = ::acid::application::g_p;
      ...
      m_paceapplication->initialize(this);

      m_paceapplication->get_property_set().merge(get_property_set());
...
//
   ...
   //::pointer<::acid::library> system::library(const ::string& strComponent, const ::string& strImplementationParam)
   //{
   ...
   //::pointer<::acid::library>& system::library(const ::string & strComponent, const ::string & strImplementation)
   //{
   ...
   //::pointer<::acid::library>system::open_containerized_component_library(const ::string & pszComponent, const ::string & pszImplementation)
   //{
   ...
   //   ::pointer<::acid::library>plibrary = psystem->m_mapContainerizedLibrary[strComponent][strImplementation];
   ...
   //      plibrary = __allocate< ::acid::library >();
   ...
   ::acid::library * system::on_get_library(const ::string & pszLibrary)
   {
   ...
   void system::get_public_internet_domain_extension_list(string_array & stra)
   {
      ...
      information() << "acid::system::get_public_internet_domain_extension_list";
   ...
   ::pointer<::acid::session>system::on_create_session(::collection::index iEdge)
   {

      ::pointer<::acid::session>psession;
      ...
      psession->m_paceapplication = m_paceapplication;
      psession->m_pacesystem = this;
      psession->m_pacenode = m_pacenode;

      if (m_paceapplication)
      {

         m_paceapplication->m_pacesession = psession->m_pacesession;
   ...
   ::acid::session * system::session(::collection::index iEdge)
   {
   ...
   void system::add_session(::collection::index iEdge, ::acid::session * psession)
   {
   ...
   void system::on_add_session(::acid::session * pacesession)
   {

      if (pacesession->m_iEdge == 0)
      {
   ...
   void system::report_system_instance()
   {
      ...
      if (strAppId.has_char())
      {
         ...
         pathFolder = acedirectory()->roaming();
         ...
         acefile()->put_contents(path, platform()->m_strCommandLine);

         ::file::path pathExecutable = acefile()->module();

         string strAppTitle = node()->executable_title_from_appid(strAppId);

         path = pathFolder / (strModifier + "_executable.txt");

         acefile()->put_contents(path, pathExecutable);
   ...
   ::acid::application * system::get_main_app()
   {
   ...
   void system::system_construct(::acid::application * papplication)
   {

      m_paceapplication = papplication;
      ...
      if (m_paceapplication)
      {
         ...
         m_paceapplication->m_pacesystem = this;
   ...
   //   ace_main_data::system_construct(argc, argv, envp);
...
//   }
   ...
   void system::compress(const ::payload & payloadTarget, const ::payload & payloadSource, const ::scoped_string & scopedstrImplementation)
   {
      ...
      auto pfileTarget = acefile()->get_file(payloadTarget, ::file::e_open_write | ::file::e_open_defer_create_directory | ::file::e_open_binary);

      auto pfileSource = acefile()->get_file(payloadSource, ::file::e_open_read | ::file::e_open_binary);
   ...
   ::pointer<::acid::application>system::new_app(const ::scoped_string & scopedstrAppId)
   {
      ...
      if (!papp)
      {
         ...
         if (strAppId.is_empty() || application()->is_console())
         {
            ...
            papp = __create < ::acid::application >();
         ...
         else
         {
            ...
            if (pfactory)
            {
               ...
               papp = __create < ::acid::application >(pfactory);
               ...
               ::acid::get()->platform()->informationf("\n\n\n|(4)----");
               ::acid::get()->platform()->informationf("| app : " + strAppId + "(papp=0x" + ::hex::upper_case_from((uptr)papp.m_p) + ")\n");
               ::acid::get()->platform()->informationf("|\n");
               ::acid::get()->platform()->informationf("|\n");
               ::acid::get()->platform()->informationf("|----");
      ...
      //
      //      ::acid::get()->platform()->informationf("\n\n\n|(3)----");
      //      ::acid::get()->platform()->informationf("| app : " + strAppId + "\n");
      //      ::acid::get()->platform()->informationf("|\n");
      //      ::acid::get()->platform()->informationf("|\n");
      //      ::acid::get()->platform()->informationf("|----");
      //
   ...
   bool system::_handle_call(::payload & payload, const ::string & strObject, const ::string & strMember, ::property_set & propertyset)
   {
      ...
      try
      {
         ...
         if (m_paceapplication)
         {
            ...
            if (m_paceapplication->_handle_call(payload, strObject, strMember, propertyset))
            {
...
//   }
   ...
   ::file::path system::local_get_matter_path()
   {
      ...
      return acedirectory()->ca2roaming() / "appmatter";
   ...
   ::file::path system::local_get_matter_cache_path()
   {
      ...
      return acedirectory()->ca2roaming() / "cache/appmatter";
...
void system::set_dark_mode(bool bDark)
{
   ...
   if (m_bDarkMode)
   {
      ...
      ::acid::get()->platform()->informationf("background_color :: Dark\n");

   }
   else
   {

      ::acid::get()->platform()->informationf("background_color :: Lite\n");
...
//CLASS_DECL_ACID class ::plane_system * get_plane_system()
//{
//
//   return system();
//
...
//
//CLASS_DECL_ACID::acid::system * get_context_system()
//{
//
//   return g_psystem;
//
...
//
//CLASS_DECL_ACID::acid::system * ace_system_init()
//{
//
//   g_psystem = __new< ::acid::system >();
//
//   return g_psystem;
//
//}
...
//CLASS_DECL_ACID void ace_system_term()
//{
//
//   ::acid::del(g_psystem);
//
...
void system_id_update(void * pSystem, ::i64 iUpdate, ::i64 iParam)
{
   ...
   auto psystem = (::acid::system *)pSystem;
...
//::pointer<::acid::system>platform_create_system(const ::scoped_string & scopedstrAppId)
//{
//
...
//
//         //   ::acid::get()->platform()->informationf("result " + as_string(iDialogResult));
//
//         //}
//
//         //throw ::exception(error_failed, strMessage + "\n\nCould not open required library.");
//
//         ::acid::get()->platform()->informationf("The application library for appid \"" + strAppId + "\" wasn't loaded.");
//
//      }
//
//   }
...
//   }
//
//   ::pointer<::acid::system>psystem = pelement;
//
//   if (!psystem)
//   {
//
...
CLASS_DECL_ACID task_pointer fork(::particle * pparticle, const ::procedure & procedure)
{
