...
//   i32 iOldDataCount    = m_iDataCount;
   ...
   if(m_iDataCount > 0)
   {
      m_pdata              = __new_array< i32 >(m_iDataCount);
      if(m_pdata != nullptr)
      {
         if(::collection::index(pdataOld) == 1)
         {
            ::acid::bit::set(m_pdata, true, 0, m_iDataCount * 8 * sizeof(i32) - 1);
         }
         else if(pdataOld != nullptr)
         {
            ::acid::bit::int_aligned_copy(m_pdata, 0, pdataOld, 0, iOldBitCount - 1);
            delete [] pdataOld;
         }
         if(iOldBitCount < m_iBitCount)
         {
            ::acid::bit::set(m_pdata, bSet, iOldBitCount, m_iBitCount - 1);
         }
      }
   }
   else
