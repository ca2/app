...
#include "acid/primitive/string/base64.h"
...
block & block::from_base64(const ::scoped_string & scopedstr) const
{
   ...
   //system()->m_pbase64->decode(*this, psz, iSize);
...
//   m_iSize = (iSize + 1) * 3 / 4;
//
//   m_pdata = (::u8 *) ::acid::get()->m_pheapmanagement->memory(::heap::e_memory_main)->allocate(m_iSize);
//
//   ::base64 base64;
//
...
namespace hex
{
   ...
   CLASS_DECL_ACID string lower_case_from(const block & block)
   {
   ...
   CLASS_DECL_ACID string upper_case_from(const block & block)
   {
