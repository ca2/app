...
::u8 * memory::impl_alloc(memsize dwAllocation)
{
   ...
   if(m_bAligned)
   {
      ...
      return (::u8 *)::acid::get()->m_pheapmanagement->memory(::heap::e_memory_main)->aligned_allocate((size_t)dwAllocation);

   }
   else
   {

      return (::u8 *)::acid::get()->m_pheapmanagement->memory(::heap::e_memory_main)->allocate((size_t)dwAllocation);
...
::u8 * memory::impl_realloc(void * pdata, memsize dwAllocation)
{
   ...
   {
      return (::u8 *)::acid::get()->m_pheapmanagement->memory(::heap::e_memory_main)->reallocate(pdata, (size_t)dwAllocation);
   }
