...
#include "acid/filesystem/file/memory_file.h"
...
CLASS_DECL_ACID char* _crypto_base64_encode(const ::u8* data, int length)
{
   int c;
   const ::u8* q;
   char* p;
...
CLASS_DECL_ACID void _crypto_base64_decode(const char* enc_data, int length, ::u8** dec_data, int* res_length)
{
...
//   }
   ...
   string base64::decode(const ::scoped_string & scopedstrBase64)
   {

      memory storage;

      decode(storage, scopedstrBase64);
   ...
   bool base64::decode(memory_base & storageBinary, const ::scoped_string & scopedstrBase64)
   {
