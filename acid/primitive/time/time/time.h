...
#include "acid/primitive/mathematics/numeric.h"
#include "acid/primitive/time/_structures.h"
#include "posix_time.h"
#include "nanosecond.h"
#include "acid/platform/uint64_utilities.h"
...
//   constexpr bool operator !() const {return is_null();}
   ...
   //inline bool operator > (const class time & time) const { return !operator <=(time); }
   //inline bool operator >= (const class time & time) const { return !operator <=(time); }
   ::pair < ::collection::count, time > count_and_remainder(const class time & time) const
   {

      auto d = *this / time;

      auto count = (::collection::count)::floor(d);
...
//CLASS_DECL_ACID class ::time random(const class ::time & d1, const class ::time & d2);
