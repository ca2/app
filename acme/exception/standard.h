...
#define DECLARE_SE_EXCEPTION_CLASS(name) class CLASS_DECL_ACME name : public standard_exception \
   { \
   \
...
/*class standard_sigsegv : public standard_exception
{
   friend class translator;
protected:
   standard_sigsegv (siginfo_t * psiginfo, void * pc) : ::elemental(pobject), standard_exception(psiginfo, pc) {}
public:
   //bool is_read_op() const { return !info()->ExceptionRecord->ExceptionInformation [0]; }
   //uptr inaccessible_address() const { return info()->ExceptionRecord->ExceptionInformation [1]; }
};*/
...
/*class standard_sigfpe : public standard_exception
{
   friend class translator;
protected:
   standard_sigfpe (siginfo_t * psiginfo, void * pc) : ::elemental(pobject), standard_exception(psiginfo, pc) {}
public:
//   bool is_read_op() const { return !info()->ExceptionRecord->ExceptionInformation [0]; }
  // uptr inaccessible_address() const { return info()->ExceptionRecord->ExceptionInformation [1]; }
};
