...
::string_array acme_file::column_cells(const ::file::path& path, ::index iColumn)
{

   ::string_array stra;

   for_each_column_cell(path, iColumn, [&stra](auto & cell) {stra.add(cell);});
...
::string_array acme_file::case_insensitive_unique_column_cells(const ::file::path& path, ::index iColumn)
{

   ::string_array stra;

   for_each_column_cell(path, iColumn, [&stra](auto &str) { stra.case_insensitive_add_unique(str); });
...
void acme_file::for_each_column_cell(const ::file::path& path, ::index iColumn, const ::function < void(const ::scoped_string & scopedstrCell) > & function)
{

   auto pfile = get_file(path, ::file::e_open_read);

   ::string str;

   while(pfile->read_string(str))
   {
      ...
      for(::index i = 0; i < iColumn; i++)
      {
         ...
         next.skip_to_first_character_in(" \t");

         next.skip_any_character_in(" \t");

         if(next.begin() == range.begin())
         {
      ...
      ::string strCell = str.consume_word(" \t");
