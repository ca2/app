...
public:
   ...
   virtual void indexed_set_item_at(::index iIndex, ::item * pitem);

   virtual void indexed_add_item(::item * pitem);

   virtual ::index add_item(::item * pitem);

   virtual ::item_pointer add_item(enum_element eelement, ::index iItem = -1);
   ...
   inline ::item * item_at(::index iIndex)
   {
   ...
   inline const ::item * item_at(::index iIndex) const
   {
      if (!m_pitema) return nullptr;
      return m_pitema->element_at(iIndex);
   }
   ...
   virtual ::item_pointer find_item(enum_element eelement, ::index iItem = -1);

   virtual ::item_pointer defer_item(enum_element eelement, ::index iItem = -1);
