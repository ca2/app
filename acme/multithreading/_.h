...
CLASS_DECL_ACME bool task_sleep(millis tick = U32_INFINITE_TIMEOUT, ::sync * psync = nullptr);
...
CLASS_DECL_ACME bool __task_sleep(task* ptask, millis tick);

CLASS_DECL_ACME bool __task_sleep(::task* ptask, sync* psync);

CLASS_DECL_ACME bool __task_sleep(task* ptask, millis tick, sync* psync);
