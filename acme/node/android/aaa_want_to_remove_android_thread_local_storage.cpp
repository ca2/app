...
/*void CLASS_DECL_ACME ::ca2::InitLocalData(HINSTANCE hInst)
{
   if (__thread_data != nullptr)
      __thread_data->AssignInstance(hInst);
}

void CLASS_DECL_ACME __term_local_data(HINSTANCE hInst, bool bAll)
{
   if (__thread_data != nullptr)
      __thread_data->DeleteValues(hInst, bAll);
}*/

// This elemental count is needed to support Win32s, such that the
// thread-local and process-local data is not destroyed prematurely.
// It is basically a elemental count of the number of processes that
// have attached to the ca2 API DLL.

long gen_TlsRef = 0;

void CLASS_DECL_ACME __tls_add_ref()
{
   ++gen_TlsRef;
}

void CLASS_DECL_ACME __tls_release()
{
