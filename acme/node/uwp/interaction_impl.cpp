...
//      IGUI_MSG_LINK(ca2m_PRODEVIAN_SYNCH, pchannel, this,&interaction_impl::_001OnProdevianSynch);
   ...
   void interaction_impl::assert_valid() const
   {
...
#endif
      {
         ...
         // you have passed a C++ generic from one thread to another
         // and have used that generic in a way that was not intended.
         // (only simple inline wrapper functions should be used)
         //
         // In general, interaction_impl objects should be passed by oswindow from
         // one thread to another.  The receiving thread can wrap
         // the oswindow with a interaction_impl generic by using ::uwp::interaction_impl::from_handle.
         //
...
//         m_puserinteraction->get_context_application()->step_timer();
      ...
      {
      ::acme::application ** ppapp= (::acme::application **) pbase->m_lparam;
...
*/
  ...
  /*   void interaction_impl::_001OnDeferPaintLayeredWindowBackground(::image * pimage)
   ...
   bool interaction_impl::GrayCtlColor(HDC hDC,oswindow hWnd,UINT nCtlColor,HBRUSH hbrGray,COLORREF clrText)
   {
      ...
      //   //         TRACE(::acme::trace::category_AppMsg, 0, "Warning: hDC is nullptr in interaction_impl::GrayCtlColor; WM_CTLCOLOR not processed.\n");
      //   return FALSE;
      //}
...
//      m_puserinteraction->m_iModal = m_puserinteraction->m_iModalCount;
      ...
      ::acme::application * pappThis1 = dynamic_cast <::acme::application *> (get_context_application());
      ::acme::application * pappThis2 = dynamic_cast <::acme::application *> (get_context_application());
   ...
   */
   ...
   CLASS_DECL_BASE LRESULT __call_window_procedure(::user::interaction * pinteraction, oswindow hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
   {
      ...
      catch(::exception::acme * pe)
      {
         ...
         //         TRACE(::acme::trace::category_AppMsg, 0, "Warning: Uncaught exception in message_handler (returning %ld).\n", spbase->get_lresult());
...
__activation_window_procedure(oswindow hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   catch(::exception::acme * pe)
   {
      ...
      //      TRACE(::acme::trace::category_AppMsg, 0, "Warning: Uncaught exception in __activation_window_procedure (returning %ld).\n",
      //       lResult);
      ::exception_pointer esp(pe);
   }
...
bool CLASS_DECL_BASE __register_class(WNDCLASS* lpWndClass)
{
   ...
   {
      //      TRACE(::acme::trace::category_AppMsg, 0, "Can't register interaction_impl class named %s\n",
