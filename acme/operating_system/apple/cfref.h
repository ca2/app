...
public:
   ...
   virtual ref * clone() const { return new ref(*this); }
...
public:
   ...
   void * data() override { return (void *) m_cfref; }

   ref * clone() const override { return new cfref(*this); }
   
   
   cfref & operator = (CFREF r)
   {
   ...
   cfref & operator = (const cfref & r)
   {
      
      if(r.m_cfref != m_cfref)
      {
...
public:
   ...
   template < typename REF >
   requires(::std::is_base_of_v<ref, REF>)
   ref_holder(const REF & r)
   {
   ...
   ref_holder(const ref_holder & holder)
      : m_pref(holder.m_pref ? holder.m_pref->clone() : nullptr)
   {
   }

   ref_holder(ref_holder && holder) noexcept
      : m_pref(holder.m_pref)
   {
      holder.m_pref = nullptr;
   }

   ref_holder & operator=(const ref_holder & holder)
   {
      if(this != &holder)
      {
         auto pref = holder.m_pref ? holder.m_pref->clone() : nullptr;
         delete m_pref;
         m_pref = pref;
      }
      return *this;
   ...
   ref_holder & operator=(ref_holder && holder) noexcept
   {
      if(this != &holder)
      {
         delete m_pref;
...
using ref_array = ::array < ref_holder >;
...
public:
   ...
   using ref_holder::ref_holder;
   
   template < typename CFREF >
   requires(!::std::is_base_of_v<ref, CFREF> && !::std::is_base_of_v<ref_holder, CFREF>)
   cfref_holder(const CFREF & cfref)
   {
      
      m_pref = new ::cfref<CFREF>(cfref);
...
class cf_array :
virtual public ::array < cfref_holder >
{
public:
  
   ::array<void * > m_voidptra;
   
   void add(const cfref_holder & holder)
   {
      
      ::array<cfref_holder>::add(holder);
      
      m_voidptra.add(this->last().m_pref->data());
      
   }

   const void ** data() {return (const void **) m_voidptra.data();}
