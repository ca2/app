#pragma once


//#include "macos_ns_exception.h"

#include "parallelization.h"



#include "main.h"


bool ns_open_file(const ::scoped_string & scopedstr);
void ns_set_this_default_browser();

::i32 raw_main_command_line(const ::scoped_string & scopedstrCommandLine, ::i32 argc, char_pointer argv[]);

::i32 aura_main_command_line(const ::scoped_string & scopedstrParams, ::i32 argc, char_pointer argv[]);

#ifdef cplusplus
string macos_error_string(OSStatus status);
string macos_error_description(OSStatus status);
#endif


struct macos_window_t
{
   CGWindowID m_cgwindowid;
   
};
