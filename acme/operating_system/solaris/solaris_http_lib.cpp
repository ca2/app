 ...
 */
::i32 tiny_http::t_read_line (::i32 fd, char_pointer buffer, ::i32 maximum)
{
   /* not efficient on long lines (multiple unbuffered 1 ::i8 reads) */
   ::i32 n=0;
   while (n<maximum)
   {
 ...
 */
::i32 tiny_http::t_read_buffer (::i32 fd, char_pointer buffer, ::i32 length, void (*callback)(void *, ::i32, dword_ptr), void * callback_param)
{
   ::i32 n,r;
 ...
 */
//      char_pointer command;	/* command to send  */
//   char_pointer url;		/* url / filename queried  */
//char_pointer additional_header;	/* additional header */
//querymode mode; 		/* type of query */
//char_pointer data;  /* Data to send after header. If nullptr, not data is sent */
//::i32 length;  /* i32_size of data */
//::i32 *pfd;    /* pointer to variable where to set file descriptor value */

tiny_http::http_retcode tiny_http::t_query(const_char_pointer command,  const_char_pointer url,  const_char_pointer additional_header, querymode mode, char_pointer data, ::i32 length, ::i32 * pfd)
{
   ::i32                  s;
   struct hostent *     hp;
   ...
   {
      memory_set((char_pointer ) &server,0, sizeof(server));
      memory_transfer((char_pointer ) &server.sin_addr, hp->h_addr, hp->h_length);
      server.sin_family = hp->h_addrtype;
      server.sin_port = (::u16) htons( port );
   }
   else
      return ERRHOST;
 ...
 *              and type to 64.
 */
//     char_pointer filename;  /* name of the ressource to create */
//   char_pointer data;      /* pointer to the data to send   */
//::i32 length;      /* length of the data to send  */
// ::i32 overwrite;   /* flag to request to overwrite the ressource if it
// was already existing */
//char_pointer type;      /* type of the data, if nullptr default type is used */
tiny_http::http_retcode tiny_http::t_put(const_char_pointer data, ::i32 length, ::i32 overwrite, void (*callback)(void *, ::i32, dword_ptr), void * callback_param)
{
   ::i8 header[MAXBUF];
   ...
   return t_query("PUT",m_strFileName,header,CLOSE, (char_pointer ) data, length, nullptr);
}
 ...
 * limitations: filename is truncated to first 256 characters
 */
//     char_pointer filename; /* name of the ressource to read */
//   char_pointer *pdata; /* address of a pointer variable which will be set
//	      to i32_point toward allocated memory containing read data.*/
//::i32  *plength;/* address of integer variable which will be set to
//		      length of the read data */
//   char_pointer typebuf; /* allocated buffer where the read data type is returned.
//	    If nullptr, the type is not returned */
tiny_http::http_retcode tiny_http::t_get(char_pointer * pdata, ::i32 * plength, void (*callback)(void *, ::i32, dword_ptr), void * callback_param)
{
   http_retcode ret;
   ...
   if (ret==200)
   {
      while (1)
      {
         ...
         sscanf(header,"content-type: %s", (char_pointer ) m_strContentType);
      }
      if (length<=0)
      {
         close(fd);
      ...
      if (!(*pdata=(char_pointer ) ca2_alloc(length)))
      {
         close(fd);
         return ERRMEM;
      }
...
//   ::i32  *plength;/* address of integer variable which will be set to
//	      length of the data */
//char_pointer typebuf; /* allocated buffer where the data type is returned.
//   If nullptr, the type is not returned */
