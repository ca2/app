...
#define LDR_DLL_NOTIFICATION_REASON_UNLOADED     2
   ...
   bool engine::init()
   {
      ...
      //if(!::file_exists("C:\\acme\\exception_engine.txt"))
      //{
      //   return false;
      //}
...
// prints a current thread's stack
...

   {
      ...
      __try
      {
         ...
         while(pcontext->m_pcontext->signal && iPatience > 0)
         {
            if(!SwitchToThread())
         ...
         if (-1 == SuspendThread(pcontext->m_pcontext->thread))
         {
            pcontext->m_pcontext->signal  = -1;
            __leave;
         }
         ...
         __finally
         {
            VERIFY(-1 != ResumeThread(pcontext->m_pcontext->thread));
         }
      }
