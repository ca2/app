...
struct WAIT
{
   ...
   ::i64 m_nanosecond.m_iNanosecond;
...
public:
   ...
   constexpr wait() {}
   constexpr wait(::i64 iNanosecond) { m_nanosecond.m_iNanosecond = iNanosecond; }
   ...
   wait(const struct TIME& time) : wait(time.m_posixtime.m_iSecond, time.m_nanosecond.m_iNanosecond) {}
   wait(const class wait& wait) : wait(wait.m_nanosecond.m_iNanosecond) {}
   ...
   constexpr bool is_infinite() { return m_nanosecond.m_iNanosecond >= infinite_nanoseconds().m_nanosecond.m_iNanosecond; }

   constexpr bool is_null() const { return m_nanosecond.m_iNanosecond <= 0; }
