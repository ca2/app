...
task_group::task_group(::matter * pmatter, ::enum_priority epriority)
{
   ...
   ::i32 cOrder = node()->get_current_process_affinity_order();
   ...
   for(::collection::index iThread = iPreviousSize; iThread < cOrder ; iThread++)
   {
      ...
      ptooltask->m_uThreadAffinityMask = node()->translate_processor_affinity((::i32) (ptooltask->m_iThread));
...
::e_status task_group::wait(const ::function < bool() > & functionShouldContinue)
{
...
void task_group::select_tool(task_tool* ptool)
{
   ...
   for (::collection::index i = 0; i < task_count(); i++)
   {
      ...
      pitem = system()->øcreate_by_type(ptool->id());
