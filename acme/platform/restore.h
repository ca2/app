...
#define ___scoped_restore(payload) ::restore < decltype(payload) > TOKEN_CONCATENATE(restore, __COUNTER__) (&payload)
#define __set_restore(payload, set) ::restore < decltype(payload) > TOKEN_CONCATENATE(restore, __COUNTER__) (&payload); ::payload = set
