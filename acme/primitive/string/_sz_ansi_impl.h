...
//inline bool isalpha(ansichar ch) noexcept { return ansi_char_isalpha(ch); }
//inline bool isdigit(ansichar ch) noexcept { return ansi_char_isdigit(ch); }
//inline bool character_isspace(ansichar ch) noexcept { return ansi_char_isspace(ch); }

//inline ansichar tolower(ansichar ch) noexcept { return ansi_char_tolower(ch); }
//inline ansichar  toupper(ansichar ch) noexcept { return ansi_char_toupper(ch); }
...
//      (ansichar *) unicode_increment(pszPos);
//   }
...
//   //if(nDestLength >= 0)
//   //{
//   //   throw ::exception(::exception("I am wasting this branching (if(nDestLength >= 0) to tell you that nDestLength should be negative so the buffer is already correct size... or you like incorrect size_i32? Go to Facebook and click in Like for Community \"I Like incorrect size_i32!!\", there should exist such community... there are so many things in the multi bramas... The hardware will check again if you didn' dirtied any other process... (only another process, though)... and you're probably be fired or even not be hired if incorrect size_i32"));
//   //}
//
...
//{
//   // returns ansichar length
//   return  get_utf8_char(pch).get_length();
//}
//
//
