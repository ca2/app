...
//   ::i32 iOldDataCount    = m_iDataCount;
   ...
   if(m_iDataCount > 0)
   {
      m_pdata              = øraw_new ::i32[m_iDataCount];
      if(m_pdata != nullptr)
      {
...
void bit_array::add_bit_array(char_pointer bits, ::i32 count)
{
   if (::collection::index(m_pdata) == 1) return;
   ensure_size(count * 8);
   for(::i32 i = 0; i < count && i < m_iDataCount*4;i++)
      ((char_pointer )m_pdata)[i] |= bits[i];
}

void bit_array::clear_bit_array(char_pointer bits, ::i32 count)
{
   if(m_pdata == nullptr)
      return;
