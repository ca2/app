#pragma once

// prototype array is a special array and should be used with care
// it uses operations like memory_transfer and memcopy to transfer objects and does not
// call constructors and destructors for the elements
// it is faster than the more matter arrays, can be faster than raw array
// for many "prototype" types because the arguments are passed as the own
// "prototype" type data and it is proper for use with prototype data types.

template < typename TYPE >
using comparable_primitive_array = comparable_raw_array < TYPE, const TYPE & >;


