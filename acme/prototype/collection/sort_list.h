...
namespace sort
{
   ...
   void QuickSortAsc(comparable_list < TYPE, ARG_TYPE, ARRAY_TYPE > & list_base)
   {
   ...
   template <class TYPE, class ARG_TYPE, class ARRAY_TYPE>
   void QuickSortDesc(comparable_list < TYPE, ARG_TYPE, ARRAY_TYPE > & list_base)
   {
      ::raw_array < POSITION > stackLowerBound;
      ::raw_array < POSITION > stackUpperBound;
      POSITION iLowerBound;
   ...
   inline bool sort_find(ARRAY & a, typename ARRAY::BASE_ARG_TYPE arg,::collection::index & iIndex, ::collection::index iStart, ::collection::index iEnd)
   {
      if(a.get_size() == 0)
      {
         return false;
      }
      ::collection::index iLowerBound = iStart;
      ::collection::index iMaxBound = iEnd;
