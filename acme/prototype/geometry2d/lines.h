...
public:
   ...
   //using POLYGON_BASE_TYPE = point_base_array < POINT_TYPE >;
   ...
   bool contains(const ::point_type <NUMBER > & point, float epsilon = 0.001f)
   {
      ...
      for (::collection::index i = 0; i < upper_bound; i++)
      {
   ...
   lines_base & operator = (const lines_base & lines)
   {
      ...
      point_base_array < NUMBER >::operator =(lines);
...
inline lines_base < NUMBER >::lines_base(lines_base&& lines) :
   point_base_array < NUMBER >(::transfer(lines))
{
