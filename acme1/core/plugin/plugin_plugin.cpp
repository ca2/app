...
void CLASS_DECL_ACME __cdecl _ca2_purecall_();
...
//      m_pbReady               = nullptr;
   ...
   i32 plugin::start_ca2_system()
   {
      ...
      ::str::begins_eat_ci(strMutex, "\\acme\\");
...
//         i32 nReturnCode = 0;
         ...
         psystem->m_hinstance = ::GetModuleHandleW(L"acme.dll");
...
//         simple_http_status * ps = (simple_http_status *) dw;
   ...
   void plugin::ready_on_main_thread()
   {
      ...
      xxdebug_box("acme plugin plugin", "ready_on_main_thread", 0);
      ...
      try
      {
         ...
         else
         {
            ...
            if(str1 == "ca2login")
            {
               ...
               /*if(!System.install().is(nullptr, strBuild, "application", "app/acme/account", strLocale, strSchema))
               {
               /*                  System.install().start(": app=session session_start=app/acme/account app_type=application install");
               #ifdef WINDOWS
               ...
               /*                  string strCommandLine(": app=session session_start=app/acme/account app_type=application install ruri=\"" + strRuri + "\" locale=" + strLocale + " schema=" + strSchema);


               #ifdef WINDOWS_DESKTOP
                                 PostMessage(m_phost->::acme::ipc::tx::m_oswindow, WM_USER + 100, 1, 1);
               #else
...
//                        System.install().start(strCommandLine);
   ...
   void plugin::message_handler(::message::acme * pmessage)
   {
