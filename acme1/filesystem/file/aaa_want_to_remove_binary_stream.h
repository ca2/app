...
public:
   ...
   binary_stream(const binary_stream & acme) :
      m_p(acme.m_p)
   {

   }
   binary_stream(binary_stream & acme) :
      m_p(::move(acme.m_p))
   {
...
//   string fileName() { return GetFilePath(); }
   ...
   stream & operator = (const binary_stream & acme) { m_p = acme.m_p; return *this; }
   stream & operator = (binary_stream & acme) { m_p = ::move(acme.m_p); return *this; }
