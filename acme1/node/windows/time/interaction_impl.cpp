...
#include "acme/user/interaction_thread.h"
#include "acme/os/windows/windowing.h"
#include "acme/os/windows/top_level_enum.h"
#include "acme/os/windows/imm_context.h"
...
CLASS_DECL_ACME int g_iMouseDown = 0;

//CLASS_DECL_ACME thread_iptr < DWORD_PTR > t_time1;
//CLASS_DECL_ACME thread_iptr < DWORD_PTR > t_time2;


//extern CLASS_DECL_ACME thread_int_ptr < DWORD_PTR > t_time1;
...
const char * gen_OldWndProc = "::acme::OldWndProc423";
...
namespace windows
{
   ...
   interaction_impl::interaction_impl(::generic * pobject) :
      ::generic(pobject)
   {
   ...
   __STATIC bool CLASS_DECL_ACME __modify_style(oswindow oswindow, i32 nStyleOffset,
   ...
   bool interaction_impl::_native_create_window_ex(::user::create_struct & cs)
   {
      ...
      // if window is not created, it may destroy this generic, so keep the app as local var
      ...
      //oswindow oswindow = CreateWindowExW(cs.dwExStyle, szWindowClass, wstrWindowName, cs.style,
      ...
      oswindow oswindow = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPED,
         CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, nullptr, nullptr, cs.hInstance, nullptr);
      //if (!oswindow)
      //{
         //return false;
      //}
      ...
      /// this Windows native window "holds" generic to the
      /// wrapping generic.
   ...
   void interaction_impl::pre_translate_message(::message::message * pmessage)
   {
      ...
      SCAST_PTR(::message::acme, pbase, pmessage);
   ...
   ///* trans   if (!::acme::HtmlHelp((pwindow)->get_handle(), pApp->m_pszHelpFilePath, nCmd, dwData))
   //{
   //// linux System.message_box(__IDP_FAILED_TO_LAUNCH_HELP);
   //System.message_box("Failed to launch help");
   //}*/
   ...
   bool interaction_impl::OnCommand(::message::acme * pbase)
   {
   ...
   bool interaction_impl::OnNotify(::message::acme * pbase)
   {
...
//   for control scrollbar - enable/disable them
   ...
   bool interaction_impl::ReflectMessage(oswindow oswindow_Child, ::message::acme * pbase)
   {
   ...
   bool interaction_impl::OnChildNotify(::message::acme * pbase)
   {

      return ReflectChildNotify(pbase);
   }

   bool interaction_impl::ReflectChildNotify(::message::acme * pbase)
   {
...
//
   ...
   public:
      ...
      print_window(::generic * pobject, oswindow oswindow, HDC hdc, tick tickTimeout) :
         ::generic(pobject)
      {
   ...
   oswindow interaction_impl::unsubclass_window()
   {
      ...
      *plpfn = nullptr;
      ...
      // and detach the oswindow from the interaction_impl generic
      return detach();
   }
...
//      Default();
//   }
//   i32 interaction_impl::OnMouseActivate(::user::interaction_impl *, UINT, UINT)
//   {
//      return (i32)Default();
//   }
...
//      Default();
//   }
//   void interaction_impl::OnAskCbFormatName(__in UINT nMaxCount, __out_ecount_z(nMaxCount) LPTSTR lpszName)
//   {
//      (nMaxCount);
//      if (nMaxCount > 0)
//      {
...
*/
   ...
   void interaction_impl::_001OnActivate(::message::message* pmessage)
   {
      ...
      if (enabled)
      {
         ...
         DwmSetWindowAttribute(get_safe_handle(), DWMWA_NCRENDERING_POLICY, &dw, sizeof(DWORD));
...
bool is_registered_windows_message(UINT message)
{
...
LRESULT CALLBACK __window_procedure(HWND oswindow, UINT message, WPARAM wparam, LPARAM lparam)
{
   ...
   else if (message == WM_TIMER)
   {
      ...
      if (wparam == timer_transparent_mouse_event)
      {
...
void CLASS_DECL_ACME _handle_activate(::user::interaction_impl * pwindow, WPARAM nState, ::user::interaction_impl * pWndOther)
{
...
namespace windows
{
   ...
   void interaction_impl::default_message_handler(::message::message * pmessage)
   {
      ...
      if (pfnWndProc == nullptr)
      {
         ...
         lresult = ::DefWindowProcW(m_oswindow, (UINT) pmessage->m_id.i64(), pmessage->m_wparam, pmessage->m_lparam);

      }
      else
      {

         lresult = ::CallWindowProc(pfnWndProc, m_oswindow, (UINT) pmessage->m_id.i64(), pmessage->m_wparam, pmessage->m_lparam);
   ...
   void interaction_impl::message_handler(::message::acme * pbase)
   {
      ...
      {
      __pointer(::acme::application)* ppapp= (__pointer(::acme::application)*) pbase->m_lparam;
...
wstring CLASS_DECL_ACME windows_get_user_interaction_window_class(::user::interaction * pinteraction)
{
   ...
   wndcls.hInstance = GetModuleHandleW(L"acme.dll");
...
bool CLASS_DECL_ACME windows_register_with_icon(WNDCLASSEXW * pwndclass, const unichar * pszClassName, UINT nIDIcon)
...
CLASS_DECL_ACME wstring windows_register_window_class(::generic * pobject, UINT nClassStyle, HCURSOR hCursor, HBRUSH hbrBackground, HICON hIcon)
{
