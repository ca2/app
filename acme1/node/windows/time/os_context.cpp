...
namespace windows
{
   ...
   bool os_context::defer_register_ca2_plugin_for_mozilla()
   {
      ...
      try
      {
         ...
         key.open(key, "application/acme", true);

         key.set("Description", "acme Document");
...
repeat:
   ...
   bool os_context::browse_file_save(::user::interaction* puiOwner, property_set & set)
   {
      ...
      try
      {
         ...
         // Create the FileSaveDialog generic.
   ...
   bool os_context::browse_folder(::user::interaction* puiOwner, property_set & set)
   {
      ...
      try
      {
         ...
         // Create the FileOpenDialog generic.
         HRESULT hr = pfileopen.CoCreateInstance(CLSID_FileOpenDialog, nullptr, CLSCTX_ALL);
...
//              to create and store a shortcut to the specified generic.
//
// Returns the result of calling the member functions of the interfaces.
//
// Parameters:
// pszPathObj  - Address of a buffer that contains the path of the generic,
...
CLASS_DECL_ACME HRESULT os_create_link(::file::path pathObj, ::file::path pathLnk, string strDesc, ::file::path pathIco, int iIcon)
{
