#include "framework.h"
#include "acme/const/procedure.h"

CLASS_DECL_ACME ::u32 os_get_pid();
...
namespace acme
{
   ...
   void ipi::task::do_task(const string & strObject, const string & strMember, const var_array & vara)
   {
      ...
      ::acme::ipc::tx & txc = m_pcall->m_pipi->tx(m_pcall->m_strApp, m_idPid);
   ...
   void ipi::call::exclude_this_app()
   {
      ...
      m_iaExclude.add((::i32)os_get_pid());
   ...
   ::estatus ipi::initialize(::generic * pobjectContext)
   {
...
//      defer_add_module(path, iPid);
   ...
   bool ipi::start(const string & strApp)
   {
      ...
      ::acme::app_launcher launcher(process_platform_dir_name2(), strApp);
...
started:
      ...
      if(m_txmap[strKey].is_null())
      {
         ...
         m_txmap[strKey] = __new(::acme::ipc::tx);
   ...
   ::acme::ipc::tx & ipi::tx(const string & strApp, const ::id & iPid)
   {
   ...
   void ipi::on_ipc_receive(::acme::ipc::rx * prx, const char * pszMessage)
   {

      string str(pszMessage);

      INFO("acme::ipi::on_receive %s", pszMessage);
