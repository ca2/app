#include "framework.h"
//#include "acme/net/sockets/_sockets.h"
...
namespace acme
{
   ...
   ::estatus log::initialize_ace_log(e_trace_level etracelevelMin, const ::id & id)
   {
...
//   extern __pointer(::acme::application) g_papp;

   CLASS_DECL_ACME i32 SimpleDebugReport(i32 iReportType, const char * pszFileName,i32 iLine,const char *,const char * pszFormat, va_list list)
   {
#ifdef WIN32
   ...
   void log::__tracea(::generic * pobjectContext, e_trace_level elevel, const char * pszFunction, const char * pszFile, i32 iLine, const char * psz)
   {
      ...
      ::acme::trace::category * pcategory = nullptr;
      ...
      retry:
         ...
         if (is_debugger_attached() && !::get_context_system()->has_ace_application_factory())
         {
            ...
            fork([this]()
               {
                  ...
                  if (!::file::app_module().contains_ci("logviewer") && file_exists(::dir::system() / "logviewer.txt"))
                  {
                     ...
                     call_async("C:\\acme\\time\\x64\\basis\\app_core_logviewer.exe", "\"" + m_strLogPath + "\"", "C:\\acme\\time\\x64\\basis", display_normal, false);
...
skip_further_possible_recursive_impossible_logging_in_file:
   ...
   //::estatus log::initialize_ace_log(e_trace_level etracelevelMin, const char * pszId)
   //{
