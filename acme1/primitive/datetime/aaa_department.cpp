...
#include "framework.h" // from "acme/acme/.h"
#include <time.h>
//#ifdef ANDROID
//#include <sys/time.h>
//#endif
...
namespace datetime
{
   ...
   i64 department::strtotime(const ::acme::str_context * pcontext, const char * psz, i32 iPath, i32 & iPathCount)
   {
   ...
   i64 department::strtotime(const ::acme::str_context * pcontext, const char * psz, time_t timeParam, i32 iPath, i32 & iPathCount)
   {
   ...
   i64 department::gmt_strtotime(const ::acme::str_context * pcontext, const char * psz, i32 iPath, i32 & iPathCount)
   {
   ...
   department::international::international(::generic * pobject) :
      ::generic(pobject)
   {
   }
   ...
   department::str::str(::generic * pobject)
   {
   ...
   string department::get_week_day_str(const acme::str_context * pcontext, i32 iWeekDay) // 1 - domingo
   {
      return System.str().get(pcontext, "datetimestr_weekday_long[" + __str(iWeekDay - 1) + "]");
   }

   string department::get_tiny_week_day_str(const acme::str_context * pcontext, i32 iWeekDay) // 1 - domingo
   {
      return System.str().get(pcontext, "datetimestr_weekday_tiny[" + __str(iWeekDay - 1) + "]");
   }

   string department::get_month_str(const acme::str_context * pcontext, i32 iMonth)
   {
      return System.str().get(pcontext, "datetimestr_month[" + __str(iMonth - 1) + "]");
   }

   string department::get_short_month_str(const acme::str_context * pcontext, i32 iMonth)
   {
   ...
   */
   ...
   string department::friend_time(const acme::str_context * pcontext, ::datetime::time timeNow, ::datetime::time time)
   {
      bool bDiff = false;
      bool bSolved = false;
      string strTime;
   ...
   string department::_001FriendTime(const ::acme::str_context* pcontext, const ::datetime::time& timeNow, const ::datetime::time& time)
   {
