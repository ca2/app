...
namespace datetime
{
   ...
   public:
      ...
      i64 strtotime(const ::acme::str_context * pcontext,const char * psz,time_t time,i32 iPath,i32 & iPathCount);
      i64 strtotime(const ::acme::str_context * pcontext,const char * psz,i32 iPath,i32 & iPathCount);
      i64 gmt_strtotime(const ::acme::str_context * pcontext,const char * psz,i32 iPath,i32 & iPathCount);

      ::datetime::time from_local(const string & str);
      ::datetime::time from_gmt(const string& str);

      // 1 - domingo
      string get_week_day_str(const acme::str_context * pcontext,i32 iWeekDay);
      // 1 - domingo
      string get_tiny_week_day_str(const acme::str_context * pcontext,i32 iWeekDay);
      string get_month_str(const acme::str_context * pcontext,i32 iMonth);
      string get_short_month_str(const acme::str_context * pcontext,i32 iMonth);
      ...
      string friend_time(const ::acme::str_context * pcontext,::datetime::time timeNow,::datetime::time time);



      virtual string _001FriendTime(const ::acme::str_context* pcontext, const ::datetime::time& timeNow, const ::datetime::time& time);
