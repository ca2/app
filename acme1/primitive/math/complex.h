...
template < typename T > complex < T > pow(const complex < T > & acme,const complex < T > & exp) // complex number exponentiation
{
   T modsqr = acme.m_x * acme.m_x + acme.m_y + acme.m_y;
   T arg = arg(acme);
...
template < typename T > complex < T > pow(const complex < T > & acme, T exp) // complex number exponentiation to real exponent
{
   T modsqr = acme.m_x * acme.m_x + acme.m_y + acme.m_y;
   T arg = arg(acme);
...
namespace std
{
   ...
   template < typename T > complex < T > pow(const complex < T > & acme, const complex < T > & exp) { return ::pow(acme,exp); }
