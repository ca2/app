...
protected:
   ...
   __pointer(::acme::application)                     m_pappContext;
   __pointer(::acme::session)                         m_psessionContext;
   __pointer(::acme::system)                          m_psystemContext;
   //__pointer(::generic)                                m_puserContext;
...
public:
   ...
   generic() : m_pmeta(nullptr) { }
   generic(::generic * pobjectContext);
   generic(e_default_init);
   virtual ~generic();
   ...
   inline ::acme::application * get_context_application() { return m_pappContext.m_p; }
   ...
   virtual void add_procedures_from(const ::id & id, ::generic * pobjectSource);
   virtual void add_callbacks_from(const ::id & id, ::generic* pobjectSource);
   ...
   inline generic & operator +=(const ::procedure & procedure)
   {
   ...
   template < typename PRED >
   inline generic& operator +=(const ::callback & callback)
   {
   ...
   template < typename PRED >
   inline void add(enum_procedure eprocedure, PRED pred, ::generic* pobjectHold = nullptr)
   {
   ...
   template < typename PRED >
   inline void add(enum_callback ecallback, PRED pred, ::generic* pobjectHold = nullptr)
   {
   ...
   ::acme::application * get_context_application() const { return m_pappContext; }

   ::acme::session * get_context_session() const { return m_psessionContext; }

   ::acme::system * get_context_system() const { return m_psystemContext; }

   //::generic * get_context_user() const { return m_puserContext; }

   ::acme::application * get_app() const { return get_context_application(); }
   ...
   virtual void set_context_app(::acme::application* pappContext);
   virtual void set_context_session(::acme::session* psessionContext);
   virtual void set_context_system(::acme::system* psystemContext);
   //virtual void set_context_user(::generic * puserContext);
#else
   inline void set_context(::context* pcontext);
   inline void set_context_thread(::thread* pthread);
   inline void set_context_app(::acme::application* pappContext);
   inline void set_context_session(::acme::session* psessionContext);
   inline void set_context_system(::acme::system* psystemContext);
   //inline void set_context_user(::generic * puserContext);
#endif


   virtual void set_context_object(::generic * pobjectContext) override;
   ...
   ::generic * parent_property_set_holder() const override;

   void copy_from(const generic & o);
   ...
   //// former user::server
   //virtual ::user::document* open_new_document(::acme::application* pappOnBehalfOf);
   virtual void on_request(::create* pcreate);
   //virtual ::user::document* open_document_file(::acme::application* pappOnBehalfOf);
   //virtual ::user::document* open_document_file(::acme::application* pappOnBehalfOf, const var& varFile, const var & varOptions, ::user::interaction* puiParent = nullptr, ewindowflag eflag = window_flag_none, ::id id = ::id());
   //virtual ::user::document* open_document_file(::acme::application* pappOnBehalfOf, const var& varFile);
...
//#endif
   ...
   ::estatus bind_update(const ::id & id, ::generic * pobject);
...
CLASS_DECL_ACME ::estatus call_sync(const runnable_array & runnablea);
