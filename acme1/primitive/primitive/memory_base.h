...
public:
   ...
   memory_base() {}
   memory_base(const memory_base & acme) : m_memory(acme.m_memory) {}
   memory_base(memory_base&& acme) { memcpy(&m_memory, &acme.m_memory, sizeof(MEMORY)); xxf_zero(acme.m_memory); }
   virtual ~memory_base();
