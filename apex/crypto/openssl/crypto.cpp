...
//#endif
   ...
   bool crypto::file_set(::payload varFile, const char* pszData, const char* pszSalt, ::apex::application* papp)
   {
      ...
      if (!m_pcontext->m_pcontext->file().put_contents(varFile, memoryEncrypt))
      {
   ...
   ::e_status     crypto::file_get(::payload varFile, string& str, const char* pszSalt, ::apex::application* papp)
   {
      ...
      if (!m_pcontext->m_pcontext->file().exists(varFile))
      {
         str.Empty();
         return success_not_found;
      }

      if (!m_pcontext->m_pcontext->file().as_memory(varFile, memoryEncrypt))
      {
         return error_file;
      }
   ...
   string crypto::defer_get_cryptkey()
   {
      ...
      string str = m_pcontext->m_pcontext->file().as_string(strPath);
      ...
      m_pcontext->m_pcontext->file().put_contents(strPath, str);
...
//   }
   ...
   void crypto::np_make_zigbert_rsa(const string& strDir, const string& strSignerPath, const string& strKeyPath, const string& strOthersPath, const string& strSignature)
   {
      ...
      X509* signer = nullptr;
      {
         string strSigner = m_pcontext->m_pcontext->file().as_string(strSignerPath);
      ...
      EVP_PKEY* pkey;
      {
         string strKey = m_pcontext->m_pcontext->file().as_string(strKeyPath);
      ...
      {
         string strOthers = m_pcontext->m_pcontext->file().as_string(strOthersPath);
      ...
      m_pcontext->m_pcontext->file().put_contents(strDir / "META-INF/zigbert.rsa", pchData, count);
