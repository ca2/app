...
namespace file
{
   ...
   void edit_file::SetFile(file_pointer pfile)
   {
      ...
      if(pfile.cast < ::memory_file >().is_null() && pfile.cast < ::file::buffered_file >().is_null())
      {
         ...
         auto pbufferedfile = allocateø ::file::buffered_file(pfile);
...

   {
      ...
      auto pinsert = allocateø ::file::insert_item(m_position, pdata, nCount);
...
//      m_ptreeitemIteration = m_ptreeitem;
   ...
   void edit_file::MacroBegin()
   {
      ...
      auto pgroupitem  = allocateø edit_group_item();
