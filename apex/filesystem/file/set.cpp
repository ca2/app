...
namespace file
{
   ...
   void set::refresh()
   {
      ...
      for(i = 0; i < m_ppathaSearch->get_size(); i++)
      {
         ...
         m_listing.m_pprovider = m_pcontext->m_pcontext;
         ...
         if(bRecursive)
         {
            ...
            m_pcontext->m_pcontext->dir().rls_file_pattern(m_listing, pathFolder, m_straFilter);

         }
         else
         {

            m_pcontext->m_pcontext->dir().ls_file_pattern(m_listing, pathFolder, m_straFilter);
   ...
   bool set::initialize_os_wallpaper(bool bAddSearch)
   {
      ...
      if (!m_pcontext->m_pcontext->os().initialize_wallpaper_fileset(this, bAddSearch))
      {
      ...
      if(m_listing.is_empty())
      {
         ...
         if (!bAddSearch)
         {
            ...
            m_pcontext->m_pcontext->os().initialize_wallpaper_fileset(this, true);
            ...
            if (m_listing.is_empty())
            {
               ...
               m_pcontext->m_pcontext->os().::os_context::initialize_wallpaper_fileset(this, bAddSearch);
