...
namespace fs
{
   ...
   bool native::has_subdir(const ::file::path & path)
   {
      ...
      return m_pcontext->m_pcontext->dir().has_subdir(path);
   ...
   bool native::is_link(const ::file::path & path)
   {
      ...
      if(m_pcontext->m_pcontext->os_is_alias(path))
      {
   ...
   ::file::listing & native::ls_relative_name(::file::listing & listing)
   {
      ...
      m_pcontext->m_pcontext->dir().ls_relative_name(listing);
   ...
   int native::is_dir(const ::file::path & path)
   {
      ...
      return m_pcontext->m_pcontext->dir().is(path) ? 1 : 0;
   ...
   ::file::listing & native::root_ones(::file::listing & listing)
   {
      ...
      m_pcontext->m_pcontext->dir().root_ones(listing);

      ::file::path pathDropbox = m_pcontext->m_pcontext->defer_process_path("dropbox://");

      if(pathDropbox.has_char() && m_pcontext->m_pcontext->dir().is(pathDropbox))
      {
      ...
      ::file::path pathOneDrive = m_pcontext->m_pcontext->defer_process_path("onedrive://");
      
      if(pathOneDrive.has_char() && m_pcontext->m_pcontext->dir().is(pathOneDrive))
      {
...
//#endif
   ...
   //   return m_pcontext->m_pcontext->file().get_ascendants_path(pszPath, stra);
   ...
   //   return m_pcontext->m_pcontext->file().get_ascendants_name(pcsz, straParam);
   ...
   //      strPath = m_pcontext->m_pcontext->dir().name(strPath);
   ...
   //   return m_pcontext->m_pcontext->file().name_(pszPath);
   ...
   bool native::file_move(const ::file::path & pszDst,const ::file::path & pszSrc)
   {
      ...
      m_pcontext->m_pcontext->file().move(pszDst, pszSrc);
   ...
   bool native::file_exists(const ::file::path & path)
   {
      ...
      return m_pcontext->m_pcontext->file().exists(path);
