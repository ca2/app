...
namespace sockets
{
   ...
   void http_debug_socket::OnFirst()
   {
      ...
      print(m_request.attr(__id(http_method)).get_string() + " " + m_request.attr(__id(request_uri)) + " " + m_request.attr(__id(http_version)) + "\n");
   }
   ...
   void http_debug_socket::OnData(const char *point_i32,memsize l)
   {
      write(point_i32,l);
      m_read_ptr += (int)l;
