      ...
      if(inheader("accept-encoding").get_string().find("gzip") >= 0)
      {
         ...
         if (str.find_ci("text") >= 0 || str.find_ci("javascript") >= 0)
         {
            ...
            if (response().m_strFile.has_char())
            {
               ...
               compress.gz(pfile, m_pcontext->m_pcontext->file().get_reader(response().m_strFile));
   ...
   bool http_base_socket::read_file(const ::file::path& pcszParam, pointer_array < int_array >* prangea, const char* pszContentType)
   {
      ...
      if (prangea == nullptr || prangea->get_count() == 0)
      {
         ...
         if (::str::begins_ci(strContentType, "audio/"))
         {
            ...
            auto preader = m_pcontext->m_pcontext->file().get_reader(pcsz);
         ...
         else
         {
            ...
            if (bMd5Request)
            {
               ...
               response().print(m_pcontext->m_pcontext->file().md5(pcsz));
            ...
            else
            {
               ...
               auto preader = m_pcontext->m_pcontext->file().shared_reader(pcsz);
