...
namespace sockets
{
   ...
   void http_post_socket::AddFile(const string & name,const string & filename,const string & type)
   {
      ...
      if (m_pcontext->m_pcontext->file().exists(filename))
      {
         ...
         m_pmultipart->m_map[name].m_spfile = m_pcontext->m_pcontext->file().get_file(filename, ::file::e_open_binary | ::file::e_open_read | ::file::e_open_share_deny_none);
         ...
         //m_mapContentLength[filename]  = m_pcontext->m_pcontext->file().length(filename);
