...
//#endif
   ...
   void http_debug_socket::OnFirst()
   {
      ...
      print(m_request.attr("http_method").as_string() + " " + m_request.attr("request_uri").as_string() + " " + m_request.attr("http_version").as_string() + "\n");
   }
   ...
   void http_debug_socket::OnData(const_char_pointer p,memsize l)
   {
      write({ p,l });
      m_read_ptr += (int)l;
