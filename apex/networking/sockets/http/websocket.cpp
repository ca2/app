...
namespace sockets
{
   ...
   //websocket::websocket(const string & url_in, const ::scoped_string & scopedstrProtocol) :
   //   //::object(&h),
   //   //base_socket(h),
   //   //socket(h),
   //   //stream_socket(h),
   ...
   //websocket::websocket(const ::scoped_string & scopedstrHost, ::networking::port_t port, const string & url_in) :
   //   object(h.get_app()),
   //   base_socket(h),
   //   socket(h),
   //   stream_socket(h),
   ...
   void websocket::initialize_websocket(::sockets::http_socket * phttpsocket)
   {
      ...
      m_phttpsocket->m_emethod = e_http_method_get;
   ...
   bool websocket::client_ping_pong_ok()
   {
      ...
      if (!m_bWebSocket)
      {
         ...
         if (m_timeLastPing.elapsed() > 60_s)
         {
            ...
            m_phttpsocket->SetCloseAndDelete();
...
//         memory m1(&data[header_size], n);
   ...
   bool websocket::http_request_step()
   {

      if (!m_bWebSocket)
      {

         m_phttpsocket->inheader("Host") = m_phttpsocket->m_urlparts.connect().host();
         ...
         //      if (!(bool)inattr("minimal_headers"))
         {
            ...
            string strUserAgent = m_phttpsocket->MyUseragent();
            m_phttpsocket->inheader("user-agent") = strUserAgent;
         }
         //inheader("Content-Length") = 0;
         m_phttpsocket->inheader("Upgrade") = "websocket";
         m_phttpsocket->inheader("Connection") = "Upgrade";
         ...
         {
            m_phttpsocket->inheader("Sec-WebSocket-Protocol") = m_strWebSocketProtocol;

         }
         m_phttpsocket->inheader("Sec-WebSocket-Version") = "13";

         if (m_strOrigin.has_character())
         {

            m_phttpsocket->inheader("Origin") = m_strOrigin;
...
//   void websocket::OnLine(const ::scoped_string & scopedstrLine)
//   {
//
//
...
//   }
   ...
   void websocket::OnDataComplete()
   {

   }


   void websocket::write(const void * p, ::memsize s)
   {

      synchronous_lock synchronouslock(m_pmutexWebsocketWrite, DEFAULT_SYNCHRONOUS_LOCK_SUFFIX);

      m_phttpsocket->write(p, s);

   }


//   void websocket::InitSSLClient()
//   {
...
//   }
   ...
   long websocket::cert_common_name_check(const ::string & common_name)
   {
   ...
   bool websocket::send_network_payload(::payload varNetworkPayload)
   {
      ...
      return !m_phttpsocket->Lost();

   }


   bool websocket::send_memory(memory & memoryParameter)
   {
