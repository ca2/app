         ...
         if(psocket != nullptr)
         {
            ...
            if(pstreamsocket != nullptr)
            {
               if(pstreamsocket->m_writer != nullptr)
               {
                  ...
                  writer->StoreAsync()->Completed = ref __new< ::winrt::Windows::Foundation::AsyncOperationCompletedHandler < unsigned int >  >([=]
...
//#if defined(BSD_STYLE_SOCKETS)
   ...
   {
      // check cache
      resolv_socket *resolv = __new< resolv_socket >(*this, p, host, port);
   ...
   {
      // check cache
      resolv_socket *resolv = __new< resolv_socket >(*this, p, host, port, true);
      resolv -> SetId(++m_resolv_id);
      resolv -> SetDeleteByHandler();
      ::networking::address addressLocal("127.0.0.1",m_resolver_port);
      if(!resolv -> open(addressLocal))
   ...
   {
      // check cache
      resolv_socket *resolv = __new< resolv_socket >(*this, p, a);
      resolv -> SetId(++m_resolv_id);
      resolv -> SetDeleteByHandler();
//      ipaddr_t local;
      ::networking::address addressLocal("127.0.0.1",m_resolver_port);
   ...
   void socket_handler::EnableResolver(port_t port)
   {
      if (!m_resolver)
      {
         ...
         m_resolver = __new< resolv_server(get_app >(), port);
      }
   }
   ...
   void socket_handler::CheckList(socket_id_list& ref,const string & listname)
   {
      ...
      while(pnode != nullptr)
      {
         ...
         if (!found)
         {
            ...
            informationf("CheckList failed for \"%s\"\n", listname);
         }
      }
   }
   ...
   void socket_handler::AddList(SOCKET s,list_t which_one,bool add)
   {
      if (s == INVALID_SOCKET)
      {
         informationf("AddList:  invalid_socket");
         return;
      }
      socket_id_list& ref =
