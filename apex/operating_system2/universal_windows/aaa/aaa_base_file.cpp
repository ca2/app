...
int_bool file_system()->put_contents(const ::file::path & path,const char * contents,count len)
{
...
pdirectorysystem->create(::file_path_folder(path));
   ...
   int_bool bOk = WriteFile(hfile,contents,(unsigned int)dwWrite,&dwWritten,nullptr) != false;
...
//   unichar * pwszPath2 = aaa_primitive_new unichar[iBufSize];
//   int iCmp = -1;
...
//
//bool PrintModules(string & strImage,unsigned int processID,const ::scoped_string & scopedstrDll)
//{
//
//   HANDLE hProcess;
//
...
//
//   unsigned int * aProcesses = aaa_primitive_new unsigned int[1024 * 8];
//
//   unsigned int cbNeeded,cProcesses;
//
//   unsigned int u;
//
//   if(!EnumProcesses(aProcesses,124 * 8 * sizeof(unsigned int),&cbNeeded))
//   {
//      delete aProcesses;
//      return;
//   }
...
//
//   cProcesses = cbNeeded / sizeof(unsigned int);
//
...
// //    HMODULE hModule = GetModuleHandle(_T("apex.dll"));
// //    if(!hModule)
// //    {
// //       information(L"GetModuleHandle(_T(\"apex.dll\")); failed");
// //       return 0;
// //    }
// //    string csFilePath;
...
// // extern "C" CLASS_DECL_APEX void GetOpenedFiles(const ::wide_character * lpPath,OF_TYPE Filter,OF_CALLBACK CallBackProc,uptr pUserContext)
// // {
...
// // unsigned int ThreadProc(void * lParam)
// // {
...
// //          CloseHandle(hProcess);
// //       }
// //       //unsigned int dwRet = pGetFinalPathNameByHandle( hDup, pThreadParam->lpPath, MAX_PATH, 0 );
// //       //unsigned int dwRet = GetFileInformationByHandleEx(hDup, FileNameInfo, pinfo, MAX_PATH * 8);
// //       unsigned int dwRet = GetFinalPathNameByHandleW(hDup,pThreadParam->lpPath,MAX_PATH,0);
...
// //    PSYSTEM_HANDLE_INFORMATION pSysHandleInformation = aaa_primitive_new SYSTEM_HANDLE_INFORMATION;
...
// //    unsigned int dwReturned = dwsize;
...
int_bool ensure_file_size_handle(HANDLE h,unsigned long long iSize)
{
   ...
   if(((unsigned long long)dwLo | ((unsigned long long)dwHi << 32)) != iSize)
   {
      ...
      if(SetFilePointer(h,iSize & 0xffffffff,&l,SEEK_SET) != (unsigned int)(iSize & 0xffffffff))
         return false;
...
//      return 0;
//
//   FILE *file = aaa_primitive_new FILE;
...
//
//   unsigned int br;
//   if(!ReadFile(hFile,src,(unsigned int)(size*count),&br,0))
...
//            {
//               if(!WriteFile(hFile,&src[startpos + dwWritten],(unsigned int)minimum(1024,i - startpos - dwWritten),&bw2,0))
//                  return 0;
//               bw += bw2;
//               dwWritten += bw2;
//            }
...
//      while(i - startpos - dwWritten > 0)
//      {
//         WriteFile(hFile,&src[startpos + dwWritten],(unsigned int)minimum(1024,i - startpos - dwWritten),&bw2,0);
//         bw += bw2;
//         dwWritten += bw2;
//      }
//   }
...
//      while(s - dwWritten > 0)
//      {
//         WriteFile(hFile,&src[dwWritten],(unsigned int)minimum(1024,s - dwWritten),&bw2,0);
//         bw += bw2;
//         dwWritten += bw2;
//      }
//   }
...
unsigned int WinSetFilePointer(HANDLE h,int lMove,PLONG plHi,unsigned int dwMeth)
{
...
unsigned int GetFileSize(HANDLE h,LPDWORD lpdwHi)
{
...
unsigned int WinGetFileAttributes(const unichar * psz)
{
...
HANDLE create_file(const char * lpcszFileName,unsigned int dwDesiredAcces,unsigned int dwShareMode,LPSECURITY_ATTRIBUTES lpSA,unsigned int dwCreationDisposition,unsigned int dwFlagsAndAttributes,HANDLE hTemplateFile)
{
...
::winrt::Windows::Storage::StorageFile ^ get_os_file(const char * lpcszFileName,unsigned int dwDesiredAcces,unsigned int dwShareMode,LPSECURITY_ATTRIBUTES lpSA,unsigned int dwCreationDisposition,unsigned int dwFlagsAndAttributes,HANDLE hTemplateFile)
{
   ...
   CREATE_ALWAYS
   2
   Creates a aaa_primitive_new file, always.
   If the specified file exists and is writable, the function overwrites the file, the function succeeds, and last-error code is set to ERROR_ALREADY_EXISTS (183).
   If the specified file does not exist and is a valid path, a aaa_primitive_new file is created, the function succeeds, and the last-error code is set to zero.
   For more information, see the Remarks section of this topic.

   CREATE_NEW
   1
   Creates a aaa_primitive_new file, only if it does not already exist.
   If the specified file exists, the function fails and the last-error code is set to ERROR_FILE_EXISTS (80).
   If the specified file does not exist and is a valid path to a writable location, a aaa_primitive_new file is created.
...
//   unsigned int dwFileAttributes = ::windows_get_file_attributes(wstr);
//   if(dwFileAttributes != INVALID_FILE_ATTRIBUTES && (dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)
//      return true;
//   else
//      return false;
...
//      dwWrite = len;
//   unsigned int dwWritten = 0;
//   bool bOk = ::WriteFile(hfile,contents,(unsigned int)dwWrite,&dwWritten,nullptr) != false;
//   ::CloseHandle(hfile);
//   return dwWrite == dwWritten && bOk != false;
//
//}
...
string file_system()->as_string(const ::file::path & path, character_count iReadAtMostByteCount)
{
   ...
   unsigned int dwSize = ::GetFileSize(hfile,&dwSizeHigh);
...
bool file_as_memory(memory_base & memory,const ::file::path & path, iptr iReadAtMostByteCount)
{
   ...
   ::collection::count  count = ::GetFileSize(hfile,&dwSizeHigh);
   ...
   ::ReadFile(hfile,memory.get_data(),(unsigned int)memory.get_size(),&dwRead,nullptr);
