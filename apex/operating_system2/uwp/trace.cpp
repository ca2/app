...
static void TraceDDE(const char * lpszPrefix, const MSG* pMsg)
{
   ...
   if (pMsg->message == WM_DDE_EXECUTE)
   {
      ...
      {
//         ::infomration(::apex::trace::category_AppMsg, 0, "Warning: Unable to unpack WM_DDE_EXECUTE lParam %08lX.\n",
         //          pMsg->lParam);
         return;
      }
...
//      ::infomration(::apex::trace::category_AppMsg, 0, "%s: Execute '%s'.\n", lpszPrefix, lpszCommands);
   ...
   else if (pMsg->message == WM_DDE_ADVISE)
   {
      ...
      if (!UnpackDDElParam(WM_DDE_ADVISE, pMsg->lParam,
                           (uptr*)&hAdvise, &nItem))
      {
//         ::infomration(::apex::trace::category_AppMsg, 0, "Warning: Unable to unpack WM_DDE_ADVISE lParam %08lX.\n",
//            pMsg->lParam);
         return;
      }
      aItem = (ATOM)nItem;
