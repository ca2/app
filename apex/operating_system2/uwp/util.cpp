...
/*bool CLASS_DECL_APEX __custom_log_font(unsigned int nIDS, LOGFONT* pLogFont)
{
...
bool CLASS_DECL_APEX __is_combo_box_control(oswindow hWnd, unsigned int nStyle)
{
   if (hWnd == nullptr)
      return false;
...
bool CLASS_DECL_APEX __compare_class_name(oswindow hWnd, const_char_pointer lpszClassName)
{
...
oswindow CLASS_DECL_APEX __child_window_from_point(oswindow hWnd, ::int_point int_point)
{
   ASSERT(hWnd != nullptr);
...
void CLASS_DECL_APEX __set_window_text(::user::interaction * hWndCtrl, const_char_pointer lpszNew)
{
