...
namespace linux
{
   ...
   //::apex::get_system()       ::apex::get_system() application, "::apex::get_system() Tools" such as say a log viewer or network monitor
...
//   }
   ...
   void desktop_file::create()
   {
      ...
      ::file::path path = get_context()->dir().matter("app.desktop");

      get_context()->file().lines(m_straLine, path);
      ...
      string strTitle = get_application()->get_title();

      string_array straCategories = proper_category(get_application()->get_categories());
...
//      ::file::path pathLaunch;
      ...
      strName = get_application()->m_strAppId;
      ...
      ::file::path pathIcon = get_context()->dir().matter("main/icon-256.png");

      {

         get_context()->file().get_file(pathIcon, ::file::e_open_read);
      ...
      strScript += "   cd " + string(get_context()->file().module().folder()) + "/\n";

      strScript += "\n";

      strScript += "   export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:.:" + string(get_context()->file().module().folder()) + "\n";

      */

      //strScript += "\n";

      //strScript += "   /bin/bash -i -c \"" + string(get_context()->file().module()) + "\" ${@:1:99}\n";
...
//
      ...
      //strScript += "/bin/bash -l -c " + string(get_context()->file().module()) + " ${@:1:99}\n";

      strScript += string(get_context()->file().module()) + " ${@:1:99}\n";
      ...
      string strModule = get_context()->file().module();
      ...
      //straLine._007SetLine("[Desktop Entry]", "Exec", string(get_context()->file().module()) + " %U");
      straLine._007SetLine("[Desktop Entry]", "Exec", strModule + " %U");
      //straLine._007SetLine("Path", string(get_context()->file().module().folder()));

      if(get_context()->file().exists(pathIcon))
      {
      ...
      //straLine._007SetLine("[Desktop Action transparent-frame]", "Exec", string(get_context()->file().module()) + " : post transparent_frame");
      //straLine._007SetLine("[Desktop Action transparent-frame]", "Exec", strName + " : post transparent_frame");
   ...
   void desktop_file::write()
   {
      ...
      get_context()->file().put_lines(path, m_straLine);
