...
CLASS_DECL_APEX bool process_contains_module(string & strImage, DWORD processID, const char * pszLibrary)
{
   ...
   if (EnumProcessModules(hProcess, hmods.get_data(), (DWORD) (hmods.get_size_in_bytes()), &cbNeeded))
   {
      ...
      for (i = 0; i < ::index(cbNeeded / __sizeof(HMODULE)); i++)
      {
...
CLASS_DECL_APEX ::file::path core_app_path(string strAppId)
{
   ...
   path = "C:\\apex\\time\\x64\\basis\\" + strAppId + ".exe";
...
CLASS_DECL_APEX BOOL LaunchAppIntoDifferentSession(const char* pszProcess, const char* pszCommand, const char* pszDir, STARTUPINFOW* psi, PROCESS_INFORMATION* ppi, int iSession)
{
...
CLASS_DECL_APEX bool enable_windows_token_privilege(HANDLE h, LPTSTR pcszName)
{
...
CLASS_DECL_APEX BOOL LaunchAppIntoSystemAcc(const char* pszProcess, const char* pszCommand, const char* pszDir, STARTUPINFOW* psi, PROCESS_INFORMATION* ppi)
{
   //PROCESS_INFORMATION pi;
   //STARTUPINFO si;
   BOOL bResult = FALSE;
