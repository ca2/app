#include "framework.h"
#include "apex/const/procedure.h"
...
namespace apex
{
   ...
   void ipi::task::do_task(const string & strObject, const string & strMember, const var_array & vara)
   {
      ...
      ::apex::ipc::tx & txc = m_pcall->m_pipi->tx(m_pcall->m_strApp, m_idPid);
...
started:
      ...
      if(m_txmap[strKey].is_null())
      {
         ...
         m_txmap[strKey] = __new(::apex::ipc::tx);
   ...
   ::apex::ipc::tx & ipi::tx(const string & strApp, const ::id & iPid)
   {
   ...
   void ipi::on_ipc_receive(::apex::ipc::rx * prx, const char * pszMessage)
   {
