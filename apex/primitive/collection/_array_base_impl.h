...
::index array_base < TYPE, ARG_TYPE, ALLOCATOR >::remove_at(::index nIndex,::count nCount)
{
   ...
   if(nMoveCount)
   {
      ::apex::memmove_s(m_pData + nIndex,(size_t)nMoveCount * sizeof(TYPE),m_pData + nUpperBound,(size_t)nMoveCount * sizeof(TYPE));
   }
...
void array_base < TYPE, ARG_TYPE, ALLOCATOR >::free_extra()
{
   ...
   if(m_nSize != m_nMaxSize)
   {
      ...
      if(m_nSize != 0)
      {
         ...
         ::apex::memcpy_s(pNewData, (size_t)m_nSize * sizeof(TYPE),m_pData, (size_t)m_nSize * sizeof(TYPE));
      }
...
::index array_base < TYPE, ARG_TYPE, ALLOCATOR >::insert_at(::index nIndex,const TYPE & newElement,::count nCount /*=1*/)
{
   ...
   else
   {
      ...
      ::apex::memmove_s(m_pData + nIndex + nCount,(size_t) ((nOldSize - nIndex) * sizeof(TYPE)),m_pData + nIndex,(size_t)((nOldSize - nIndex) * sizeof(TYPE)));
...
#endif      // copy new data from old
      ::apex::memcpy_s(pNewData,(size_t)nNewMax * sizeof(TYPE),m_pData,(size_t)m_nSize * sizeof(TYPE));
