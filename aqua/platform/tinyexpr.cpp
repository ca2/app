...
#define NEW_EXPR(type, ...) new_expr((type), {__VA_ARGS__})
   ...
   void te_free_parameters(const te_expr *n) {
      if (!n) return;
   ...
   static const te_variable *find_builtin(const_char_pointer pszName, ::i32 len) {
      ...
      while (imax >= imin) {
         ...
         ::i32 c = strncmp(pszName, functions[i].pszName, len);
         if (!c) c = '\0' - functions[i].pszName[len];
         if (c == 0) {
            return functions + i;
         } else if (c > 0) {
            imin = i + 1;
   ...
   static const te_variable *find_lookup(const state *s, const_char_pointer pszName, ::i32 len) {
      ...
      for (var = s->find, iters = s->lookup_len; iters; ++var, --iters) {
         if (strncmp(pszName, var->pszName, len) == 0 && var->pszName[len] == '\0') {
            return var;
         }
      }
      return 0;
               ...
               if (!var) {
                  ...
                  switch(TYPE_MASK(var->type))
                  {
                     ...
                     case TE_FUNCTION4: case TE_FUNCTION5: case TE_FUNCTION6: case TE_FUNCTION7:     /* Falls through. */
                        s->type = var->type;
                        s->function = var->address.function;
                        break;
                  }
               }
   ...
   ::f64 te_eval(const te_expr *n) {
      ...
      switch(TYPE_MASK(n->type)) {
         ...
         case TE_FUNCTION4: case TE_FUNCTION5: case TE_FUNCTION6: case TE_FUNCTION7:
            switch(ARITY(n->type)) {
               case 0: return n->function.m_fn0();
               case 1: return n->function.m_fn1(M(0));
               case 2: return n->function.m_fn2(M(0), M(1));
               case 3: return n->function.m_fn3(M(0), M(1), M(2));
               case 4: return n->function.m_fn4(M(0), M(1), M(2), M(3));
               case 5: return n->function.m_fn5(M(0), M(1), M(2), M(3), M(4));
               case 6: return n->function.m_fn6(M(0), M(1), M(2), M(3), M(4), M(5));
               case 7: return n->function.m_fn7(M(0), M(1), M(2), M(3), M(4), M(5), M(6));
               default: return NAN;
            }
         ...
         case TE_CLOSURE4: case TE_CLOSURE5: case TE_CLOSURE6: case TE_CLOSURE7:
            switch(ARITY(n->type)) {
               case 0: return n->function.m_clj0(n->parameters[0]);
               case 1: return n->function.m_clj1(n->parameters[1], M(0));
               case 2: return n->function.m_clj2(n->parameters[2], M(0), M(1));
               case 3: return n->function.m_clj3(n->parameters[3], M(0), M(1), M(2));
               case 4: return n->function.m_clj4(n->parameters[4], M(0), M(1), M(2), M(3));
               case 5: return n->function.m_clj5(n->parameters[5], M(0), M(1), M(2), M(3), M(4));
               case 6: return n->function.m_clj6(n->parameters[6], M(0), M(1), M(2), M(3), M(4), M(5));
               case 7: return n->function.m_clj7(n->parameters[7], M(0), M(1), M(2), M(3), M(4), M(5), M(6));
               default: return NAN;
            }
...
#undef M
   ...
   static void te_optimize(const te_expr *n) {
