...
namespace xml
{
   ...
   node & node::operator = (const node & node)
   {
      ...
      //m_ppropertyset      = allocateø(property_node.m_attra;
   ...
   */
   ...
   void node::LoadProcessingInstruction(::ansi_range & rangeXml, parse_info * pparseinfo)
   {
      ...
      if( m_pdocument )
      {
         ...
         ::pointer<node>pnode = allocateø node(this);
         pnode->m_pnodeParent = this;
         pnode->m_pdocument = m_pdocument;
         pnode->m_enode = ::data::e_node_xml_pi;

         const_char_pointer pTagEnd = ::ansi_range(pszStart, rangeXml.m_end).find_first_character_in(" ?>");
...
//         }
//
//         const_char_pointer pszStart = rangeXml.m_begin;
//
//         // XML Attr Name
...
//   }
   ...
   void node::LoadComment(::ansi_range & rangeXml, parse_info * pparseinfo)
   {
      ...
      if(pnodeParent)
      {
         ...
         auto pnode = allocateø node(this);
   ...
   void node::LoadOtherNodes(bool* pbRet, ansi_range & rangeXml, parse_info * pparseinfo)
   {
      ...
      *pbRet = false;
      ...
      while( rangeXml.has_character() && do_other_type)
      {
         ...
         rangeXml.consume_spaces(0);
         const_char_pointer prev = rangeXml.m_begin;
   ...
   void node::load(const ::scoped_string & scopedstrXml, parse_info * pparseinfo)
   {
      ...
      const_char_pointer pszNext = nullptr;
      ...
      while (true)
      {
            ...
            //                        ^- current pointer
         {
            ...
            if (::str::trimmed_is_empty(pnode->m_strValue))
            {
               ...
               //   //   if(pszEnd[0] == '&')
               //   ////   {
               //   ////      pszEnd = m_pdocument->patch_entity_ref((const_char_pointer &) pszEnd, true, &xml, nullptr);
               //   ////   }
               //   ////   else
               //   ////   {
            ...
            if(rangeXml.has_character() && *rangeXml.m_begin)
            {
               ...
               if (rangeXml.has_character() && *rangeXml.m_begin && *(rangeXml.m_begin + 1) && *rangeXml.m_begin == '<' && *(rangeXml.m_begin + 1) == '/')
               {
                  ...
                  //if ((xml = _tcsskip(xml)))
                  {
                     ...
                     const_char_pointer pszEnd = rangeXml.find_first_character_in(" >");
...
//         }
   ...
   // bro      2002-10-29
   //========================================================
   /*bool node::get_attr_value( const_char_pointer attrname, string & strValue )
   {
   ...
   node * node::add_child( const ::scoped_string & scopedstrName /*= nullptr*/, const ::scoped_string & scopedstrValue /*= nullptr*/ )
   {
      ...
      auto pnode = allocateø node((node *) this);
   ...
   void node::_CopyBranch( node * pnode )
   {
      ...
      for(auto & pnodeChild : m_nodea)
      {
         ...
         if(pnodeChild)
         {
            ...
            auto pnodeNewChild = allocateø class node ();
