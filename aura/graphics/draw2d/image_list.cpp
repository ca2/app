...
i32 image_list::add(::draw2d::icon * picon, int iItem)
{
   ...
   m_pimage->get_graphics()->DrawIcon(iItem * m_size.cx, 0, picon, m_size.cx, m_size.cy, 0, nullptr, DI_IMAGE);
...
i32 image_list::add_icon(var varFile, int iItem)
{
...
i32 image_list::add_file(var varFile, int iItem)
{
   ...
   fork([this, varFile, iItem]()
      {
         ...
         ::image_pointer pimage = load_image(varFile, true);
...
//i32 image_list::add_matter(const char * pcsz, ::object * pobjectContext, int iItem)
//{
//
//   ::file::path path;
//
...
::image_pointer image_list::get_image(int iImage)
{
   ...
   auto estatus = __construct(pimage, m_size);
