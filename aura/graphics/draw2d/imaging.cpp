...
::estatus imaging::initialize(::object * pobjectContext)
{
...
i32                 cy)
{
   ...
   */
   ...
   rectText.left    = x;
   rectText.right   = LONG (x + cx + sizeText.cx);
   rectText.top     = y;
   rectText.bottom  = LONG(y + cy + sizeText.cy);
...

{
   ...
   nSize;
   pSize->cy = (LONG)((tm.tmHeight + tm.tmExternalLeading) * nSize);
...
bool imaging::change_hue(image_list * pilHue, image_list * pil, COLORREF crHue,double dCompress)
{
...
bool imaging::color_blend(image_list * pilBlend, image_list * pil, COLORREF cr, BYTE bAlpha)
{
...
bool imaging::color_blend(::image * pimage, COLORREF cr, BYTE bAlpha)
{
...
////
////   const ::size & size = pitmap->get_size();
...
////#endif
////
////   const ::size & sizeMask = pitmapMask->get_size();
...
bool imaging::clip_color_blend(::draw2d::graphics * pgraphics, const rect & rectParam, COLORREF cr, BYTE alpha)
{
...
bool imaging::color_blend(::draw2d::graphics * pgraphics, const ::rect64 & rect, COLORREF cr, BYTE alpha)
{

   return color_blend(pgraphics,(const ::rect &) rect,cr,alpha);

}


bool imaging::color_blend(::draw2d::graphics * pgraphics, const ::rect & rect, COLORREF cr, BYTE alpha)
{

   return color_blend(pgraphics, rect.origin(), rect.size(), cr, alpha);

}


bool imaging::color_blend(::draw2d::graphics * pgraphics,i32 x,i32 y,i32 cx,i32 cy,COLORREF cr,BYTE bA)
{
...
bool imaging::color_blend(::draw2d::graphics * pgraphics,const ::point & point,const ::size & size,COLORREF cr,BYTE bA)
{
...
bool imaging::CreateBitmap(::draw2d::graphics * pgraphics, ::draw2d::graphics * pdcScreen, ::draw2d::bitmap * pbitmap, ::draw2d::bitmap * pbitmapOld, BITMAP * pmp, i32 cx, i32 cy)
{
   ...
   if(pbitmap->get_os_data() != nullptr)
   {
      ...
      const ::size & size = pbitmap->get_size();
...
bool imaging::blur(::image * pimage, rect rect, i32 iRadius)
{
   ...
   if (!fastblur.initialize(rect.size(), abs(iRadius)))
      return false;
...
bool imaging::channel_gray_blur(::draw2d::graphics *pdcDst,const ::point & pointDst,const ::size & size,::draw2d::graphics * pdcSrc,const ::point & pointSrc,i32 iChannel,i32 iRadius)
{
   ...
   if(!pimageSrc->from(nullptr,pdcSrc,pointSrc,size))
      return false;
   ...
   if(!pimageDst->to(pdcDst,pointDst,size))
      return false;
   ...
   for(i32 i = 0; i < 4; i++)
   {
      ...
      for(y = yL; y <= yU; y++)
      {
         ...
         for(; x <= xU; x++,x3+=4)
         {
            ...
            if(iDivisor == 0)
            {
               pDestination_1[0] = (BYTE)MAX(0,MIN(dwI,255));


               pDestination_1[1] = (BYTE)MAX(0,MIN(dwI,255));


               pDestination_1[2] = (BYTE)MIN(0,MIN(dwI,255));
      ...
      for(y = yL; y <= yU; y++)
      {
         ...
         for(; x <= xU; x++,x3+=4)
         {
            ...
            if(iDivisor == 0)
            {
               ...
               pDestination_1[3] = (BYTE)MIN(0,MIN(dwI,255));
...
/*
bool imaging::color_blend(::draw2d::graphics * pgraphics, const ::point & point, const ::size & size, ::draw2d::graphics * pdcColorAlpha, const ::point & pointAlpha, ::image * pimageWork, ::image * pimageWork2)
{
...
bool imaging::color_blend(::draw2d::graphics * pgraphics,const ::point & pointParam,const ::size & size,::draw2d::graphics * pdcColorAlpha,const ::point & pointAlphaParam,double dBlend)
{

   ::point point(pointParam);

   ::point pointAlpha(pointAlphaParam);
   ...
   else
   {
      ...
      pimage->from(nullptr,pdcColorAlpha,pointAlpha,size);
...
bool imaging::color_blend(::draw2d::graphics * pgraphics,const rect & rectParam,::draw2d::graphics * pdcColorAlpha,const ::point & pointAlpha,double dBlend)
{

   ::rect rect(rectParam);

   return color_blend(pgraphics,rect.top_left(),rect.size(),pdcColorAlpha,pointAlpha,dBlend);
...
//bool imaging::alpha_spread_R2(::draw2d::graphics *pdcDst,const ::point & pointDst,const ::size & size,::draw2d::graphics * pdcSrc,const ::point & pointSrc,BYTE bMin)
//{
//   if(size.cx <= 0 || size.cy <= 0)
//      return true;
//
...
//
//
//   ::rect rectDst(pointDst,size);
//   ::rect rectSrc(pointSrc,size);
//
//   i32 iwDest;
//
//   BITMAPINFO bmiDst;
...
//
//
//   const ::point & pointViewportDst = pdcDst->GetViewportOrg();
//   const ::point & pointViewportSrc = pdcSrc->GetViewportOrg();
//
...
//bool imaging::alpha_spread(::draw2d::graphics *pdcDst,const ::point & pointDst,const ::size & size,::draw2d::graphics * pdcSrc,const ::point & pointSrc,BYTE bMin,i32 iRadius)
//{
//
//   if(size.cx <= 0 || size.cy <= 0)
//      return true;
...
//
//
//   const ::point & pointViewportDest = pdcDst->GetViewportOrg();
//   const ::point & pointViewportSrc = pdcSrc->GetViewportOrg();
//
...
bool imaging::spread__32CC(::image * pimageDst, ::image * pimageSrc,i32 iRadius,COLORREF crSpreadSetColor)
{
   ...
   ::copy_colorref((COLORREF *)pDst, cx,cy,pimageDst->scan_size(),(COLORREF *)pSrc,pimageSrc->scan_size());
...
//
//   const ::point & pointViewportDest = pdcDst->GetViewportOrg();
//   const ::point & pointViewportSrc1 = pdcSrc1->GetViewportOrg();
//   const ::point & pointViewportSrc2 = pdcSrc2->GetViewportOrg();
//
//   i32 xvpDest = xDest + pointViewportDest.x;
...
// dCompress de 0 a 1
bool imaging::HueVRCP(::image * pimage,COLORREF crHue,double dCompress)
{
...
void imaging::AlphaTextOut(::draw2d::graphics *pgraphics,i32 left,i32 top,const char * pcsz,i32 len,COLORREF cr,double dBlend)
{
...
::estatus imaging::load_cursor(::draw2d::cursor * pcursor, ::file::path path, bool bSync, bool bFromCache)
{
   ...
   pcursor->m_pimage = Context.load_image(path, bFromCache, bSync);
...
::estatus imaging::load_svg(::image * pimage, ::memory_pointer pmemory)
{
