...
image_pointer image::rotated(const ::angle & angle, double dScale)
{
   ...
   if (dScale == 1.0)
   {
      ...
      if (angle.degree() == 90.0)
      {
            ...
            auto pimage = pcontext->m_pauracontext->create_image({height(), width() });
      ...
      else  if (angle.degree() == 180.0)
      {
         ...
         auto pimage = pcontext->m_pauracontext->create_image(size());
   ...
   auto pimage = pcontext->m_pauracontext->create_image({ b, a });
...
__pointer(::image) image::get_image(const ::size_i32 & size)
{
   ...
   auto pimageNew = pcontext->m_pauracontext->create_image(size);
