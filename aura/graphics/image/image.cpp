...
//    }
   ...
   ::i32_size image::image::image_source_size() const
   {
   ...
   ::draw2d::bitmap_pointer image::get_bitmap_as_source(::draw2d::graphics * pdraw2dgraphics) const
   {
      ...
      else if (m_bWasMappedAfterLastGraphicsAcquisition)
      {
         ...
         ((::image::image *)this)->m_bWasMappedAfterLastGraphicsAcquisition = false;
   ...
   void image::preserve(const ::i32_size& size, ::enum_flag eflagCreate)
   {
      ...
      else if (m_ppixmapOwned)
      {
         ...
         m_ppixmapOwned->m_memoryPixmap.set_size(this->scan_area_in_bytes());

         //m_memoryPixmap.set_size(this->scan_area_in_bytes());

         //copy(pixmap);

         m_ppixmapOwned->copy(ppixmapImageBefore);
   ...
   //
   //
   // void image::blend(const ::i32_point& pointDst, ::image::image* pimageSrc, const ::i32_point& pointSrc, const ::i32_size& size)
   // {
   //
   ...
   //
   //
   // void image::precision_blend(const ::i32_point& pointDst, ::image::image* pimageSrc, const ::i32_point& pointSrc, const ::i32_size& size)
   // {
   //
   ...
   //    ::i32_size size(sizeParam);
   //
   //    auto ppixmapTarget = pimageDst->map();
   //
   //    auto ppixmapSource = pimageSrc->map();
   //
   //    pointDst += m_point;
   //
   //    if (pointSrc.x < 0)
   ...
   //
   //
   // void image::fork_blend(const ::i32_point& pointDst, ::image::image* pimageSrc, const ::i32_point& pointSrc, const ::i32_size& size)
   // {
   //
   //    //return
   //
   ...
   //
   //
   // void image::draw_ignore_alpha(const ::i32_point& pointDstParam, ::image::image* pimage, const ::i32_rectangle& rectangleSrcParam)
   // {
   //
   //    ::i32_point pointDst(pointDstParam);
   //
   ...
   //
   //
   // void image::blend(const ::i32_point& pointDstParam, ::image::image* pimageSrc, const ::i32_point& pointSrcParam, ::image::image* pimageAlf, const ::i32_point& pointDstAlfParam, const ::i32_size& sizeParam)
   // {
   //
   //    ::image::image* pimageDst = this;
   //
   ...
   void image::blend(const ::i32_point & pointDst,::image::image *pimageSrc, const ::i32_point & pointSrc, const ::i32_size & size)
   {
      ...
      pimageDst->mult_alpha(pointDst, ::i32_size(xEnd, yEnd));
      pimageSrc->mult_alpha(pointSrc, ::i32_size(xEnd, yEnd));
   ...
   */
   ...
   //
   //
   // void image::div_alpha(const ::i32_point& pointDstParam, const ::i32_size& sizeParam)
   // {
   //
   //    ::i32_point pointDst(pointDstParam);
   //
   ...
   //
   //
   // void image::channel_from(::color::enum_channel echannel, ::image::image* pimage, const ::i32_rectangle& rectangleParam)
   // {
   //
   //    map();
   //
   ...
   //
   //
   // void image::channel_multiply(::color::enum_channel echannel, ::image::image* pimage, const ::i32_rectangle& rectangleParam, bool bIfAlphaIgnorePreDivPosMult)
   // {
   //
   //    map();
   //
   ...
   //
   //
   // void image::copy_from_no_create(::image::image *pimage, const ::i32_point & point)
   // {
   //
   //    ::i32_size s(pimage->size() - point);
   //
   ...
   //
   //
   // void image::copy_from(::image::image* pimage, const ::i32_point  & point, ::enum_flag eflagCreate)
   // {
   //
   //    ::i32_size s(pimage->size() - point);
   //
   ...
   //
   //
   // //void image::copy_to(::image::image* pimage, const i32_point & point)
   // //{
   // //
   ...
   //
   //
   // void image::fill_rectangle(const ::i32_rectangle& rectangle, ::color::color color)
   // {
   //
   //    if (m_bMapped)
   //    {
   ...
   //       }
   //
   //       ::i32 iScan = ppixmapThis->m_iScan;
   //
   //       ::i32 iHeight = ppixmapThis->size().cy;
   //
   ...
   //       }
   //
   //       memory_set(ppixmapThis->data(), uch, (memsize)(iScan * iHeight));
   //
   //    //}
   //    //else
   //    //{
   ...
   //
   //
   // ::i64 image::get_rgba_area(::color::color color, const ::i32_rectangle &rect) const
   // {
   //
   ...
   //
   //
   // void image::invert_rgb(const ::i32_rectangle& rectangle)
   //
   // {
   //
   //    ::i32 s = m_iScan;
   ...
   //
   // //
   // //void image_create_image(::image::image* pimage, const ::i32_size& size)
   // //{
   // //
                                    ...
                                    ::i32 iGoodStride)
   {
      ...
      if (m_ppixmapOwned.is_set())
      {
         ...
         m_sizeRaw = m_sizeRaw.maximum(m_ppixmapOwned->m_sizeRaw);
      ...
      //if (m_sizeRaw.is_set())
      {
         ...
         m_sizeRaw = m_sizeRaw.maximum(m_point + size);

      }

      m_eflagElement = eflagCreate;

      ::i32 iScanRequired = raw_size().width() * 4;

      if (iGoodStride > iScanRequired)
      {

         m_iScan = maximum(m_iScan, iGoodStride);

      }
      else
      {

         m_iScan = maximum(m_iScan, iScanRequired);
   ...
   ::image::image* image::get_source_image()
   {
      ...
      auto pimage = calc_current_frame(m_dynamic);
   ...
   CLASS_DECL_AURA void draw_freetype_bitmap(::image::image* m_p, ::i32 Δx, ::i32 Δy, void* pftbitmap, ::i32 xParam,
                                             ::i32 yParam)
   {
   ...
   CLASS_DECL_AURA void draw_freetype_bitmap(::image::image* m_p, ::i32 Δx, ::i32 Δy, void* pftbitmap, ::i32 xParam,
                                             ::i32 yParam, ::u8 aParam, ::u8 rectangle, ::u8 g, ::u8 b)
   {
      ...
      for (i = x, point = 0; i < x_max; i++, i32_point++)
      {
         for (j = y, q = 0; j < y_max; j++, q++)
         {
            ...
            if (a > 0)
            {
               ...
               *((image32_t *)&((::u8 *)m_p->data())[(Δy + j) * m_p->m_iScan + (Δx + i) * 4]) = argb(
                  a * aParam / 255, rectangle, g, b);
   ...
   */
   ...
   ::image_pixmap_lease image::_map(::image::enum_map emap, const ::i32_rectangle& rectangle)
   {
      ...
      m_ppixmapOwned->m_estatus = ::success;

      m_ppixmapOwned->set_ok_flag();

      auto sizeRawThis = raw_size();

      auto pimage32Owned =
         (::image32_t *) m_ppixmapOwned->m_memoryPixmap.data();

      auto bUsingOwnedMemory =
         pimage32Owned
         && m_ppixmapOwned->m_pimage32Raw == pimage32Owned;

      if (sizeRawThis != m_ppixmapOwned->m_sizeRaw
         || m_iScan != m_ppixmapOwned->m_iScan
         || !m_ppixmapOwned->m_pimage32Raw
         || (bUsingOwnedMemory
            && m_ppixmapOwned->m_memoryPixmap.size()
               < (m_ppixmapOwned->m_iScan * m_ppixmapOwned->m_sizeRaw.cy)))
      {
   ...
   void image::create_frame(::image::image_frame * pframeSource, ::image::image_frame_array * pframea)
   //void image::create(::image::image_frame * pframeSource, const ::pixmap * ppixmap, ::image::image_frame_array * pframea)
   {
      ...
      //m_point.x = (decay<decltype(m_point.x)>)(pframeSource->m_rectangle.left * dx);
      ...
      // pframe->m_rectangle.right = m_point.x + m_size.cx;
      // pframe->m_rectangle.bottom = m_point.y + m_size.cy;
      ...
      //
      // ::image::image_source imagesourceFrame(this);
      //
      ...
      // pframe->m_edisposal = pframeSource->m_edisposal;
      // pframe->m_colorTransparent = pframeSource->m_colorTransparent;
      // pframe->m_bTransparent = pframeSource->m_bTransparent;
      // pframe->m_bLocalPalette = pframeSource->m_bLocalPalette;
      // pframe->m_pparticleImage = this;
   ...
   ::image::image_pointer image::frame_image(::image::image_frame * pframe)
   {

      if (!pframe->m_pparticleImage)
      {

         auto pimageNew = create_newø <::image::image>();

         pimageNew->create_frame(pframe, m_pextension->m_pframea);

         pframe->m_pparticleImage = pimageNew;

      }

      return pframe->m_pparticleImage;

   }


   ::image::image_pointer image::calc_current_frame(image_dynamic & dynamic)
   {

      if (!m_pextension || !m_pextension->m_pframea || m_pextension->m_pframea->is_empty())
      {
      ...
      auto pframe = m_pextension->m_pframea->first_pointer();

      if (m_pextension->m_pframea->is_empty(1))
      {

         return frame_image(pframe);
      ...
      if(m_pextension->m_pframea->m_timeTotal <= 0_s)
      {
      ...
      auto uiLoop = elapsed / m_pextension->m_pframea->m_timeTotal;

      if (uiLoop > m_pextension->m_pframea->m_iLoop)
      {

         m_pextension->m_pframea->m_iLoop = (::collection::index) uiLoop;

         dynamic.m_iFrame = -1;

      }

      if (m_pextension->m_pframea->m_countLoop == 0 || m_pextension->m_pframea->m_iLoop < m_pextension->m_pframea->m_countLoop)
      {

         elapsed %= m_pextension->m_pframea->m_timeTotal;

         class ::time timeUpperBound;

         ::collection::index iFrame = 0;

         for (; iFrame < m_pextension->m_pframea->get_count(); iFrame++)
         {

            auto pframe = m_pextension->m_pframea->element_at(iFrame);
         ...
         return frame_image(m_pextension->m_pframea->element_at(iFrame));

      }
      else
      {

         dynamic.m_iFrame = m_pextension->m_pframea->get_upper_bound();

         return frame_image(m_pextension->m_pframea->last());
   ...
   void image::update(::image::image *pimageHost, ::image::image_frame_array * pimageframearray, const ::image::image_drawing & imagedrawing)
   {
      ...
      pimageframearray->m_pparticleImage = pimageHost;
      ...
      for (::collection::index iFrame = 0; iFrame < pframes->get_count(); iFrame++)
      {
         ...
         auto & pframeHost = pimageframearray->element_at(iFrame);
         ...
         pimage->create_frame(pframeSource, pimageframearray);

      }

      pimageframearray->m_timeTotal = timeTotal;

      pimageframearray->m_pparticleImage->m_estatus = ::success;

      pimageframearray->m_pparticleImage->set_ok_flag();
