...
string true_type_font_utilities::GetFontNameFromFile(const ::file::path & path)
{
   ...
   ttOffsetTable.uNumOfTables = swap_unsigned_short(ttOffsetTable.uNumOfTables);
   ttOffsetTable.uMajorVersion = swap_unsigned_short(ttOffsetTable.uMajorVersion);
   ttOffsetTable.uMinorVersion = swap_unsigned_short(ttOffsetTable.uMinorVersion);
   ...
   if (bFound)
   {
      ...
      ttNTHeader.uNRCount = swap_unsigned_short(ttNTHeader.uNRCount);
      ttNTHeader.uStorageOffset = swap_unsigned_short(ttNTHeader.uStorageOffset);
      ...
      for (int i = 0; i < ttNTHeader.uNRCount; i++)
      {
         ...
         {
            ttRecord.uStringLength = swap_unsigned_short(ttRecord.uStringLength);
            ttRecord.uStringOffset = swap_unsigned_short(ttRecord.uStringOffset);
