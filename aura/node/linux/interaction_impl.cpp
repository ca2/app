...
//   }
   ...
   bool interaction_impl::_native_create_window_ex(__pointer(::user::system_struct) pusersystemstruct)
   {

      ENSURE_ARG(pusersystemstruct->m_createstruct.lpszName == nullptr || __is_valid_string(pusersystemstruct->m_createstruct.lpszName));

      if (!m_puserinteraction->pre_create_window(pusersystemstruct))
      {
      ...
      if(pusersystemstruct->m_createstruct.hwndParent == (oswindow) MESSAGE_WINDOW_PARENT)
      {
      ...
      else
      {
         ...
         x11_sync([&]()
         {
            ...
            if(pusersystemstruct->m_createstruct.cx <= 0)
            {

               pusersystemstruct->m_createstruct.cx = 1;

            }

            if(pusersystemstruct->m_createstruct.cy <= 0)
            {

               pusersystemstruct->m_createstruct.cy = 1;
            ...
            INFO("XCreateWindow (l=%d, t=%d) (w=%d, h=%d)", pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y, pusersystemstruct->m_createstruct.cx, pusersystemstruct->m_createstruct.cy);

            Window window = XCreateWindow(display, DefaultRootWindow(display), pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y, pusersystemstruct->m_createstruct.cx, pusersystemstruct->m_createstruct.cy,
            0,
            m_iDepth,
            InputOutput,
            vis,
   ...
   //
   //            uistate.m_point.set(pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y);
   //
   //            uistate.m_size.set(pusersystemstruct->m_createstruct.cx, pusersystemstruct->m_createstruct.cy);
   //
   ...
   //
            {
               ...
               m_puserinteraction->layout().sketch() = ::point(pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y);

               m_puserinteraction->layout().sketch() = ::size(pusersystemstruct->m_createstruct.cx, pusersystemstruct->m_createstruct.cy);

               m_puserinteraction->layout().sketch().screen_origin() = ::point(pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y);
...
//            else
            ...
            if(pusersystemstruct->m_createstruct.lpszName != nullptr && strlen(pusersystemstruct->m_createstruct.lpszName) > 0)
            {

               XStoreName(m_oswindow->display(), m_oswindow->window(), pusersystemstruct->m_createstruct.lpszName);

            }


//            if(pusersystemstruct->m_createstruct.dwExStyle & WS_EX_TOOLWINDOW)
//            {
//
...
//            }
            ...
            if(pusersystemstruct->m_createstruct.style & WS_VISIBLE)
            {
            ...
            //if(m_px11data->m_attr.map_state != IsUnmapped)
            {
               ...
               if(!attr.override_redirect)
               {
                  ...
                  if(is_docking_appearance(m_puserinteraction->layout().sketch().display()))
                  {
                     ...
                     //m_oswindow->set_window_pos(zorder_top, pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y, pusersystemstruct->m_createstruct.cx, pusersystemstruct->m_createstruct.cy, SWP_SHOWWINDOW);
                     m_oswindow->set_window_pos(zorder_top, pusersystemstruct->m_createstruct.x, pusersystemstruct->m_createstruct.y, pusersystemstruct->m_createstruct.cx, pusersystemstruct->m_createstruct.cy, 0);
      ...
      if(bOk)
      {
         ...
         m_puserinteraction->send_message(e_message_create, 0, (LPARAM) &pusersystemstruct->m_createstruct);
   ...
   bool interaction_impl::pre_create_window(::user::system_struct * pusersystemstruct)
   {
