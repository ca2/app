...
CLASS_DECL_AURA i32 call_async(const char * pszPath, const char * pszParam, const char * pszDir, ::edisplay edisplay, bool bPrivileged, unsigned int * puiPid = nullptr);
...
CLASS_DECL_AURA bool is_command_line_param_true(string& strValue, const char* pszCommandLine, const char* pszParam, bool bDefault = false);
...
CLASS_DECL_AURA bool launch_command(const char * const pszCommand);
...
CLASS_DECL_AURA id_array module_path_get_pid(const char * pszModuleName, bool bModuleNameIsPropertyFormatted = true);


#ifndef WINDOWS

CLASS_DECL_AURA string_array cmdline_from_pid(unsigned int pid);
CLASS_DECL_AURA id_array app_get_pid(const char * pszModuleName);
