...
CLASS_DECL_AURA ::file::path core_app_path(string strAppId)
{
   ...
   path = "C:\\aura\\time\\x64\\basis\\" + strAppId + ".exe";
...
CLASS_DECL_AURA BOOL LaunchAppIntoDifferentSession(const char* pszProcess, const char* pszCommand, const char* pszDir, STARTUPINFOW* psi, PROCESS_INFORMATION* ppi, int iSession)
{
...
CLASS_DECL_AURA bool enable_windows_token_privilege(HANDLE h, LPTSTR pcszName)
{
...
CLASS_DECL_AURA BOOL LaunchAppIntoSystemAcc(const char* pszProcess, const char* pszCommand, const char* pszDir, STARTUPINFOW* psi, PROCESS_INFORMATION* ppi)
{
   //PROCESS_INFORMATION pi;
   //STARTUPINFO si;
   BOOL bResult = FALSE;
