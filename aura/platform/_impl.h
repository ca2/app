...
::image_pointer get_image(::object * pobject, const ::payload & varFile, bool bAsync = false)
{
   ...
   ::image_pointer & pimage = ::aura::get_system()->m_mapImage[path];
   ...
   if (!pimage)
   {
      ...
      ::aura::get_system()->m_pimaging->_load_image(pobject->get_context(), pimage, varFile, !bAsync, false);
...
namespace aura
{
   ...
   bool application::gudo_get(const string & strKey, TYPE & obj)
   {
      ...
      synchronization_lock synchronizationlock(::aura::get_system()->m_spmutexUserAppData);
