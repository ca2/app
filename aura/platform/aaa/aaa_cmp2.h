...
void __swap(const TYPE & t1,const TYPE & t2)
{
...
public:
   ...
   void __swap(TUPLE_TYPE&) noexcept
   {
   }
...
public:
   ...
   void __swap(tuple& right)
   {

      __swap(m_val,right.m_val);

      BASE_TYPE::__swap((BASE_TYPE&)right);
...
template<class... TYPES>
inline void __swap(tuple<TYPES...>& left, tuple<TYPES...>& right)
{
   return left.__swap(right);
}
...
template<size_t TUPLE_SIZE> struct cmp_tuple
{
   ...
   {
      ::i32 r = cmp_tuple<TUPLE_SIZE - 1>()(y1,t2);
...
template<size_t TUPLE_SIZE> struct icmp_tuple
{
   ...
   ::i32 operator()(const T1 & t1,const T2 & t2) const
   {
      ::i32 r = icmp_tuple<TUPLE_SIZE - 1>()(y1,t2);
