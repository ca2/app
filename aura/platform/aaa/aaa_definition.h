...
#ifndef __cplusplus
#error aura requires C++ compilation (use a .cpp suffix)
#endif

#ifdef UNDER_CE
#error This version of aura is not currently supported for CE. Look for the CE specific version.
#endif
...
#ifdef UNICODE
#ifndef _UNICODE
#define _UNICODE        // _UNICODE is used by C-runtime/aura API headers
#endif
#endif
...
aura 8 has two existing validation models
...
We added ENSURE because there were too many unreported error paths in aura and we wanted to bail out of more
...
ENSURE might seem slightly better. It is debuggable and consistent with aura in general. In fact, many parts of aura do just this.
...
// rather only used as a aura class.  Additionally, the constructor and
...
///////////////////////////////////////////////////////////////////////////////
// By default, the wizards will generate aaa_primitive_new aura object classes with this
...
#else
// If not linking to 80.DLL, use the static registrar and not building aura.dll
#ifndef _DLL_IMPL
...
#define API CLASS_DECL_AURA __declspec(nothrow) HRESULT __stdcall
#define API_(x)CLASS_DECL_AURA __declspec(nothrow) x __stdcall
#define APIINL API
#define APIINL_(x) API_(x)


#define offsetofclass(aura, derived) ((uptr)(static_cast<aura*>((derived*)_PACKING))-_PACKING)
...
//aura/ API code should use standard pointer to member standard syntax &MyClass::MyMethod, instead
