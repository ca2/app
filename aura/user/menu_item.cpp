...
namespace user
{
   ...
   bool menu_item::load_menu(::data::node * pnode)
   {
   ...
   bool menu_item::load_menu(::data::node * pnode, i32 iLevel)
   {

      ::count iItemCount = pnode->get_node_count();

      m_bPopup = iItemCount > 0 && pnode->name() == "menubar";

      if(pnode->name() == "separator")
      {
      ...
      for (i32 i = 0; i < iItemCount; i++)
      {
         ...
         auto pnodeChild = pnode->node_at(i);
