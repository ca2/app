...
namespace database
{
   ...
   //void     database::connect(const ::scoped_string & scopedstrName, const ::scoped_string & scopedstrHost, const ::scoped_string & scopedstrPort, const ::scoped_string & scopedstrUser, const ::scoped_string & scopedstrPass, const ::scoped_string & scopedstrSocket, ::u64 uConnectionFlags)
   //{
  ...
  /* ::collection::count database::get_affected_rows_count()
   ...
   //   auto pvara = allocateø payload_array();
...
/*
   ...
   bool database::query_table_item(::payload& payload, const ::string & table, const ::scoped_string & scopedstrItem, const ::string & where)
   {
      ...
      strSql.formatf("SELECT `%s` FROM `%s` WHERE %s",scopedstrItem.as_string().c_str(), table.c_str(), where.c_str());
