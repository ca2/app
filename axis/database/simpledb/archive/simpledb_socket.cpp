...
//            u32 dw= ::time::now();
   ...
   bool socket::http_filter_response_header(atom key, string & strValue)
   {
      if(strKey.case_insensitive_order("Location") == 0)
      {
         ...
         else if(domain.m_strName == "account.ca2software.com" && purl->get_protocol(strValue) == "https")
         {
            strValue = "https://" + papp->m_strFontopusServer + purl->get_object(strValue);
         }
      }
   ...
   */
   ...
   bool socket::read_file(const ::file::path & lpcsz,pointer_array < ::i32_array > * prangea, const ::string & pszContentType)
   {
      ...
      {
         outheader("content-type") = "application/x-spa";
      }
      else if(str == "mid")
      {
