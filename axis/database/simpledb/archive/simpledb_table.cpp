...
namespace simpledb
{
   ...
   table::table(database * pdatabase, const ::scoped_string & scopedstrName) :
      ...
      m_xmldocumentMeta(pdatabase->get_app())
   {
      ...
      strMetaPath = directory()->install() / "database" / m_pdatabase->getDatabase() / m_strName/ "meta.xml";
      ...
      if(strFixedPath.is_empty())
         strFixedPath = strMetaPath = directory()->install() / "database" / m_pdatabase->getDatabase() /  m_strName/"fixed.txt";
