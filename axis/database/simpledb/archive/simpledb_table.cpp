...
namespace simpledb
{
      ...
      m_xmldocumentMeta(pdatabase->get_app())
   {
      ...
      strMetaPath = dir()->install() / "database" / m_pdatabase->getDatabase() / m_strName/ "meta.xml";
      ...
      if(strFixedPath.is_empty())
         strFixedPath = strMetaPath = dir()->install() / "database" / m_pdatabase->getDatabase() /  m_strName/"fixed.txt";
