...
namespace simpledb
{
      ...
      m_xmldocumentMeta(pdatabase->get_application())
   {
      ...
      strMetaPath = pcontext->m_pcontext->dir().install() / "database" / m_pdatabase->getDatabase() / m_strName/ "meta.xml";
      ...
      if(strFixedPath.is_empty())
         strFixedPath = strMetaPath = pcontext->m_pcontext->dir().install() / "database" / m_pdatabase->getDatabase() /  m_strName/"fixed.txt";
