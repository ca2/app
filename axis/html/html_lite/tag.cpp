   ...
   :m_pcollAttr(nullptr)
{
   ...
   else if (rSource.m_pcollAttr != nullptr)
   {
      ...
      try
      {
         ...
         m_pcollAttr = __raw_new LiteHTMLAttributes(*rSource.m_pcollAttr, true);
                                 ...
                                 bool bParseAttrib /* = true */)
{
...

   {
      ...
      ASSERT(!bClosingTag);
      nRetVal = (unsigned int) (scopedstrBegin - &strString[iPos]);
   ...
   strTagName = string(scopedstrBegin, pszEnd - pszBegin);
...

   {
      ...
      if (bParseAttrib)   // parse attribute/value pairs?
      {
         ...
         if ((pcollAttr = __raw_new LiteHTMLAttributes()) == nullptr)
         {
//            TRACE0("(Error) lite_html_tag::parseFromStr: Out of memory.\n");
         ...
         nTemp = (unsigned int) pcollAttr->parseFromStr(preader, pszBegin, strString.length() - (scopedstrBegin - (const ::string &) strString));
