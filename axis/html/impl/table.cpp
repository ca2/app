   ...
   namespace impl
   {
      ...
      void table::implement_phase1(html_data * pdata, ::html::element* pelemental)
      {
         ...
         if(ptag != nullptr)
         {
            ...
            {
               m_iBorder         = (::f32) ::str::to_u32(ptag->get_attr_value("border"));
            }

            if(ptag->get_attr_value("cellspacing").has_character())
            {
               m_iCellSpacing    = (::f32) ::str::to_u32(ptag->get_attr_value("cellspacing"));
            }

            if(ptag->get_attr_value("cellpadding").has_character())
            {
               m_iCellPadding    = (::f32) ::str::to_u32(ptag->get_attr_value("cellpadding"));
            }
...
//         }
      ...
      void table::layout_phase0(html_data * pdata)
      {
         ...
         for (::collection::index j = 0; j < m_rowptra.get_size(); j++)
         {
      ...
      void table::layout_phase1_end(html_data * pdata)
      {
         ...
         for (::collection::index i = 0; i < m_columna.get_count(); i++)
         {
         ...
         ::collection::index iCol;

         cell * pcell;

         for (::collection::index i = 0; i < m_cellholdera.get_count(); i++)
         {

            for (::collection::index j = 0; j < m_cellholdera[i].get_count(); j++)
            {
...
//         ::f32 cx = m_iCellSpacing;
      ...
      table::column::column(::collection::index iCol)
      {
      ...
      void table::set_cell(::collection::index iCol, ::collection::index iRow, cell * pcell)
      {
