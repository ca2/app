...
public:
   ...
   {	// proxy for an element
	...
	public:
		context_object& operator=(bool _Val)
      {	// assign Boolean to element
			_Pbitset->set(_Mypos, _Val);
			return (*this);
      }

		context_object& operator=(const context_object& _Bitref)
      {	// assign context_object to element
	...
	private:
		context_object(bitset<_Bits>& _Bitset, size_t _Pos)
      : _Pbitset(&_Bitset), _Mypos(_Pos)
      {	// construct from bitset context_object and position
      }
...
public:
	...
	context_object at(size_t _Pos)	// retained
...
template<size_t _Bits> inline
   ...
   /*inline ::aura::ostream & operator << (::aura::ostream2 & _Ostr, const bitset<_Bits>& _Right);

	inline ::aura::istream & operator>>(::aura::istream  _Istr, bitset<_Bits>& _Right);
