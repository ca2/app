#include "framework.h"
#include "zip/_.h"
#include "zip/_internal.h"
#include "compress_gz.h"
//#include "compress_bz.h"


compress_context::compress_context(::object * pobject) :
   ::object(pobject)
{


}


compress_context::~compress_context()
{


}


bool compress_context::get_patha(::file::path & path, string_array & straPath, const ::file::path & pathSource)
{

   bool bFinalIsZip = false;

   straPath.remove_all();

   path.Empty();

   {

      index iFind = -1;

      index iStart = 0;

      const char * pszFileName = pathSource;

      while ((iFind = ::str::find_ci(".zip:", pszFileName, iStart)) >= 0)
      {

         string strPath = string(&pszFileName[iStart], iFind + strlen(".zip"));

         if (path.is_empty())
         {

            path = strPath;

         }
         else
         {

            straPath.add(strPath);

         }

         iStart = iFind + strlen(".zip:");

      }

      straPath.add(&pszFileName[iStart]);

   }

   return path.has_char();

}


bool compress_context::ungz(::file::file * pfileOut, ::file::file * pfileIn)
{

   uncompress_gz ungz(get_context_object());

   return Context.file().output(pfileOut, &ungz, &::uncompress_gz::transfer, pfileIn);

}





bool compress_context::gz(::file::file * pfileOut, ::file::file * pfileIn, int iLevel)
{

   compress_gz gz(get_context_object(), iLevel);

   return Context.file().output(pfileOut, &gz, &compress_gz::transfer, pfileIn);

}


bool compress_context::unbz(::file::file* pfileOut, ::file::file* pfileIn)
{

   __throw(interface_only_exception());

   return false;

}


//
//   uncompress_bz unbz(get_context_object());
//
//   return Context.file().output(pfileOut, &unbz, &::uncompress_bz::transfer, pfileIn);
//
//}
//
//


bool compress_context::bz(::file::file* pfileOut, ::file::file* pfileIn, int iBlockSize, int iVerbosity, int iWorkFactor)
{

   __throw(interface_only_exception());

   return false;

}



//
//   compress_bz bz(get_context_object(), iBlockSize, iVerbosity, iWorkFactor);
//
//   return Context.file().output(pfileOut, &bz, &::compress_bz::transfer, pfileIn);
//
//
//}


bool compress_context::compress(memory & memoryOut, const ::memory & memoryIn)
{

   memoryOut.set_size(compressBound((uLong)memoryIn.get_size()) * 2);

   uLongf ulDestSize = (uLongf)memoryOut.get_size();

   i32 i = ::compress(memoryOut.get_data(), &ulDestSize, (BYTE *)memoryIn.get_data(), (uLongf)memoryIn.get_size());

   if (i != Z_OK)
   {

      ulDestSize = 0;

   }

   memoryOut.set_size(ulDestSize);

   return i == Z_OK;

}


bool compress_context::uncompress(memory& memoryOut, const ::memory& memoryIn)
{

   uLongf ulDestSize = (uLongf)memoryOut.get_size();

   i32 i = ::uncompress(memoryOut.get_data(), &ulDestSize, (BYTE*)memoryIn.get_data(), (uLongf)memoryIn.get_size());

   if (i != Z_OK)
   {

      ulDestSize = 0;

   }

   memoryOut.set_size(ulDestSize);

   return i == Z_OK;

}

//bool compress_context::uncompress(memory & dest, memory & memory)
//{
//
//   int iRate = 1;
//   while (true)
//   {
//
//      dest.allocate(memory.get_size() * 10);
//
//      uLong l;
//
//      l = uLong (dest.get_size());
//
//      int i = ::uncompress(dest.get_data(), &l, memory.get_data(), uLong (memory.get_size()));
//
//      if (i == Z_OK)
//      {
//
//         dest.allocate(l);
//
//         return true;
//
//      }
//
//      if (i == Z_BUF_ERROR)
//      {
//         iRate++;
//         if (iRate > 4)
//            return false;
//         dest.allocate(dest.get_size() * 2);
//         continue;
//      }
//
//      return false;
//
//   }
//
//}


::count compress_context::extract_all(const ::file::path & pszFile, ::aura::application * pobject)
{

   string strDir = pszFile;

   ::str::ends_eat_ci(strDir, ".zip");

   return extract_all(strDir, pszFile, pobject);

}


::count compress_context::extract_all(const ::file::path & pszDir, const ::file::path & pszFile, ::aura::application * pobject)
{
   
   ::file::patha patha;

   zip_context zip(get_context_object());
   
   zip.extract_all(pszDir, pszFile, &patha);
   
   return patha.get_count();
   
}


bool compress_context::zip(const ::file::path & pszZip, const ::file::path & psz, ::aura::application * pobject)
{

   INFO("compress_context::zip");

   zip::in_file infile;

   if (!infile.zip_open(pszZip, 0))
   {

      ERR("Could not open zip file");

      return false;

   }

   if (Context.dir().is(psz))
   {

      ::file::listing patha(get_context());

      ::file::path strPath;

      file_pointer file;

      patha.rls_file(psz);

      for (i32 i = 0; i < patha.get_size(); i++)
      {

         infile.add_file(psz, patha[i].relative());

      }

   }

   return true;

}


bool compress_context::zip(const ::file::path & psz, ::aura::application * pobject)
{

   __throw(todo());

   return false;

}


bool compress_context::unzip(const var & varFile, const ::file::path & pathZipFileCompressed)
{

   auto pfileTarget = Context.file().get_writer(varFile);

   if (!pfileTarget)
   {

      return false;

   }

   memory m;

   if (!unzip(m, pathZipFileCompressed))
   {

      return false;

   }

   pfileTarget->write(m.get_data(), m.get_size());

   return true;

}


bool compress_context::unzip(memory & m, const ::file::path & pathZipFileCompressed)
{

   zip_context zip(get_context_object());

   ::file::path pathZip;

   string_array straPath;

   if(!get_patha(pathZip, straPath, pathZipFileCompressed))
   { 

      return false;

   }

   auto pfile = Context.file().get_reader(pathZip);

   if (!pfile)
   {

      return false;

   }

   if (!zip.extract(m, pfile, straPath))
   {

      return false;

   }

   return true;

}





