...
CLASS_DECL_AURA bool file_load_stra(const char * lpszName, string_array & stra, bool bAddEmpty = true);
...
CLASS_DECL_AURA bool file_put_contents_dup(const char * path, const memory_base & memory);
CLASS_DECL_AURA int_bool file_put_contents_dup(const char * path, const char * contents);
CLASS_DECL_AURA bool file_as_memory(memory_base & memory, const char * path, memsize iReadAtMostByteCount = -1);
CLASS_DECL_AURA memsize file_as_memory(const char * path, void * p, memsize s);

CLASS_DECL_AURA filesize fsize_dup(HANDLE h);



template < typename N > class numeric_array;
typedef CLASS_DECL_AURA numeric_array < u32 > u32_array;
...
CLASS_DECL_AURA memory file_as_memory(const char * path, strsize iReadAtMostByteCount = -1);
CLASS_DECL_AURA memsize file_as_memory(const char * path, void * p, memsize s);
CLASS_DECL_AURA string file_as_string(const char * path, strsize iReadAtMostByteCount = -1);
...
CLASS_DECL_AURA string get_temp_file_name_dup(const char * lpszName,const char * pszExtension);
...
CLASS_DECL_AURA string solve_relative(const string & str, bool * pbUrl = nullptr);
CLASS_DECL_AURA bool solve_relative_inline(string & str, bool & bUrl, bool & bOnlyNativeFileSep, strsize * iaSlash, int * piSlashCount); // returns true if original string had trailing slash
CLASS_DECL_AURA string defer_solve_relative(const char * pszRelative,const char * pszCurrent);
...
CLASS_DECL_AURA bool file_append_wait_dup(const string & strFile, const string & str, DWORD tickTimeout = INFINITE);
CLASS_DECL_AURA bool file_append_wait_dup(const string & strFile, const char * psz, strsize s, DWORD tickTimeout = INFINITE);

CLASS_DECL_AURA bool write_memory_to_file(FILE * file, const void * pdata, memsize nCount, memsize * puiWritten);
