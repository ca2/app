...
//   i32 iOldDataCount    = m_iDataCount;
   ...
   if(m_iDataCount > 0)
   {
      ...
      if(m_pdata != nullptr)
      {
         if(::index(pdataOld) == 1)
         {
            ::aura::bit::set(m_pdata, true, 0, m_iDataCount * 8 * sizeof(i32) - 1);
         }
         else if(pdataOld != nullptr)
         {
            ::aura::bit::int_aligned_copy(m_pdata, 0, pdataOld, 0, iOldBitCount - 1);
            delete [] pdataOld;
         }
         if(iOldBitCount < m_iBitCount)
         {
            ::aura::bit::set(m_pdata, bSet, iOldBitCount, m_iBitCount - 1);
         }
      }
   }
   else
