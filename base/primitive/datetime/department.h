...
CLASS_DECL_AURA i32 SWN(i32 y,i32 m,i32 d);
//where DP ("Days Passed") is given by:
//   DP( y, 1 ) = 0
//  DP( y, m+1 ) = DP( y, m ) + ML( y, m )
CLASS_DECL_AURA i32 DP(i32 y,i32 m);
CLASS_DECL_AURA i32 ML(i32 y,i32 m);
CLASS_DECL_AURA i32 LEAP(i32 y);
CLASS_DECL_AURA i32 dayofweek(i32 y,i32 m,i32 d);	/* 0 = Sunday */
CLASS_DECL_AURA i32 SDOW(i32 y,i32 m,i32 d); // ( 0 = Monday, ..., 6 = Sunday )
CLASS_DECL_AURA i32 DOW(i32 y,i32 m,i32 d);
CLASS_DECL_AURA i32 getDayOfWeek(i32 month,i32 day,i32 year,i32 CalendarSystem);
...
namespace datetime
{
   ...
   public:
      ...
      i64 strtotime(const ::aura::str_context * pcontext,const char * psz,time_t time,i32 iPath,i32 & iPathCount);
      i64 strtotime(const ::aura::str_context * pcontext,const char * psz,i32 iPath,i32 & iPathCount);
      i64 gmt_strtotime(const ::aura::str_context * pcontext,const char * psz,i32 iPath,i32 & iPathCount);

      ::datetime::time from_local(const string & str);
      ::datetime::time from_gmt(const string& str);

      // 1 - domingo
      string get_week_day_str(const aura::str_context * pcontext,i32 iWeekDay);
      // 1 - domingo
      string get_tiny_week_day_str(const aura::str_context * pcontext,i32 iWeekDay);
      string get_month_str(const aura::str_context * pcontext,i32 iMonth);
      string get_short_month_str(const aura::str_context * pcontext,i32 iMonth);
