...
namespace gpu
{
   ...
   void renderer::initialize_gpu_renderer(::gpu::context* pgpucontext)
   {
      ...
      if (pgpuwindowattachment && pgpuwindowattachment->get_frame_count() > 0)
      {

         m_iDefaultFrameCount = pgpuwindowattachment->get_frame_count();
...
///         m_sizeRenderer = size;
   ...
   void renderer::read_to_cpu_buffer21()
   {
...
//#endif
   ...
   void renderer::on_end_frame()
   {
      ...
      //throw todo;
      //auto bCpuBuffer =
      //   m_pgpucontext && m_pgpucontext->m_eoutput == ::gpu::e_output_aaa_cpu_buffer;
   ...
   //   void renderer::to_draw2d_graphics(::draw2d::graphics_pointer & pdraw2dgraphics)
   //   {
   //
   //      
   //   //   m_pgpucontext->prepare_for_gpu_read();
   ...
   void renderer::on_layout(::draw2d::graphics_pointer& pdraw2dgraphics)
   {
   ...
   void renderer::on_begin_render(::gpu::layer * pgpulayer)
   {
      ...
      if (pcommandbuffer->m_estate != command_buffer::e_state_recording)
      {
         ...
         auto pcontext = this->m_pgpucontext;
...
//      }
   ...
   void renderer::layer_start(bool bFirstLayer, enum_start_layer estartlayer)
   {
      ...
      player->m_estartlayer = estartlayer;
   ...
   void renderer::start_layer(bool bFirstLayer, enum_start_layer estartlayer)
   {
...
//      auto escene = m_pgpucontext->m_escene;
      ...
      //if (pgraphicscontext->egraphics() == e_graphics_draw)
      {
         ...
         layer_start(bFirstLayer, estartlayer);
