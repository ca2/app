...
namespace coding
{
   ...
   install_form_handler::install_form_handler()
   {
   ...
   void install_form_handler::update_title()
   {
      ...
      m_pinstall->m_propertyTitle.set_property(
         strCheck + m_pinstall->get_title(),
         ::e_source_sync);
      auto strRemark = m_pinstall->get_remark();
      strRemark.find_replace("$fs_folder_sync",m_papp->m_propertyCloudDataProvider.get_property().as_atom());
      m_pinstall->m_propertyRemark.set_property(
         strRemark,
         ::e_source_sync);

      bool bVisible = !m_pinstall->is_installed(true);

      if (bVisible)
      {

         if(m_pstillStatus) m_pstillStatus->display();
         //if (m_pstillCheckBox)m_pstillCheckBox->display();
         if (m_pcheckbox)
         {
            m_pcheckbox->display();
            select();
      ...
      else
      {
         ...
         if (m_pstillStatus)m_pstillStatus->hide();
         //if (m_pstillCheckBox)m_pstillCheckBox->hide();
         if (m_pcheckbox)
         {
            m_pcheckbox->hide();
            unselect();
      ...
      m_plinelayout->get_parent()->m_bNeedPerformLayout = true;

      m_plinelayout->get_parent()->set_need_layout();

      m_plinelayout->get_parent()->set_need_redraw();

      m_plinelayout->get_parent()->post_redraw();

   }


   void install_form_handler::create_install_form_handler(::coding::code * pcode, ::coding::install * pinstall, ::user::interaction* puserinteractionParent)
   {
      ...
      m_pcheckbox = create_check_box(pline, "install");
      ...
      m_pcheckbox->check_changed(this) += [this](auto & check)
      {
         ...
         if (check.action_context().is_user_source())
         {
      ...
      m_pstillTitle = create_title2(pline, pinstall->m_propertyTitle);

      m_pstillRemark = create_label(pline, pinstall->m_propertyRemark);
   ...
   void install_form_handler::on_finished()
   {

      m_pinstall->m_bInstalling = false;

      auto strTitle = m_pinstall->get_title();
   ...
   void install_form_handler::start_installation(int iTryCount)
   {
   ...
   void install_form_handler::defer_start_installation()
   {
...
//   }
   ...
   void install_form_handler::install_status(::coding::install * pinstall, const ::scoped_string& scopedstr)
   {
   ...
   void install_form_handler::install_status2(::coding::install* pinstall, const ::scoped_string& scopedstr)
   {
   ...
   void install_form_handler::install_append_status2(::coding::install* pinstall, const ::scoped_string& scopedstr)
   {
   ...
   void install_form_handler::on_update_install_status(::coding::install * pinstall)
   {
   ...
   bool install_form_handler::is_selected()
   {
   ...
   void install_form_handler::select()
   {
   ...
   void install_form_handler::unselect()
   {
   ...
   void install_form_handler::hide_selection()
   {
   ...
   void install_form_handler::enable_window(bool bEnable)
   {
   ...
   bool install_form_handler::has_check()
   {
   ...
   bool install_form_handler::has_check(::user::check* pcheck)
   {
   ...
   void install_form_handler::enable_selection()
   {
   ...
   void install_form_handler::disable_selection()
   {
