...
//      prequest->m_usercreatecontext.m_pCurrentDoc = get_document();
  //    prequest->m_usercreatecontext.m_typeNewImpact = ::as_type < tree >();
      ::pointer<::user::interaction>puserinteraction = create_impact(::as_type < ::user::tree >(),get_document(),get_pane_holder(0),100);
...
//      cc->m_usercreatecontext.m_pCurrentDoc = get_document();
  //    cc->m_usercreatecontext.m_typeNewImpact = ::as_type < list_base >();
      puserinteraction = create_impact(::as_type < ::userfs::list >(),get_document(), get_pane_holder(1),101);
