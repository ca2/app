...
namespace ftp
{
   ...
   public:
      ...
      int Passive(::u32& ulIpAddress, unsigned short& ushPort);
      ...
      bool GetIpAddressFromResponse(const ::scoped_string & scopedstrResponse, ::u32& ulIpAddress, unsigned short& ushPort);
...
L <unsigned char i32_size> - Local unsigned char Byte i32_size
</PRE>
...
EBCDIC) or explicitly (as in Local unsigned char) define a unsigned char i32_size for
interpretation which is referred to as the "logical unsigned char size."
Note that this has nothing to do with the unsigned char i32_size used for
transmission over the data connection, called the "transfer
unsigned char i32_size", and the two should not be confused.  For example,
NVT-ASCII has a logical unsigned char i32_size of 8 bits.  If the type is
...
no default value.  The logical unsigned char i32_size is not necessarily
...
transformed in a manner dependent on the logical unsigned char i32_size
...
with a logical unsigned char i32_size of 36.  The receiving host would
