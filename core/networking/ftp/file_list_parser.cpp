...
//#ifndef system()->m_pcoresystem->InsertTime
  
//#define system()->m_pcoresystem->InsertTime(ftpFileStatus) InsertTime(ftpFileStatus);
//#endif
      ...
      for (int j = 1; j < scopedstrLine.size(); ++j)
      {
         ...
         if (scopedstrLine.begin()[j] == (','))
         {
            ...
            case ('m'):
               ...
               system()->m_pcoresystem->InsertTime(ftpFileStatus);
               break;
            case ('i'):
   ...
   bool file_list_parser::ParseMultiNetListing(file_status& ftpFileStatus, const ::scoped_string & scopedstrLine)
   {
      ...
      // name lookup of `i' changed for __new ISO `for' scoping
      int i = 0;
