...
namespace uwp
{
   ...
   CLASS_DECL_CORE DWORD       GetFileAttributes(const char * lpFileName);
   CLASS_DECL_CORE bool        CreateDirectory(const char * lpPathName, LPSECURITY_ATTRIBUTES lpSecurityAttributes);
   CLASS_DECL_CORE DWORD       GetCurrentDirectory(string & str);
   CLASS_DECL_CORE DWORD       GetTempPath(string & str);
   CLASS_DECL_CORE LONG        RegQueryValue(HKEY hkey, const char * lpszSubKey, string & str);
   CLASS_DECL_CORE HICON       ExtractIcon(HINSTANCE hInst, const char * lpszExeFileName, UINT nIconIndex);
   CLASS_DECL_CORE bool        DeleteFile(const char * lpFileName);
   CLASS_DECL_CORE int         GetMenuStringW(HMENU hMenu, UINT uIDItem, string & str, UINT flags);
...
CLASS_DECL_CORE LRESULT CALLBACK __window_procedure(oswindow hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam);
...
CLASS_DECL_CORE LONG delete_registry_tree_helper(HKEY hParentKey, const string & strKeyName);
...
CLASS_DECL_CORE void __end_thread(::object * pobject, UINT nExitCode, bool bDelete = TRUE);
