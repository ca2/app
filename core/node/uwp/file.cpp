...
//            m_psystem->m_spfilesystem.m_p->FullPath(m_path, m_path);
   ...
   string CLASS_DECL_CORE vfxStringFromCLSID(REFCLSID rclsid)
   {
   ...
   bool CLASS_DECL_CORE vfxGetInProcServer(const char * lpszCLSID, string & str)
   {
   ...
   bool CLASS_DECL_CORE vfxFullPath(unichar * lpszPathOut, const unichar * lpszFileIn)
   ...
   void CLASS_DECL_CORE vfxGetModuleShortFileName(HINSTANCE hInst, string& strShortName)
   {
...
#define _wcsinc(_pc)    ((_pc)+1)
   ...
   void CLASS_DECL_CORE vfxGetRoot(const unichar * lpszPath, string& strRoot)
   {
      ASSERT(lpszPath != nullptr);
      wstring wstrRoot;
      // determine the root name of the volume
...
#define EDEADLOCK       EDEADLK
  ...
  /*    ::file::file_status status;
   ...
   //UINT CLASS_DECL_CORE vfxGetFileName(const unichar * lpszPathName, unichar * lpszTitle, UINT nMax)
   //{
   ...
   //void CLASS_DECL_CORE throw_exception(__pointer(::aura::application) papp, int cause, LONG lOsError, const char * lpszFileName /* == nullptr */)
   //{
...
bool CLASS_DECL_CORE vfxResolveShortcut(string & strTarget, const char * pszSource, ::user::primitive * puiMessageParentOptional)
{
