...
namespace account
{
   ...
   public:
      ...
      virtual void client_to_screen(POINT32 * ppt);
...
//         ::get_window_rect(::get_desktop_window(), &m_rectangleDesktop);
   ...
   bool account::prepare_window(bool bShow)
   {
      ...
      rectangle & rectangleDesktop = m_rectangleDesktop;
...
//      SetTimer(m_oswindow, 123, 23, nullptr);
   ...
   void account::client_to_screen(POINT32 * ppt)
   {
      ::client_to_screen(m_oswindow, ppt);
   }

   void account::screen_to_client(POINT32 * ppt)
   {
