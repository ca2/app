//  Created by Camilo Sasuke Tsumanuma on 05/01/18.
#pragma once

//CLASS_DECL_CORE string get_command_line();
//
//#if defined(MACOS)
//
//#include "aura/os/ansios/_.h"
//#include "aura/os/apple/_.h"
//#include "aura/os/macos/_.h"
//
//#elif defined(APPLE_IOS)
//
//#include "aura/os/ansios/_.h"
//#include "aura/os/apple/_.h"
//#include "aura/os/ios/_.h"
//
//#elif defined(LINUX)
//
//#include "aura/os/cairo/_.h"
//#include "aura/os/ansios/_.h"
//#include "aura/os/linux/_.h"
//
//#elif defined(_UWP)
//
//#include "aura/os/ansios/_.h"
//#include "aura/os/windows_common/_.h"
//#include "aura/os/universal_windows/_.h"
//
//#elif defined(WINDOWS_DESKTOP)
//
//#include "aura/os/ansios/_.h"
//#include "aura/os/windows_common/_.h"
//#include "aura/os/windows/_.h"
//
//#elif defined(ANDROID)
//
//#include "aura/os/ansios/_.h"
//#include "aura/os/android/_.h"
//
//#elif defined(SOLARIS)
//
//#include "aura/os/ansios/_.h"
//#include "aura/os/solaris/_.h"
//
//#else
//
//#error "The operating system wasn't expected."
//
//#endif
//
//template < typename PRED >
//inline bool predicate_Sleep(int iTime, PRED pred);
//
//
////void adapt_font_name(string & str);
//
//
////namespace str
////{
////
////   //string CLASS_DECL_CORE get_window_text_timeout(oswindow oswindow, ::duration tickTimeout = 1000);
////
////} // namespace str
////
//
//CLASS_DECL_CORE void defer_dock_application(int_bool bDock);
//
//
//CLASS_DECL_CORE string get_current_login_name();
//
//
//#if defined(WINDOWS)
//
//
//CLASS_DECL_CORE _locale_t get_c_locale();
//
//
//#else
//
//
//CLASS_DECL_CORE locale_t get_c_locale();
//
//#endif
//
//

