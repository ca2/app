...
namespace usernet
{
   ...
   void network_configuration::on_show()
   {
      ...
      if(doc.load(pcontext->m_pcontext->file().as_string(pcontext->m_pcontext->dir().appdata()/"proxy.xml")))
      {
      ...
      if(pevent->m_eevent == ::user::e_event_button_clicked)
      {
         ...
         if(pevent->m_puie->m_id == "submit")
         {
            ...
            if(strServer.get_length() == 0)
            {
               ...
               pcontext->m_pcontext->file().del(pcontext->m_pcontext->dir().appdata()/ "proxy.xml");
            ...
            else
            {
               ...
               pcontext->m_pcontext->file().put_contents(pcontext->m_pcontext->dir().appdata()/"proxy.xml", doc.get_xml());
