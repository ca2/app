...
namespace user
{
   ...
   void draw_list_subitem::draw_image(::draw2d::graphics_pointer & pdraw2dgraphics)
   {
      ...
      else
      {
            ...
            || (m_pitem->m_pmesh->m_plist->m_dIconOpacity < 1.0))
         {
            ...
            m_pitem->m_pmesh->defer_constructø(pimage);
            ...
            if (pimage.nok() && m_iImage >= 0 && m_iImage < get_image_list()->get_image_count())
            {
               ...
               if (m_pitem->m_pmesh->m_plist->m_iIconBlur > 0 && m_pitem->m_pmesh->m_plist->m_iIconBlurRadius > 0)
               {
                  ...
                  pimage->create_as_descriptor(m_rectangleImage.size() + i32_size(m_pitem->m_pmesh->m_plist->m_iIconBlurRadius * iRate * 2, m_pitem->m_pmesh->m_plist->m_iIconBlurRadius * iRate * 2), m_pitem->m_pmesh->draw2d_domain());
               ...
               else
               {
                  ...
                  pimage->create_as_descriptor(m_rectangleImage.size(), m_pitem->m_pmesh->draw2d_domain());
            ...
            if (pimage.ok())
            {
               ...
               ::f64_rectangle rectangleDib(m_rectangleImage.top_left() - i32_size(m_pitem->m_pmesh->m_plist->m_iIconBlurRadius * iRate, m_pitem->m_pmesh->m_plist->m_iIconBlurRadius * iRate),
         ...
         else
         {
            ...
            pimagelist->_draw(m_pitem->m_pdrawlistitem->m_pgraphics, (::i32)m_iImage, m_rectangleImage.top_left(), m_rectangleImage.size(), ::i32_point(), 0);
   ...
   void draw_list_group::draw_group_image(::draw2d::graphics_pointer & pdraw2dgraphics)
   {
      ...
      m_pmesh->m_plist->m_pimagelistGroup->draw(m_pgraphics, (::i32)m_iImage, m_rectangleImage.top_left(), m_rectangleImage.size(), ::i32_point(), 0);
      ...
      if (m_bOk)
      {
         ...
         if (m_pitem->m_pmesh->m_plist->m_eview == list::impact_icon && m_pitem->m_pmesh->m_plist->m_bEmboss && ((m_pitem->m_pmesh->m_plist->m_iTextBlurRadius > 0 && m_pitem->m_pmesh->m_plist->m_iTextBlur > 0) || m_pitem->m_pmesh->m_plist->m_iTextSpreadRadius > 0))
         {
            ...
            if (m_strText.has_character())
            {
               ...
               m_pitem->m_pmesh->m_plist->defer_constructø(pimage2);

               auto psystem = m_pitem->m_pmesh->m_plist->system();

               auto pdraw2d = psystem->draw2d();

               m_pitem->m_pmesh->m_plist->defer_construct_newø(m_pitem->m_pmesh->m_plist->m_pfastblurIconText);
