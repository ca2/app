...
namespace userex
{
   ...
   void color_view::install_message_routing(::channel * pchannel)
   {
      ...
      m_pimageTemplate = pcontext->m_pauracontext->create_image({2048,  2048});

      ::visual::colors_with_shades_of_grey(m_pimageTemplate);

      m_pimageLuminance = pcontext->m_pauracontext->create_image({100,  100});
   ...
   void color_view::on_layout(::draw2d::graphics_pointer & pgraphics)
   {
      ...
      m_pimage = pcontext->m_pauracontext->create_image({m_rectangleColors.width() / 2,  m_rectangleColors.height()});
      ...
      m_pimageLuminance = pcontext->m_pauracontext->create_image({m_rectangleColors.width() / 8,  m_rectangleColors.height()});
