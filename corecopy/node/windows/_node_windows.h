...
namespace windows
{
   ...
   CLASS_DECL_CORE DWORD       get_file_attributes(const char * pFileName);

   CLASS_DECL_CORE DWORD       get_current_directory(string & str);
   CLASS_DECL_CORE DWORD       get_temp_path(string & str);
   CLASS_DECL_CORE LONG        reg_query_value(HKEY hkey,const char * pszSubKey,string & str);

   CLASS_DECL_CORE HICON       extract_icon(HINSTANCE hInst,const char * pszExeFileName,UINT nIconIndex);

   CLASS_DECL_CORE bool        delete_file(const char * pFileName);

   CLASS_DECL_CORE i32     get_menu_string(HMENU hMenu,UINT uIDItem,string & str,UINT flags);
...
CLASS_DECL_CORE HINSTANCE __get_resource_handle();
CLASS_DECL_CORE HINSTANCE __find_string_resource_handle(UINT nID);
