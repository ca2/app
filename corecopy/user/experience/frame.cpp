...
//         m_bFirstLayoutDone = true;
   ...
   void frame::title_bar_layout()
   {
      ...
      try
      {
         ...
         if (m_pframewindow->window_is_full_screen())
         {
            ...
            if (pointCursor.y <= get_control_box()->origin().y)
            {
         ...
         else if (m_pframewindow->is_this_screen_visible())
         {
      ...
      if (!rectClient.is_empty())
      {
         ...
         if (m_pframewindow->is_this_screen_visible() && !is_iconic(m_pframewindow->window_display()))
         {

            auto rectControlBoxWindow = get_control_box()->parent_client_rect();
   ...
   void frame::on_defer_display()
   {
      ...
      if (m_pframewindow->is_this_screen_visible())
      {
   ...
   void frame::defer_frame_placement_snapping()
   {
         ...
         (::is_docking_appearance(edisplay) && iWkspace != m_pframewindow->m_windowrect.m_iWkspace))
      {
         ...
         m_pframewindow->set_display_request(edisplay);
