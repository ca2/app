...
namespace user
{
   ...
   auto pred_list_compare_key = [](auto& p1, auto& p2)
   {
   ...
   void list::_001OnDraw(::draw2d::graphics_pointer & pgraphics)
   {
      ...
      sync_lock sl(m_pmeshdata->mutex());
   ...
   //   m_crSmallMask = pcolumn->m_crSmallMask;
   ...
   void list::_001OnColumnChange()
   {
      ...
      if (iFontHeight > iItemHeight)
      {
         ...
         iItemHeight = (decltype(iItemHeight)) MAX(iFontHeight, iItemHeight);
...
//      m_iVScrollOffset = m_iItemHeight;
   ...
   bool list::_001HitTest_(const ::point & point, index &iItem, index&iSubItem)
   {
   ...
   bool list::_001HitTest_(const ::point & point, index & iItem)
   {
   ...
   bool list::_001DisplayHitTest(const ::point & point, index&iItemParam, index&iSubItemParam)
   {
   ...
   bool list::_001DisplayHitTest(const ::point & point, index&iItemParam)
   {
      ...
      else if (m_eview == impact_icon)
      {
         ...
         if (m_bDrag && m_iItemLButtonDown >= 0)
         {
            ...
            if (pdrawitem->m_iItem == m_iItemLButtonDown || m_rangeSelection.has_item(pdrawitem->m_iDisplayItem))
            {
               ...
               ::point point = m_pointLButtonUp;

               ::point point2 = m_pointLButtonDown1;
               ...
               if (pdrawitem->m_pcolumn->m_bIcon)
               {
                  ...
                  if (pdrawitem->m_bOk && pdrawitem->m_iImage >= 0)
                  {
                     ...
                     if (eelement == ::user::list::element_image)
                     {
                        ...
                        ::rect rectAlign(pdrawitem->m_rectSubItem);
         ...
         else
         {
            ...
            if (m_bMultiSelect && Session.is_key_pressed(::user::key_shift))
            {
               ...
               if (_001DisplayHitTest(point, iDisplayItem))
               {
                  ...
                  auto iLItem = MIN(m_iShiftFirstSelection, iDisplayItem);

                  auto iUItem = MAX(m_iShiftFirstSelection, iDisplayItem);
   ...
   void list_column_array::OnChange()
   {
      ...
      pred_sort(pred_list_compare_key);
//      sort::array::quick_sort(*this, list_column::CompareKey);
      ...
      pred_sort(list_column::less_SectEndNonVisible);
...
//      }
   ...
   void list::_001EnsureVisible(index iItem, e_align ealign, bool bRedraw)
   {
             ...
             iItem >= pointOffset.y / m_iItemHeight + m_nDisplayCount))
      {
         ...
         pointOffset.y = (LONG)(iItem * m_iItemHeight);
   ...
   void list::_001EnsureVisible(index iItem, range & range)
   {
      ...
      if (pointOffset.y / MAX(1, m_iItemHeight) != iyScroll)
      {
         ...
         pointOffset.y = (LONG)(iyScroll * m_iItemHeight);
      ...
      if (m_iTopDisplayIndex < 0 && m_iItemHeight > 0 && m_eview == impact_report && point.y != 0 && m_nDisplayCount > 0)
      {
         ...
         if (scroll_x::m_pscrollbarHorz != nullptr)
         {
            ...
            if (point.y > (_001GetItemCount() - m_nDisplayCount) * m_iItemHeight + (m_bHeaderCtrl ? m_iItemHeight : 0))
            {
               ...
               point.y = (LONG) ((_001GetItemCount() - m_nDisplayCount) * m_iItemHeight + (m_bHeaderCtrl ? m_iItemHeight : 0));
   ...
   bool draw_list_item::draw_image()
   {
      ...
      else
      {
         ...
         sync_lock sl(get_image_list()->mutex());
            ...
            || (m_plist->m_dIconOpacity < 1.0))
         {
            ...
            if (pimage.nok() && m_iImage >= 0 && m_iImage < get_image_list()->get_image_count())
            {
               ...
               if (m_plist->m_iIconBlur > 0 && m_plist->m_iIconBlurRadius > 0)
               {
                  ...
                  pimage->create(m_rectImage.size() +size(m_plist->m_iIconBlurRadius * iRate * 2, m_plist->m_iIconBlurRadius * iRate * 2));
                  ...
                  && m_iImage >= 0 && m_iImage < get_image_list()->get_image_count()   )
            {
               ...
               m_plist->m_pimageTime = __create_image(m_plist->m_pimageSpot->size());

               ::rect rect = rectI;

               rect.offset(-m_rectImage.top_left());
               ...
               get_image_list()->draw(m_plist->m_pimageTime->get_graphics(), (i32)m_iImage,
                                      rect2.top_left(),
                                      rect.size(), rect.top_left(), 0);

               //m_pgraphics->draw(rectI + point(200,200), m_plist->m_pimageTime->get_graphics(), rect2.top_left());
         ...
         {
            m_pgraphics->set_alpha_mode(::draw2d::alpha_mode_blend);
            return get_image_list()->draw(m_pgraphics, (i32)m_iImage, m_rectImage.top_left(), m_rectImage.size(), ::point(), 0);
         }
      }
      return false;
   }
   ...
   {
      return m_plist->m_pilGroup->draw(m_pgraphics, (i32)m_iImage, m_rectImage.top_left(), m_rectImage.size(), ::point(), 0);
   }
...
//      }
   ...
   //bool list::get_client_rect(RECT * prect)

   //{

      //mesh::get_client_rect(prect);
