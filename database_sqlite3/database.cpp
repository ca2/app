...
namespace sqlite
{
   ...
   //   return allocateø class dataset (this);
...
int database_sqlite3_sqlite_callback(void * res_ptr,int ncol, char** reslt,char** cols)
{
   ...
   presultset->defer_construct_newø(pfielda);

   presultset->defer_construct_newø(prowa);
   ...
   if (pfielda->is_empty())
   {
      ...
      for (::collection::index i = 0; i < ncol; i++)
      {
         ...
         presultset->defer_construct_newø(pfielda->element_at(i));
...
//         }
   ...
   if (reslt != nullptr)
   {
      ...
      auto prow = allocateø ::database::row();
