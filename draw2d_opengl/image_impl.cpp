...
namespace draw2d_opengl
{
   ...
   //void image::ToAlphaAndFill(i32 i, COLORREF cr)
   //{
   //   BYTE *dst=(BYTE*)m_pcolorref;
   //   i64 size = area();
   ...
   //void image__copy(image image)
   //{
   //   // If DibSize Wrong Re-create image
   //   if (pimage->get_size() != m_size)
   //      image = create_image (m_size);
   //   // do copy
   //   ::memcpy_dup ( pimage->get_data(), m_pcolorref, (size_t) area() * sizeof(COLORREF) );
   //}
   ...
   //   // do Paste
   //   ::memcpy_dup ( m_pcolorref, pimage->get_data(), (size_t) area() * sizeof(COLORREF) );
   //}
   ...
   //   // Prepare buffer Addresses
   //   COLORREF *src=pimage->get_data()+((py-y)*pimage->width())+px-x;
...
//         return false;
//
//      m_pgraphics->attach((HDC) pbase->m_wparam);
//
//      ::rect rectx;
//
...
//         m_pgraphics->SelectClipRgn(nullptr);
//         m_pgraphics-> SetViewportOrg(::point());
...
#define new ACME_NEW
   ...
   bool image::_unmap()
   {
      ...
      glDrawPixels(m_size.cx, m_size.cy, GL_BGRA, GL_UNSIGNED_BYTE, m_pcolorrefRaw);
...
/*      if (!m_pgraphics)
      ...
      if (::is_null(ppixmap->m_pcolorrefRaw))
      {
      ...
      glReadPixels(0, 0, ppixmap->m_size.cx, ppixmap.m_size.cy, GL_BGRA, GL_UNSIGNED_BYTE, ppixmap.m_pcolorref1);
