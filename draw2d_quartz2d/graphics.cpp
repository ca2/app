...
//   void graphics::DPtoLP(LPPOINT lpPoints, i32 nCount)
//   {
...
//   void graphics::LPtoDP(LPPOINT lpPoints, i32 nCount)
//   {
...
//   }
   ...
   bool graphics::PatBlt(i32 x, i32 y, i32 nWidth, i32 nHeight, DWORD dwRop)
   {
   ...
   bool graphics::BitBltRaw(i32 x, i32 y, i32 nWidth, i32 nHeight, ::draw2d::graphics * pgraphicsSrc, i32 xSrc, i32 ySrc, DWORD dwRop)
   {
   ...
   bool graphics::StretchBltRaw(double xDst, double yDst, double nDstWidth, double nDstHeight, ::draw2d::graphics * pgraphicsSrc, i32 xSrc, i32 ySrc, i32 nSrcWidth, i32 nSrcHeight, DWORD dwRop)
   {
...
//               return BitBlt(point((i64) x, (i64) y), rectText.size(), pimage1->get_graphics(), ::point());
   ...
   bool graphics::ExtTextOut(i32 x, i32 y, UINT nOptions, const ::rectd & rect, const char * lpszString, UINT nCount, int * lpDxWidths)
   {
   ...
   bool graphics::ExtTextOut(i32 x, i32 y, UINT nOptions, const ::rect & rect, const string & str, LPINT lpDxWidths)
   {
      ...
      //      ASSERT(get_handle1() != nullptr);
      //      return ::ExtTextOut(get_handle1(), x, y, nOptions, rect, str, (UINT)str.get_length(), lpDxWidths) != FALSE;
...
//
//      pmetric->tmAveCharWidth        = (LONG) (width * (m_pfont.is_null() ? 1.0 : m_pfont->m_dFontWidth) / (double) str.get_length());
   ...
   /*
    ...
    DWORD graphics::GetCharacterPlacement(const char * lpString, i32 nCount, i32 nMaxExtent, LPGCP_RESULTS lpResults, DWORD dwFlags)
    {
    ...
    DWORD graphics::GetCharacterPlacement(string & str, i32 nMaxExtent, LPGCP_RESULTS lpResults, DWORD dwFlags)
    {
   ...
   bool graphics::ScrollDC(i32 dx, i32 dy, const ::rect & rectScroll, const ::rect & rectClip, ::draw2d::region* pRgnUpdate, LPRECT rectUpdate)
   {
   ...
   bool graphics::MaskBlt(i32 x, i32 y, i32 nWidth, i32 nHeight, ::draw2d::graphics * pgraphicsSrc, i32 xSrc, i32 ySrc, ::draw2d::bitmap& maskBitmap, i32 xMask, i32 yMask, DWORD dwRop)
   {
   ...
   bool graphics::PlgBlt(LPPOINT lpPoint, ::draw2d::graphics * pgraphicsSrc, i32 xSrc, i32 ySrc, i32 nWidth, i32 nHeight, ::draw2d::bitmap& maskBitmap, i32 xMask, i32 yMask)
   {
   ...
   bool graphics::PolyPolyline(const POINT* lpPoints, const DWORD* lpPolyPoints, i32 nCount)
   {
   ...
   bool graphics::PolyBezier(const POINT* lpPoints, i32 nCount)
   {
   ...
   i32 graphics::GetPath(LPPOINT lpPoints, LPBYTE lpTypes, i32 nCount)
   {
...
//       __is_valid_address(rectLast, sizeof(RECT), FALSE));
//
...
//   }
   ...
   void graphics::draw3d_rect(const ::rectd & rect, COLORREF clrTopLeft, COLORREF clrBottomRight, eborder eborder)
   {
      LONG x = rect.left;
      LONG y = rect.top;
      LONG cx = width(rect);
...
//      rect.size.height  = rect.bottom - rect.top;
   ...
   /*
    ...
    ::_001ScreenToClient(get_handle1(), (LPPOINT)rect);
    ::_001ScreenToClient(get_handle1(), ((LPPOINT)rect)+1);
    ...
    {
    ASSERT(::IsWindow(get_handle1()));
    ::_001ClientToScreen(get_handle1(), (LPPOINT)rect);
    ::_001ClientToScreen(get_handle1(), ((LPPOINT)rect)+1);
    if (GetExStyle() & WS_EX_LAYOUTRTL)
    rect::swap_left_right(rect);
    }*/
...
//   }
   ...
   bool graphics::PolyDraw(const POINT* lpPoints, const BYTE* lpTypes, i32 nCount)
   {
   ...
   bool graphics::PolylineTo(const POINT* lpPoints, i32 nCount)
   {
   ...
   bool graphics::PolyBezierTo(const POINT* lpPoints, i32 nCount)
   {
...
//      cairo_set_source_rgba(m_pdc, colorref_get_r_value(pbrush->m_color) / 255.0, colorref_get_g_value(pbrush->m_color) / 255.0, colorref_get_b_value(pbrush->m_color) / 255.0, colorref_get_a_value(pbrush->m_color) / 255.0);
      ...
      CGContextSetRGBFillColor(m_pdc, pbrush->m_color.m_iR / 255.0, pbrush->m_color.m_iG / 255.0, pbrush->m_color.m_iB / 255.0, pbrush->m_color.m_iA / 255.0);
   ...
   bool graphics::set(const ::draw2d::pen * ppen)
   {
      ...
      if(ppen->m_etype == ::draw2d::pen::type_brush && ppen->m_pbrush)
      {
         ...
         if(ppen->m_pbrush->m_etype == ::draw2d::brush::type_solid)
         {
            ...
            CGContextSetRGBStrokeColor(m_pdc, ppen->m_pbrush->m_color.m_iR / 255.0, ppen->m_pbrush->m_color.m_iG / 255.0, ppen->m_pbrush->m_color.m_iB / 255.0, ppen->m_pbrush->m_color.m_iA / 255.0);
      ...
      else
      {
         ...
         CGContextSetRGBStrokeColor(m_pdc, ppen->m_color.m_iR / 255.0, ppen->m_color.m_iG / 255.0, ppen->m_color.m_iB / 255.0, ppen->m_color.m_iA / 255.0);
   ...
   bool graphics::set_pen(const ::draw2d::brush * pbrush, double dWidth)
   {
      ...
      CGContextSetRGBStrokeColor(m_pdc, pbrush->m_color.m_iR / 255.0, pbrush->m_color.m_iG / 255.0, pbrush->m_color.m_iB / 255.0, pbrush->m_color.m_iA / 255.0);
...
//      cairo_move_to(m_pdc, p.m_x, p.m_y);
   ...
   bool graphics::draw_inline(const ::draw2d_quartz2d::path::string_path & stringpath, ::draw2d::pen * ppen)
   {
      ...
      for(auto str : stra)
      {
         ...
         internal_show_text(stringpath.m_x + dx,stringpath.m_y + dy + offsety, 0, DT_TOPLEFT, str, kCGTextStroke, true, &ascent, &descent, &leading, &width, ppen, nullptr, stringpath.m_pfont);
...
//         }
   ...
   bool graphics::fill_inline(const ::draw2d_quartz2d::path::string_path & stringpath, ::draw2d::brush * pbrush)
   {
      ...
      for(auto str : stra)
      {
         ...
         internal_show_text(stringpath.m_x + dx, stringpath.m_y + dy + offsety, 0, DT_TOPLEFT,  str, kCGTextFill, true, &ascent, &descent, &leading, &width, nullptr, pbrush, stringpath.m_pfont);
...
//         }
   ...
   void graphics::internal_set_fill_color(COLORREF cr)
   {
      ...
      CGContextSetRGBFillColor(m_pdc, colorref_get_r_value(cr) / 255.0f, colorref_get_g_value(cr) / 255.0f, colorref_get_b_value(cr) / 255.0f, colorref_get_a_value(cr) / 255.0f);
   ...
   bool graphics::internal_show_text(::draw2d::font_pointer spfont,::draw2d::brush_pointer spbrush,::draw2d::pen_pointer sppen, double x, double y, double wAlign, UINT nFormat, const string & str, CGTextDrawingMode emode, bool bDraw, CGFloat * pascent, CGFloat * pdescent, CGFloat * pleading, CGFloat * pwidth)
   {
      ...
      if(bDraw)
      {
         ...
         if(emode == kCGTextStroke|| emode == kCGTextFillStroke)
         {
            ...
            crStroke = sppen.is_null() ? ARGB(255, 0, 0, 0) : sppen->m_color.operator COLORREF();
