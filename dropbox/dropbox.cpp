...
namespace dropbox
{
   ...
   ::dropbox::dropbox * dropbox::get(::particle * pparticle)
   {

      ::dropbox::dropbox * pdropbox = nullptr;

      {

         _synchronous_lock synchronouslock(pparticle->system()->synchronization());

         pdropbox = pparticle->system()->payload("dropbox").cast < ::dropbox::dropbox >();
      ...
      auto pdropboxNew = pparticle->create_newø<::dropbox::dropbox>();

      pparticle->system()->payload("dropbox") = pdropboxNew;

         pdropbox = pdropboxNew;

      }

      pdropbox->initialize_dropbox(pparticle);
   ...
   void dropbox::initialize_dropbox(::particle * pparticle)
   {
