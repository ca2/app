...
/*               auto estyle = translate_style(m_strStyle);
   ...
   void frame::Glass(::draw2d::graphics_pointer & pgraphics, const ::rectangle_i32 & rectangleParam)
   {
      ...
      pimage1 = pcontext->m_pauracontext->create_image({rectangleClient.width() + iInflate * 2,  rectangleClient.height() + iInflate * 2});

      pimage2 = pcontext->m_pauracontext->create_image({rectangleClient.width() + iInflate * 2,  rectangleClient.height() + iInflate * 2});
