#include "framework.h" 
#include "core/user/simple/scroll_bar.h" 
...
namespace experience
{
   ...
   bool theme::_001DrawSimpleScrollBar(::draw2d::graphics_pointer & pgraphics, ::user::scroll_bar * pscrollbar)
   {
      ...
      COLORREF crTrack = pbar->scrollbar_color(this, ::user::element_scrollbar_rect);

      pbar->m_brushDraw->create_solid(crTrack);

      pgraphics->set(pbar->m_penDraw);
      ...
      if (pbar->m_bTracking || pbar->is_true("tracking_on"))
      {
         ...
         byte uchAlpha = MAX(0, MIN(255, pbar->find_u32("tracking_alpha")));
