...
inline floating_matrix4 aiMatrix4x4ToColumnMajorScalar(const aiMatrix4x4 &m)
{
   ...
   // [ d1  d2  d3  d4 ]
   //
   // floating_matrix4 is column-major (OpenGL style):
   //
...
inline float_matrix4 aiMatrix4x4ToColumnMajorSSE(const aiMatrix4x4 &from)
{
   ...
   _mm_storeu_ps(&to.fa[0], col0);
   _mm_storeu_ps(&to.fa[4], col1);
   _mm_storeu_ps(&to.fa[8], col2);
   _mm_storeu_ps(&to.fa[12], col3);
