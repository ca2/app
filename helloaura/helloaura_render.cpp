...
namespace helloaura
{
   ...
   string render::get_helloaura()
   {
      ...
      return Application.get_helloaura();
   ...
   void render::full_render()
   {
      ...
      size sizeNew = m_rectClient.size();
...
//      double dFps = m_pview->get_wnd()->m_pimpl.cast < ::user::interaction_impl >()->m_dFps;
      ...
      double dFps = Application.m_dFps;
...
/*                  m_pimage->g()->text_out((m_cxCache1 - size->cx) / 2, (m_cyCache1 - size->cy) / 2, strHelloAura);
...
/*                  m_pimageTemplate->channel_copy(::color::channel_alpha, ::color::channel_green, m_pimage);
...

      {
         ...
         if (strHelloAura != get_helloaura() || m_cxCache1 != m_cxTarget || m_cyCache1 != m_cyTarget || m_pimageTemplate->area() <= 0)
            return;
...
//      else if (m_bAlternate)
      ...
      System.imaging().bitmap_blend(pgraphics,
                                         point((m_rectClient.width() - m_pimageTemplate2->width()) / 2,
                                               (m_rectClient.height() - m_pimageTemplate2->height()) / 2)
...
/*            m_pimage->g()->text_out((m_rectClient.width() - size.cx) / 2, (m_rectClient->height() - size->cy) / 2, strHelloAura);

            if (m_dMinRadius > 3.0)
            {

/*               m_pimage->blur(m_rectClient->width(), m_rectClient->height());
...
//         sync_lock sl1(m_pview->get_wnd()->mutex());
         ...
         if (m_bFast || m_pimageFast->is_null())
         {
...

            {
               ...
               helloaura_fast_render(get_helloaura());
         ...
         pgraphics->BitBlt(::point(), size(m_rectClient.width(), m_rectClient.height()), m_pimageFast->get_graphics());
...
/*         m_pimageFast = create_image(sizeNew);
      ...
      font->create_pixel_font(m_strFont, fHeight, FW_BOLD);
      ...
      font->create_pixel_font(m_strFont, MIN(m_rectClient.height() * ratey, m_rectClient.width() * size.cy * ratey / size.cx), FW_BOLD);
