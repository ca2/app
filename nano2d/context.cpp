...
void TransformSkewX(float* t, float a)
   ...
   void context::move_to(const ::float_point & p)
   {
      //float vals[] = { ::nano2d::e_command_move_to, x, y };
      //context::__appendCommands)(ctx, vals, NANO2D_COUNTOF(vals));
      //move_to(x, y);
   ...
   void context::line_to(const ::float_point & p)
   {
      /*float vals[] = { ::nano2d::e_command_line_to, x, y };
      context::__appendCommands)(ctx, vals, NANO2D_COUNTOF(vals));*/
	...
	//}
	//
	::float_rectangle context::text(float x, float y, const ::scoped_string& scopedstr)
	{
...
//         }
         ...
         else
         {
            ...
            if (rowStart == NULL)
            {
               ...
               if (type == NANO2D_CHAR || type == NANO2D_CJK_CHAR)
               {
                  ...
                  rowEnd = next;
                  rowWidth = (float) (right - left);
                  //rowMinX = q.x0 - rowStartX;
                  //rowMaxX = q.x1 - rowStartX;
						rowMinX = (float) (left - rowStartX);
	...
	float context::text_bounds(float x, float y, const ::scoped_string& string, ::float_rectangle * bounds)
	{
	...
	void context::text_box_bounds(float x, float y, ::nano2d::text_box * ptextbox, ::float_rectangle * bounds)
	{
		...
		//int nrows = 0, i;
		auto oldAlign = m_pstate->m_ealignText;
		int haling = m_pstate->m_ealignText & (::e_align_left | ::e_align_horizontal_center | ::e_align_right);
		int valign = m_pstate->m_ealignText & (::e_align_top | ::e_align_vertical_center | ::e_align_bottom | ::e_align_baseline);
...
//		rmaxy *= invscale;
	...
	::collection::count context::character_metric(::double_array& daLeft, ::double_array& daRight, const ::scoped_string & scopedstr, character_count iStart, character_count iEnd)
	{
