...
#include "framework.h"
#include "Widget.h"
#include "nanovg/NVGcontext.h"
#include "nanovg/draw2d_NVGcontext.h"
...
::size_i32 Widget::preferred_size(::draw2d::graphics_pointer & pgraphics)
{
   ...
   ::draw2d::NVGcontext nvgcontext(pgraphics);

   const Widget * pthisConst = this;

   auto v2 = pthisConst->preferred_size(& nvgcontext);
...
void Widget::perform_layout(::draw2d::graphics_pointer & pgraphics)
{
   ...
   auto v2 = preferred_size(&nvgcontext);

   ::size_i32 size(v2.v[0], v2.v[1]);

   m_puserinteraction->set_size(size);

   perform_layout(&nvgcontext);
