...
namespace nanoui
{
      ...
      PopupButton(parent, "")
   {
      ...
      m_ppopup->set_layout(allocateø GroupLayout());
      ...
      m_color_wheel = allocateø ColorWheel(m_ppopup, hls);

      m_ptab = allocateø TabWidget(m_ppopup);

      m_ptab->set_erase_children(false);

      m_pgridRgb = allocateø MyGridWidget(m_ptab, 2);
      ...
      m_pgridHSV = allocateø MyGridWidget(m_ptab, 2);
      ...
      m_pgridHEX = allocateø MyGridWidget(m_ptab, 2);

      {

         allocateø nanoui::Label(m_pgridHEX, "#", "sans", 14);

         auto pedit = allocateø  nanoui::TextBox(m_pgridHEX);
         pedit->set_editable(true);
         pedit->set_font_size(14);
         pedit->set_fixed_size({ 70,25 });
      ...
      m_pick_button = allocateø Button(m_ppopup, "Pick");
      m_pick_button->set_background_color(hls);
      m_pick_button->set_text_color(color.contrasting_color());
      m_pick_button->set_fixed_size({100, 20});

      // set the reset button to the specified color
      m_reset_button = allocateø Button(m_ppopup, "Reset");
