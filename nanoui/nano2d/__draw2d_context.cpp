...
namespace nano2d
{
   ...
   void draw2d_context::_create_new_state()
   {
      ...
      m_pstate = allocateø state(m_pgraphics);

      m_pstate->initialize(m_pgraphics);

      m_pgraphics->constructø(m_pstate->m_ppen);
      m_pgraphics->constructø(m_pstate->m_pbrush);
   ...
   void draw2d_context::save()
   {
      ...
      *stateNew.m_pbrush = *stateOld.m_pbrush;
      ...
      if (stateOld.m_ppath)
      {
         ...
         m_pgraphics->constructø(stateNew.m_ppath);
   ...
   void draw2d_context::begin_path()
   {
      ...
      m_pgraphics->constructø(m_pstate->m_ppath);
