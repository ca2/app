...
extern CLASS_DECL_NANOUI const_char_pointer  type_name(VariableType type);
...
namespace detail {
   ...
   template <typename T> using is_nanoui_array_det = std::enable_if_t<T::IsNanoGUI && !T::IsMatrix>;
   template <typename T> using is_nanoui_matrix_det = std::enable_if_t<T::IsNanoGUI && T::IsMatrix>;
}
...
template <typename T>
constexpr bool is_nanoui_array_v = is_detected_v<detail::is_nanoui_array_det, std::decay_t<T>>;

template <typename T>
constexpr bool is_nanoui_matrix_v = is_detected_v<detail::is_nanoui_matrix_det, std::decay_t<T>>;
