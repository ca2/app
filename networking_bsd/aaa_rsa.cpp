...
namespace crypto_openssl
{
   ...
   int rsa::public_encrypt(memory& out, const memory& in, string& strError)
   {
      ...
      int i = RSA_public_encrypt((int)in.get_size(), (const uchar*)(const char*)in.get_data(), out.get_data(), m_prsa, RSA_PKCS1_PADDING);
   ...
   int rsa::public_decrypt(memory& out, const memory& in, string& strError)
   {
      ...
      ::collection::count i = EVP_PKEY_decrypt(pctx, nullptr, &out_len, in.get_data(), (int)in.get_size());
