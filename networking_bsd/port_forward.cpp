...
namespace net
{
   ...
   // message is always the same: a unsigned int named UWM_PORT_FORWARD_ENGINE_THREAD_NOTIFICATION.
   ...
   extern const unsigned int UWM_PORT_FORWARD_ENGINE_THREAD_NOTIFICATION = ::RegisterWindowMessage(
   ...
   // choose a different interaction_impl, such as your CImpact-derived interaction_impl for SDI applications
   //
   // The interaction_impl that you choose must be able to process the message, which is a unsigned int named
   ...
   HRESULT /* STDMETHODCALLTYPE */ port_forward_change_callbacks::OnNewExternalIPAddress(const ::scoped_string & scopedstrNewExternalIPAddress )
   {
      ...
      informationf(tempStr+ "Change Detected in External IP Address");
