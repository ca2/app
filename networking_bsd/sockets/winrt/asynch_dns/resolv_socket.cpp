...
namespace sockets
{
   ...
   void resolv_socket::OnLine(const string & line)
   {
      ...
      if (m_bServer)
      {
         ...
         information(" *** resolv_socket server; query=%s, data=%s\n", m_query, m_data);
         ...
         // %! check cache
         {
            ...
            if(::apexacmesystem()->sockets().m_resolvcache[m_query].lookup(m_data, result))
            {
               ...
               if (time(nullptr) - ::apexacmesystem()->sockets().m_resolvtimeout[m_query][m_data] < 3600) // ttl
               {
                  ...
                  information(" *** Returning cache for [%s][%s] = '%s'\n", m_query, m_data, result);
      ...
      information(" *** resolv_socket response;  %s: %s\n", key, value);
      ...
      else if (key == "Failed" && m_parent)
      {
         ...
         if (!m_cached)
         {
            ...
            information(" *** Update cache for [%s][%s] = '%s'\n", m_query, m_data, value);
   ...
   void resolv_socket::OnDetached()
   {
      ...
      information(" *** resolv_socket::OnDetached(); query=%s, data=%s\n", m_query, m_data);
