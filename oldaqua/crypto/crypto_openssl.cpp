...
namespace str
{
   ...
   CLASS_DECL_AQUA void from(string & str, const MD5_CTX & ctx)
   {
   ...
   CLASS_DECL_AQUA void from(string & str, const WHIRLPOOL_CTX & ctx)
   {
...
CLASS_DECL_AQUA i32 crypto_decrypt2(memory& storageDecrypt, const memory& storageEncrypt, memory& key)
{
...
CLASS_DECL_AQUA i32 crypto_encrypt2(memory& storageDecrypt, const memory& storageEncrypt, memory& key)
{
   const char* iv = "skdajhgbmvkbjghs";
   EVP_CIPHER_CTX* cipher_ctx = EVP_CIPHER_CTX_new();
   auto cipher_type = EVP_aes_128_cbc();
