...
const char * callstack::get_dup(const char * pszFormat, i32 iSkip, int iCount)
{
   ...
   psz = strdup(::exception::engine().stack_trace(iSkip, m_caller_address, m_pszFormat, m_iCount));
...
CLASS_DECL_AQUA  __pointer(callstack) get_callstack(const char* pszFormat, iptr iSkip, void* caller_address, iptr iCount)
{
...
CLASS_DECL_AQUA __pointer(callstack) get_callstack(e_callstack ecallstack, int iCallStackAddUp)
{
...
//CLASS_DECL_AQUA bool is_callstack_enabled(e_callstack ecallstack)
//{
//
