...
CLASS_DECL_AQUA bool __node_further_file_is_equal(const ::file::path & path1, const ::file::path & path2);
...
namespace file
{
   ...
   CLASS_DECL_AQUA e_path get_path_type(const string & str, e_path epathForce = path_none);

   struct CLASS_DECL_AQUA path_meta :
      public cflag < ::file::e_flag >
   {
      ...
      path_meta(e_path epath = path_none, i64 iSize = -1, i32 iDir = -1, i64 iName = -1, strsize iRelative = -1, cflag < ::file::e_flag > eflag = ::file::flag_none)
      {
         ...
         cflag < ::file::e_flag >::m_eenum   = eflag.cflag < ::file::e_flag >::m_eenum;
   ...
   struct CLASS_DECL_AQUA path_os
   {
   ...
   CLASS_DECL_AQUA string normalize_path(string strPath, e_path epath = path_none);

   CLASS_DECL_AQUA bool normalize_path_inline(string & strPath, e_path & epath);
   ...
   public:
      ...
      inline path operator | (e_flag e) const { path path(*this); path |= e; return path; }

   };


   CLASS_DECL_AQUA e_path get_path_type(const string & str, e_path epathForce);
   ...
   CLASS_DECL_AQUA bool begins_eat_ci(string & str, const char * pcszPrefix);
...
CLASS_DECL_AQUA ::file::path node_full_file_path(file::path path);


CLASS_DECL_AQUA ::file::path __node_full_file_path(file::path path);


inline ::file::path CLASS_DECL_AQUA operator / (const ansichar * psz, const ::file::path & pathConcat)
{
