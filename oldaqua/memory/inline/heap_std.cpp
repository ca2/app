...
#include "aqua/memory/heap_memory.h"
...
void * aligned_memory_alloc(memsize size, memsize align)
{
   ...
   else
   {
      ...
      void * p = ::aqua::g_pheap->_alloc(sizeProvision);
...
void * unaligned_memory_alloc(memsize size)
{
   ...
   else
   {
      ...
      p = ::aqua::g_pheap->_alloc(heap_memory_unaligned_provision_get_size(size));
...
void * aligned_memory_alloc_dbg(memsize size, i32 nBlockUse, const char * szFileName, i32 nLine, memsize align)
{
   ...
   else
   {
      ...
      void * p = ::aqua::g_pheap->alloc_dbg(sizeProvision, nBlockUse, szFileName, nLine);
...
void * unaligned_memory_alloc_dbg(memsize size, i32 nBlockUse, const char * szFileName, i32 nLine)
{
   ...
   else
   {
      ...
      p = ::aqua::g_pheap->alloc_dbg(heap_memory_unaligned_provision_get_size(size), nBlockUse, szFileName, nLine);
...
void * memory_realloc_dbg(void * pmemory, memsize size, i32 nBlockUse, const char * szFileName, i32 nLine)
{
   ...
   else if(blockuse == 1) // aligned
   {
      ...
      p = ::aqua::g_pheap->realloc_dbg(heap_memory_base_get(pmemory), heap_memory_aligned_provision_get_size(size, align), heap_memory_aligned_provision_get_size(sizeOld,align), align, nBlockUse,szFileName,nLine);
   ...
   else if(blockuse == 2) // unaligned
   {
      ...
      p = ::aqua::g_pheap->_realloc(heap_memory_base_get(pmemory), heap_memory_unaligned_provision_get_size(size), heap_memory_unaligned_provision_get_size(sizeOld),0);
   ...
   else if(blockuse == 3) // unaligned
   {
      ...
      p = ::aqua::g_pheap->realloc_dbg(heap_memory_base_get(pmemory), heap_memory_unaligned_provision_get_size(size), heap_memory_unaligned_provision_get_size(sizeOld),0,nBlockUse,szFileName,nLine);
...
void memory_free_dbg(void * pmemory, i32 iBlockType)
{
   ...
   if(pheapmemory->m_blockuse == 0)
   {
      ...
      if(::aqua::g_pheap)
      {
         ...
         ::aqua::g_pheap->_free(p, iAlignedSize);
   ...
   else if(pheapmemory->m_blockuse == 1)
   {
      ...
      if(::aqua::g_pheap)
      {
         ...
         ::aqua::g_pheap->free_dbg(p, heap_memory_aligned_provision_get_size(pheapmemory->m_size, pheapmemory->m_align));
   ...
   else if(pheapmemory->m_blockuse == 2)
   {
      ...
      ::aqua::g_pheap->_free(p, iUnalignedSize);
   ...
   else if(pheapmemory->m_blockuse == 3)
   {
      ...
      ::aqua::g_pheap->free_dbg(p, heap_memory_unaligned_provision_get_size(pheapmemory->m_size));
