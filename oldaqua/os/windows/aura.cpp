...
::aqua::system * app_common_prelude(int & iError, ::create * & pmaininitdata, app_core & appcore,  HINSTANCE hinstance = nullptr, HINSTANCE hinstancePrev = nullptr, const char * pszCmdLine = nullptr, int nShowCmd = display_normal);
int app_common_term(int iError, ::aqua::system * psystem, app_core & appcore);



CLASS_DECL_AQUA i32 __cdecl _memory_type(const void * p);
...
//       ::message_box(nullptr, "Failure to initialize FreeImage (::aqua::init_core)", "FreeImage_Initialise failure", MB_ICONEXCLAMATION);
...
// #endif // USE_OS_IMAGE_LOADER
   ...
   if (g_pgdiplusStartupOutput != nullptr)
   {
      ...
      ::aqua::del(g_pgdiplusStartupInput);
      ::aqua::del(g_pgdiplusStartupOutput);
   }

   if (is_verbose())
   {

      output_debug_string(L"aqua terminating!\n");
...
CLASS_DECL_AQUA string get_error_string(u64 ui)
{
