...
class CLASS_DECL_AQUA app_core :
...
public:
   ...
   __pointer(::aqua::library)       m_plibrary;
   ...
   __result(::aqua::application) get_new_application(::object* pobjectContext);
   __result(::aqua::application) get_new_application(::object* pobjectContext, const char* pszAppId);
...
CLASS_DECL_AQUA long aura_aura(::aqua::system * pmaindata);

CLASS_DECL_AQUA bool node_fill(::aqua::system * pappcore);

//CLASS_DECL_AQUA i32 aura_entry_point(HINSTANCE hinstance, HINSTANCE hPrevInstance, LPTSTR pCmdLine, int nCmdShow, PFN_NEW_AURA_APPLICATION pfnNewAuraApplication = nullptr, PFN_NEW_AURA_LIBRARY pfnNewLibrary = nullptr);

//CLASS_DECL_AQUA int aura_entry_point(int argc, char* argv[], char* pszCommandLin, PFN_NEW_AURA_APPLICATION pfnNewAuraApplication = nullptr, PFN_NEW_AURA_LIBRARY pfnNewLibrary = nullptr);

CLASS_DECL_AQUA string transform_to_c_arg(const char * psz);
CLASS_DECL_AQUA string_array get_c_args_from_string(const char * psz);
CLASS_DECL_AQUA string_array get_c_args_from_c(const char * psz);
CLASS_DECL_AQUA string_array get_c_args_for_c(const char * psz);
CLASS_DECL_AQUA string_array get_c_args(int argc, char ** argv);
#ifdef WINDOWS_DESKTOP
CLASS_DECL_AQUA string ca2_command_line(HINSTANCE hinstance);
#else
CLASS_DECL_AQUA string ca2_command_line();
#endif


class CLASS_DECL_AQUA aura_level
{
public:
