...
#ifndef __cplusplus
#error aqua requires C++ compilation (use a .cpp suffix)
#endif

#ifdef UNDER_CE
#error This version of aqua is not currently supported for CE. Look for the CE specific version.
#endif
...
#ifdef UNICODE
#ifndef _UNICODE
#define _UNICODE        // _UNICODE is used by C-runtime/aqua API headers
#endif
#endif
...
aqua 8 has two existing validation models
...
We added ENSURE because there were too many unreported error paths in aqua and we wanted to bail out of more
...
ENSURE might seem slightly better. It is debuggable and consistent with aqua in general. In fact, many parts of aqua do just this.
...
// rather only used as a aqua class.  Additionally, the constructor and
...
///////////////////////////////////////////////////////////////////////////////
// By default, the wizards will generate new aqua object classes with this
...
#else
// If not linking to 80.DLL, use the static registrar and not building aqua.dll
#ifndef _DLL_IMPL
...
#define API CLASS_DECL_AQUA __declspec(nothrow) HRESULT __stdcall
#define API_(x)CLASS_DECL_AQUA __declspec(nothrow) x __stdcall
#define APIINL API
#define APIINL_(x) API_(x)


#define offsetofclass(aqua, derived) ((uptr)(static_cast<aqua*>((derived*)_PACKING))-_PACKING)
...
//aqua/ API code should use standard pointer to member standard syntax &MyClass::MyMethod, instead
