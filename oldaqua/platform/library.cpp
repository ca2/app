#include "framework.h"
#include "aqua/platform/static_setup.h"
...
namespace aqua
{
   ...
   bool library::open(const char * pszPath,bool bAutoClose,bool bCa2Path)
   {
      ...
      catch(...)
      {
         ...
         ERR("aqua::library::open Failed to open library %s with errors %s", (bCa2Path ? " (ca2 path)" : ""), m_strMessage.c_str());

         return false;

      }

      INFO("aqua::library::open success");
   ...
   __result(::aqua::application) library::get_new_application(::object * pobject, const char * pszAppId)
   {
...
//string_map < __pointer(::aqua::library) > * g_pmapLibCall = nullptr;


::aqua::library * lib(const char * psz)
{
   ...
   __pointer(::aqua::library) & plibrary = ::get_context_system()->m_mapLibCall[psz];
