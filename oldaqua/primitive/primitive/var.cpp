...
#if defined(WINDOWS)
//extern _locale_t ::aqua::g_localeC;
#else
#if defined(__APPLE__)
#include <xlocale.h>
#define _atof_l atof_l
#endif
//extern locale_t ::aqua::g_localeC;
#endif
...
float var::get_float(float fDefault) const
{
   ...
   #else
      return (float) _atof_l(m_str, ::aqua::g_localeC);
   #endif
   case type_pvar:
...
double var::get_double(double dDefault) const
{
   ...
   else if(m_etype == ::type_string)
   {
      ...
      d = _atof_l(m_str, ::aqua::g_localeC);
...
#else
      return _atof_l(m_str, ::aqua::g_localeC) / (double) ul;
#endif
   case ::type_pvar:
...
#else
      return (double) ul / _atof_l(var.m_str, ::aqua::g_localeC);
#endif
   case ::type_pvar:
...
#else
      if(is_scalar()
            && (fmod(_atof_l(str, ::aqua::g_localeC), 1.0) == 0.0
                && fabs(_atof_l(str, ::aqua::g_localeC)) <= pow(2.0, 31.0)))
#endif
      {
         str.trim();
         if(str.get_length() == 0)
...
void var::consume_number(const char * & psz, const char * pszEnd)
{
   ...
   if(bFloat)
   {
      ...
      double d = _atof_l(strNumber, ::aqua::g_localeC);
