...
inline void to(long  & l, i32 iBase, const ansichar * psz)
{
   ...
   inline  void           to(ansichar sz[n], const ansichar * psz)
   {
      ...
      if (strlen(psz) >= n)
      {
         ...
         __throw(::exception::exception(error_would_reach_buffer_limit));
   ...
   inline void from(string & str, const generic_object & o)
   {
   ...
   inline void from(string & str, const context_object & o)
   {
...
// besides returning a context_object (and const does not really impedes changing), do not change a string_base (or string) directly,
...
//}
   ...
   CLASS_DECL_AQUA void from(string& str, const float& f);


   CLASS_DECL_AQUA void from(string& str, const double& d);
...
namespace str
{
   ...
   inline CLASS_DECL_AQUA string from_strdup(const ansichar* psz)
   {
