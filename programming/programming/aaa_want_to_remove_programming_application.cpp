...
namespace programming
{
   ...
   void application::on_request(::create * pcreate)
   {
      ...
      if (pcreate->m_pcommandline->m_varQuery.has_property("introj"))
      {
         ...
         __pointer(::acme::library) lib = pcompiler->compile(::dir::system() / "config\\xmpp_resident\\room\\ca2@livecoding.tv\\xmpp_bot.cpp", bNew);
...
///         m_p = p.cast < plugin >();
   ...
   string application::get_visual_studio_build()
   {
      ...
      string strBuild = pcontext->m_pcontext->file().as_string(path);
