...
static BOOL xf_sw_desktop_resize(rdpContext* context)
{
   ...
   if (!(xfc->image = XCreateImage(xfc->display, xfc->draw2d, xfc->depth, ZPixmap,
                                   0,
...
BOOL xf_create_window(xfContext* xfc)
{
   ...
   if (!xfc->image)
   {
      ...
      xfc->image = XCreateImage(xfc->display, xfc->draw2d,
                                xfc->depth,
...
static const char TEST_PTR_STR [] = "Virtual aura XTEST pointer";
