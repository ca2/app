...
static void wl_shell_surface_handle_ping(void* data, struct wl_shell_surface* shell_surface, u32 serial)
{
...
static void wl_callback_done(void* data, struct wl_callback* callback, u32 time)
{
	wlfWindow* window = data;
	wlfBuffer* buffer;
	...
	::memcpy_dup(buffer->shm_data, (void*) window->data, window->width * window->height * 4);
