...
static void xf_draw_screen_scaled(xfContext* xfc, int x, int y, int w, int h)
{
   ...
   picFormat = XRenderFindVisualFormat(xfc->display, xfc->draw2d);
...
BOOL xf_sw_desktop_resize(rdpContext* context)
{
   ...
   if (xfc->image)
   {
      ...
      if (!(xfc->image = XCreateImage(xfc->display, xfc->draw2d, xfc->depth, ZPixmap, 0,
...
BOOL xf_create_window(xfContext* xfc)
{
   ...
   XFlush(xfc->display);
   if (!xfc->image)
      xfc->image = XCreateImage(xfc->display, xfc->draw2d, xfc->depth, ZPixmap, 0,
...
BOOL xf_get_pixmap_info(xfContext* xfc)
{
   ...
   {
      vi = vis + i;
      if (vi->draw2d == window_attributes.draw2d)
      {
         xfc->draw2d = vi->draw2d;
         break;
      }
   }
   ...
   {
      /*
       * Detect if the server draw2d has an inverted colormap
   ...
   if ((xfc->draw2d == NULL) || (xfc->scanline_pad == 0))
   {
      return FALSE;
   }
...
BOOL xf_post_connect(freerdp* instance)
{
   ...
   else
   {
      ...
      xf_gdi_register_update_callbacks(pupdate);
   }
   ...
   pointer_cache_register_callbacks(pupdate);

   if (!settings->SoftwareGdi)
   {
      glyph_cache_register_callbacks(pupdate);
      brush_cache_register_callbacks(pupdate);
      bitmap_cache_register_callbacks(pupdate);
      offscreen_cache_register_callbacks(pupdate);
      palette_cache_register_callbacks(pupdate);
