...
UINT32 xf_convert_rdp_order_color(xfContext* xfc, UINT32 color)
{
   ...
   case 32:
   case 24:
      if (xfc->draw2d->red_mask == 0xFF0000 &&
            xfc->draw2d->green_mask == 0xFF00 &&
            xfc->draw2d->blue_mask == 0xFF)
      {
         return color;
      }
   ...
   case 16:
      ...
      if (xfc->draw2d->red_mask == 0xF800 &&
            xfc->draw2d->green_mask == 0x07E0 &&
            xfc->draw2d->blue_mask == 0x001F)
      {
         return color;
      }
...
Pixmap xf_brush_new(xfContext* xfc, int width, int height, int bpp, BYTE* data)
{
   ...
   if (data)
   {
      ...
      image = XCreateImage(xfc->display, xfc->draw2d, xfc->depth,
...
BOOL xf_gdi_bitmap_update(rdpContext* context, BITMAP_UPDATE* bitmapUpdate)
{
   ...
   for (index = 0; index < bitmapUpdate->number; index++)
   {
      ...
      image = XCreateImage(xfc->display, xfc->draw2d, xfc->depth, ZPixmap, 0,
