...
int xf_keyboard_execute_action_script(xfContext* xfc, XF_MODIFIER_KEYS* mod, KeySym keysym)
{
	...
	if (mod->Shift)
		ansi_concatenate(combination, "Shift+");

	if (mod->Ctrl)
		ansi_concatenate(combination, "Ctrl+");

	if (mod->Alt)
		ansi_concatenate(combination, "Alt+");

	ansi_concatenate(combination, keyStr);
