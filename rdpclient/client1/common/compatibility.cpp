...
BOOL freerdp_client_old_parse_hostname(char* str, char** ServerHostname, unsigned int* ServerPort)
{
	char* p;
...
int freerdp_client_parse_old_command_line_arguments(int argc, char** argv, rdpSettings* settings)
{
	...
	do
	{
		...
		CommandLineSwitchCase(arg, "g")
		{
			...
			free(str);
			WLog_WARN(TAG,  "-g %s -> /int_size:%s or /w:%d /h:%d", arg->Value, arg->Value,
