...
void nsUniversalDetector::HandleData(const ::string & aBuf, PRunsigned int aLen)
{
    ...
    if (aBuf[i] & '\x80' && aBuf[i] != '\xA0')  //Since many Ascii only page contains NBSP 
    {
      ...
      if (mInputState != eHighbyte)
      {
        ...
        if (nsnull == mCharSetProbers[0])
          mCharSetProbers[0] = ___new nsMBCSGroupProber();
        if (nsnull == mCharSetProbers[1])
          mCharSetProbers[1] = ___new nsSBCSGroupProber();
        if (nsnull == mCharSetProbers[2])
          mCharSetProbers[2] = ___new nsLatin1Prober(); 
      }
    }
    else
    {
  ...
  case eEscAscii:
    if (nsnull == mEscCharSetProber)
      mEscCharSetProber = ___new nsEscCharSetProber();
