...
#define IDLE_PARSE(time) (100+time*4)
   ...
   void base_editor::remapLRS(bool recreate)
   {
      if (recreate || lrSupport == nullptr)
      {
         ...
         if (regionCompact)
         {
            lrSupport = ___new LineRegionsCompactSupport();
         }
         else
         {
            lrSupport = ___new LineRegionsSupport();
         }
   ...
   PairMatch *base_editor::getPairMatch(index lineNo, index linePos)
   {
      ...
      if (pair != nullptr)
      {
         PairMatch *pm = allocateø< PairMatch(pair, lineNo, pair->region->hasParent >(def_PairStart));
         pm->setStart(pair);
         return pm;
      }
   ...
   void base_editor::validate(index lno, bool rebuildRegions)
   {
      ...
      /*
      * Calculate changes, required by aaa_memory_new screen position, if any
      */
      if (lrSize != wSize*2)
      {
         lrSize = wSize*2;
