...
namespace colorertake5
{
   ...
   {
      ::collection::count iOldLineCount = lineRegions.get_size();
      lineRegions.allocate(lineCount);
      for(::collection::index i = iOldLineCount; i < lineCount; i++)
      {
   ...
   ::collection::count LineRegionsSupport::get_size()
   {
      return lineCount;
   }
   ...
   void LineRegionsSupport::clear()
   {
      ...
      for(::collection::index idx = 0; idx < lineRegions.get_size(); idx++)
      {
   ...
   ::collection::index LineRegionsSupport::getLineIndex(index lno) const
   {
   ...
   ::collection::index LineRegionsSupport::getFirstLine()
   {
      return firstLineNo;
   }
   ...
   void LineRegionsSupport::clearLine(index lno, const ::scoped_string & scopedstrLine)
   {
      ...
      LineRegion *lfirst = allocateø< LineRegion(*schemeStack.last >());
      lfirst->start = 0;
      lfirst->end = -1;
      lfirst->next = nullptr;
      lfirst->prev = lfirst;
   ...
   void LineRegionsSupport::addRegion(index lno, const ::string &line, character_count sx, character_count ex, class region* region)
   {
   ...
   void LineRegionsSupport::enterScheme(index lno, const ::string &line, character_count sx, character_count ex, class region* region, class scheme *scheme)
   {
      __UNREFERENCED_PARAMETER(line);
   ...
   void LineRegionsSupport::leaveScheme(index lno, const ::string &line, character_count sx, character_count ex, class region* region, class scheme * scheme)
   {
      __UNREFERENCED_PARAMETER(line);
      ...
      {
         LineRegion *lr = allocateø< LineRegion(*schemeStack.last >());
         lr->start = ex;
         lr->end = -1;
