...
#define __TIMESTAMP__ "28 May 2006"
   ...
   void ParserFactory::init()
   {
      ...
      while(elem != nullptr)
      {
         ...
         if (elem->get_type() == xml::node_element && elem->get_name() == "hrc-sets")
         {
            ...
            {
               string str = pcontext->m_pcontext->file().as_string(logLocation);
   ...
   string ParserFactory::searchPath()
   {
      ...
      return pcontext->m_pcontext->dir().matter("colorer.zip:catalog.xml");
   ...
   HRCParser* ParserFactory::getHRCParser()
   {
      ...
      for(i32 idx = 0; idx < hrcLocations.get_size(); idx++)
      {
         ...
         if(pcontext->m_pcontext->dir().is(path))
         {
            ...
            {
               if(!pcontext->m_pcontext->dir().is(patha[i]))
               {
                  string str = pcontext->m_pcontext->file().as_string(patha[i]);
                  try
                  {
         ...
         {
            string strSource = pcontext->m_pcontext->file().as_string(path);
      ...
      for(i32 idx = 0; idx < hrdLocV->get_size(); idx++)
      {
         if (hrdLocV->element_at(idx).has_char())
         {
            ...
            try
            {
               ...
               stream spfile(pcontext->m_pcontext->file().get_file(strPath, ::file::e_open_read | ::file::e_open_binary), FIRST_VERSION);
   ...
   TextHRDMapper *ParserFactory::createTextMapper(string nameID)
   {
      ...
      for(i32 idx = 0; idx < hrdLocV->get_size(); idx++)
      {
         if (hrdLocV->element_at(idx).has_char())
         {
            try
            {
               stream stream(pcontext->m_pcontext->file().get_file(hrdLocV->element_at(idx), ::file::e_open_read |::file::e_open_binary), FIRST_VERSION);
