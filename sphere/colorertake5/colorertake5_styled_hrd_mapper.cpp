...
//  DocumentBuilder docbuilder;
      ...
      for(::pointer<::xml::node>urel = hbase.first_child(); curel; curel = curel->get_next_sibling())
      {
   ...
   */
   ...
   void StyledHRDMapper::setRegionDefine(const ::scoped_string & scopedstrName, RegionDefine *rd)
   {
      RegionDefine *rd_old = regionDefines[name];
      delete rd_old;

      const StyledRegion *new_region = StyledRegion::cast(rd);
      RegionDefine *rd_new = ___new StyledRegion(*new_region);
