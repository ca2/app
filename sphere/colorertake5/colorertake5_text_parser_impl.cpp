      ...
      while(true)
      {
         ...
         if (color32 == 0 && right-left == 1)
         {
            ...
            if (!node->kwList->kwList[pos].isSymbol)
            {
               if (!node->worddiv)
               {
                  if (gx && (unicode_is_letter_or_digit(&((const ::string &)str)[gx-1]) || ((const ::string &)str)[gx-1] == '_')) badbound = true;
                  if (gx + kwlen < lowlen &&
                        (unicode_is_letter_or_digit(&((const ::string &)str)[gx + kwlen]) || ((const ::string &)str)[gx + kwlen] == '_')) badbound = true;
               }
               else
               {
      ...
      for(index idx = 0; idx < cscheme->nodes.get_size(); idx++)
      {
         ...
         case SNT_SCHEME:
         {
            ...
            if (updateCache)
            {
               ...
               if (forward)
               {
                  forward->next = memory_new parse_cache;
               ...
               {
                  forward = memory_new parse_cache;
   ...
   bool text_parser_impl::colorize(cregexp *root_end_re, bool lowContentPriority)
   {
      ...
      for (; gy < gy2; )
      {
         ...
         if (clearLine != gy)
         {
            ...
            /*if (str == nullptr){
               throw ::exception(exception(string("nullptr string passed into the parser: ")+as_string(gy)));
               gy = gy2;
               break;
            };*/
