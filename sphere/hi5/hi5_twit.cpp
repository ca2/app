...
namespace hi5
{
   ...
   *--*/
   twit::twit(::particle * pparticle, simple_log * psimplelog, ::collection::index iLogTarget) :
      ...
      TWIT_DIRECTMESSAGENEW_URL("http://twitter.com/direct_messages/aaa_memory_new.xml"),
...
//   }
   ...
   *--*/
   bool twit::performDelete( const string & deleteUrl )
   {
      ...
      /* Send http request */
      bool bOk = http()->get(deleteUrl, m_strResponse, set);
   ...
   *--*/
   bool twit::performPost( const string & postUrl, property_set & set)
   {
      ...
      if (set.has_property("headers"))
      {
         ...
         bOk = http()->get(postUrl, m_strResponse, set);
      ...
      else
      {
         ...
         bOk = http()->get(postUrl, m_strResponse, setHttp) && setHttp["get_status"].estatus() == ::success;
