...
namespace mail
{
   ...
   void pop3_socket::OnLine(const string & line)
   {
      ...
      case state_transaction:
         ...
         if(stra.get_size() > 0 && stra[0] == "+OK")
         {
            if(case_insensitive_string_begins(m_ppop3->get_transaction(), "UIDL"))
            {
               m_estate = state_listmessages_start;
            }
            else if(case_insensitive_string_begins(m_ppop3->get_transaction(), "STAT"))
            {
               m_estate = state_stat_start;
            }
            else if(case_insensitive_string_begins(m_ppop3->get_transaction(), "TOP"))
            {
               m_estate = state_top_start;
            }
            else if(case_insensitive_string_begins(m_ppop3->get_transaction(), "RETR"))
            {
               m_estate = state_retr_start;
            }
            else if(case_insensitive_string_begins(m_ppop3->get_transaction(), "LIST") && m_ppop3->get_transaction().get_length() >= 6)
            {
               m_estate = state_receive_list;
            }
            else if(case_insensitive_string_begins(m_ppop3->get_transaction(), "RETRHD"))
            {
