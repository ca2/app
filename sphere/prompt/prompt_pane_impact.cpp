...
namespace prompt
{
   ...
   void pane_impact::on_change_cur_sel()
   {
      ...
      else if(get_impact_id() == CONTEXT_MENU_IMPACT)
      {
         ...
         pdocument->browse(dir()->appdata()/ "command/menu", ::e_source_system);
      }
      else
      {
   ...
   void pane_impact::on_create_impact(::user::impact_data * pimpactdata)
   {
      ...
      case CONTEXT_MENU_IMPACT:
      {
         ...
         bool bOk = pdata->open(get_app(), dir()->appdata() / "command/menu", ::e_source_system);
...
//      break;
      case impact_three_action_launch:
      {
         ...
         //auto pdocument = pdata->browse(dir()->appdata() / "co
         pdata->open(get_app(), dir()->appdata() / "command\\3-action-launch", ::e_source_system);
