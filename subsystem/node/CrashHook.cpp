...
#include "platform/RegistryKey.h"
...
namespace subsystem
{
   ...
   ::pointer < ::subsystem::RegistryKey > CrashHook::m_prootHkey;
   ...
   CrashHook::CrashHook(ApplicationCrashEvents *notifier)
   {
      ...
      if (!m_prootHkey)
      {
         ...
         m_prootHkey = allocateø ::subsystem::RegistryKey(MainSubsystem().Registry().getCurrentUserKey(), "remoting crashes");
   ...
   void CrashHook::setHklmRoot()
   {
     critical_section_lock al(&m_guiEnabledMutex);
         ...
         m_prootHkey = new ::subsystem::RegistryKey(MainSubsystem().Registry().getLocalMachineKey(), "remoting crashes");
   }
