...
namespace subsystem
{
   ...
   const static ulong bigbyte[24] = {0x800000L, 0x400000L, 0x200000L, 0x100000L, 0x80000L, 0x40000L,
   ...
   const static ::u8 pc1[56] = {56, 48, 40, 32, 24, 16, 8,  0,  57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42,
                                         34, 26, 18, 10, 2,  59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6,  61, 53,
                                         45, 37, 29, 21, 13, 5,  60, 52, 44, 36, 28, 20, 12, 4,  27, 19, 11, 3};

   const static ::u8 totrot[16] = {1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28};

   const static ::u8 pc2[48] = {13, 16, 10, 23, 0,  4,  2,  27, 14, 5,  20, 9,  22, 18, 11, 3,
   ...
   void DesCrypt::encrypt(::u8 *dst, const ::u8 *src, size_t dataLen, const ::u8 *key)
   {
      deskey(key, ENCRYPT);
   ...
   void DesCrypt::decrypt(::u8 *dst, const ::u8 *src, size_t dataLen, const ::u8 *key)
   {
      deskey(key, DECRYPT);
      for (size_t i = 0; i < dataLen; i += 8)
      {
