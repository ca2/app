...
static inline i32 FLOATTIME_DateFromJulian(i32 dateIn)
{
...
/*static inline void FLOATTIME_DMYFromJulian(i32 jd, USHORT *year, USHORT *month, USHORT *day)
{
  i32 j, i, l, n;
...
//    WARN("Ignoring VAR_VALIDDATE\n");
  ...
  /* Date */
  dateVal = FLOATTIME_DateFromJulian((i32) FLOATTIME_JulianFromDMY(ud.st.wYear, ud.st.wMonth, ud.st.wDay));
...
//xxx        if (dwLen && !strncmpiW(strIn, tokens[i], dwLen))
        //if (dwLen && !ansi_count_compare_ci(strIn, tokens[i], dwLen))
//         if(dwLen && 0)
         if(false)
        {
          if (i <= 25)
      ...
      if (!bFound)
      {
        ...
        {
          /* Special case - 'a' and 'p' are recognised as i16 for am/pm */
          if (*strIn == 'a' || *strIn == 'A')
          {
...
//xxx  TRACE("(%g,0x%08x,%p)\n", dateIn, dwFlags, lpUdate);
  ...
  /* Compensate for i32 truncation (always downwards) */
...
i32 __cdecl FloatTimeToSystemTime(double vtime, struct _SYSTEMTIME * pst)
{
...
namespace datetime
{
   ...
   float_time &float_time::operator=(const FILETIME &filetimeSrc) RELEASENOTHROW
   {
    ...
    float_time WINAPI float_time::GetCurrentTime() RELEASENOTHROW
    {
        ...
        return float_time(::_time64(nullptr));
        
#else
        
        return float_time(::time(nullptr));
