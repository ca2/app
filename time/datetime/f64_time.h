...
CLASS_DECL_CA2_TIME int32_t SystemTimeToFloatTime(LPSYSTEMTIME lpSt, ::f64 *pDateOut);
CLASS_DECL_CA2_TIME int32_t FloatTimeToSystemTime(::f64 dateIn, LPSYSTEMTIME lpSt);
...
namespace datetime
{
   ...
   public:
      ...
      ::f64 GetTotalDays() const RELEASENOTHROW;    // span in days (about -3.65e6 to 3.65e6)
      ::f64 GetTotalHours() const RELEASENOTHROW;   // span in hours (about -8.77e7 to 8.77e6)
      ::f64 GetTotalMinutes() const RELEASENOTHROW; // span in minutes (about -5.26e9 to 5.26e9)
      ::f64 GetTotalSeconds() const RELEASENOTHROW; // span in seconds (about -3.16e11 to 3.16e11)
   ...
   protected:
      static ::f64 WINAPI DoubleFromDate(FLOAT_DATE date) RELEASENOTHROW;
      static FLOAT_DATE WINAPI DateFromDouble(::f64 f) RELEASENOTHROW;
...
inline f64_time_span::f64_time_span(::f64 dblSpanSrc) RELEASENOTHROW :
   ...
   int32_t nSecs) RELEASENOTHROW
   {
      ...
      m_span = lDays + ((::f64)nHours)/24 + ((::f64)nMins)/(24*60) +
      ((::f64)nSecs)/(24*60*60);
      m_estatus = e_status_valid;
      check_range();
   }
...
inline f64_time::f64_time(const FILETIME& file_timeSrc) RELEASENOTHROW :
   ...
   inline f64_time& f64_time::operator +=(f64_time_span dateSpan) RELEASENOTHROW
   {
      ASSERT(get_status() == e_status_valid);
      ASSERT(dateSpan.get_status() == e_status_valid);
      m_dt = DateFromDouble( DoubleFromDate( m_dt )+(::f64)dateSpan );
      return *this;
   }
   ...
   inline FLOAT_DATE WINAPI f64_time::DateFromDouble(::f64 f) RELEASENOTHROW
   {
      ...
      ::f64 fTemp = floor(f); // fTemp is now whole part
...
//            ATLTRACE(atlTraceTime, 0, _T("\nCOleDateTime VarDateFromStr call failed.\n\t"));
   ...
   inline string f64_time::Format(LPCTSTR pFormat) const
   {
      ...
      string strDate;
      char_pointer lpszTemp = strDate.GetBufferSetLength(256);
