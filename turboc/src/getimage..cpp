...
/*   Handle = dynamic_cast < image*>(App(tc().get_context_application()).alloc(Sys(tc().get_context_application()).type_info< ::image_pointer >()));
...
/*   Buffer->Handle->from(tc().m_pimage);
...
/*   tc().m_pimage->from(Buffer->Handle);
   setviewport (Left, Top, Right, Bottom, Clip);
   if (OldWritemode != TcWritemode)
      setwritemode (OldWritemode);
#undef Buffer
