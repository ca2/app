...
// constants actually used by Turbo C.  If desirable, we can add ___new
...
//
// Note that if we add ___new possibilities later -- say, 1600x1200 --
...
CLASS_DECL_TURBOC void _graphfreemem (void *ptr, unsigned int_size);
CLASS_DECL_TURBOC void *_graphgetmem (unsigned int_size);
...
CLASS_DECL_TURBOC void int_rectangle (int left, int top, int right, int bottom);
