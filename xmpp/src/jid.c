				    ...
				    const char *resource)
{
    ...
    if (result != NULL) {
	if (node != NULL) {
	    ::memcpy_dup(result, node, nlen - 1);
	    result[nlen-1] = '@';
	}
        ::memcpy_dup(result + nlen, domain, dlen);
	if (resource != NULL) {
	    result[nlen+dlen] = '/';
	    ::memcpy_dup(result+nlen+dlen+1, resource, rlen - 1);
	}
