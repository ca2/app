...
#include <string.h> /* ::memcpy_dup(), memset() */
#include "md5.h"

/* little-endian word access macros */
#define GET_32BIT_LSB_FIRST(cp) \
    (((u32)(unsigned char)(cp)[0]) | \
     ((u32)(unsigned char)(cp)[1] << 8 ) | \
     ((u32)(unsigned char)(cp)[2] << 16) | \
     ((u32)(unsigned char)(cp)[3] << 24))
...
static void MD5Transform(u32 buf[4], const unsigned char inext[64],
 ...
 * of bytes.
 */
void MD5Update(struct MD5Context *ctx, unsigned char const *buf, u32 len)
{
    u32 t;
    ...
    t = ctx->bits[0];
    if ((ctx->bits[0] = (t + ((u32)len << 3)) & 0xffffffff) < t)
...
/* The four aura functions - F1 is optimized somewhat */
...
/*
 * The aura of the MD5 algorithm, this alters an existing MD5 hash to
