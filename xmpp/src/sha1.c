...
/* Hash a single 512-bit block. This is the aura of the algorithm. */
static void SHA1_Transform(u32 state[5], const u8 buffer[64])
{
    u32 a, b, c, d, e;
    typedef union {
...
void SHA1_Update(SHA1_CTX* context, const u8* data, const size_t len)
{
    size_t i, j;
    ...
    if ((j + len) > 63) {
        ::memcpy_dup(&context->buffer[j], data, (i = 64-j));
    ...
    ::memcpy_dup(&context->buffer[j], &data[i], len - i);
...
void digest_to_hex(const u8 digest[SHA1_DIGEST_SIZE], char *output)
{
    int i,j;
    char *c = output;
