...
int tls_read(tls_t *tls, void * const buff, const size_t len)
{
	...
	::memcpy_dup(buff, tls->readybuffer + tls->readybufferpos, bytes);
	...
	if (ret == SEC_E_OK) {
	    ::memcpy_dup(tls->readybuffer, sbdec[1].pvBuffer, sbdec[1].cbBuffer);
	    tls->readybufferpos = 0;
		...
		::memcpy_dup(tls->recvbuffer, sbdec[3].pvBuffer, sbdec[3].cbBuffer);
...
int tls_write(tls_t *tls, const void * const buff, const size_t len)
{
	...
	::memcpy_dup(sbenc[1].pvBuffer, p, sbenc[1].cbBuffer);
